  ...
  render() {
    ...
    const {previewVisible,previewImage} = this.state;
        ...
        if (this.props.title === undefined) {
            fileTotal = (
                <div >{this.$({ id: "upload.information" }/*附件信息*/)} <Badge showZero count={fileList.length} offset={[-16, 0]} style={fileList.length === 0 && { backgroundColor: '#52c41a' }} /></div>
            );
        } else {
        ...
        let upload = (
            <Upload name="file"
                ...
                {this.props.disabled ? '' : <Button><Icon type="upload" /> {this.props.buttonText || this.$({ id: 'constants.approvelHistory.addAttachment' })}</Button>}
            </Upload>
        );
