...
class UploadButton extends React.Component {
  ...
  preview = record => {
    this.setState({
      previewVisible: true,
      previewImage: record.response ? record.response.thumbnailUrl : record.thumbnailUrl,
    });
  };
  ...
  render() {
    ...
    fileList.map(item => {
      ...
      item['url'] = `${
        config.baseUrl
      }/api/attachments/download/${attachmentOID}?access_token=${localStorage.getItem('token')}`;
    });
    let fileTotal;
                ...
                <Row key={item.uid} className="file-item">
                  ...
                  >
                    <i
                      ...
                      style={{ color: item.status == 'error' ? 'red' : '' }}
                    />
                    {type !== 'IMAGE' ? (
                      <a
                        href={`${
                          config.baseUrl
                        }/api/attachments/download/${attachmentOID}?access_token=${localStorage.getItem(
                          'token'
                        )}`}
                        style={{ marginLeft: 10, color: item.status == 'error' ? 'red' : '' }}
                      >
