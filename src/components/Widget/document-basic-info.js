        ...
        <Col style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} span={16}>
          {
            linkId ?
              ...
              : <span title={value} className="content">{value}</span>
          }
        </Col>
      </Row>
    )
  ...
  render() {
    ...
    return (
      <Spin spinning={detailLoading}>
        <div style={{ marginBottom: "14px", marginTop: 14 }} className="header-title">
        ...
        <Row>
            ...
            <Row>
              <Col span={24}>
                <Row style={{ fontSize: '12px', lineHeight: "32px", overflow: "hidden", }} className="list-info">
                  <Col span={2}>
                    <Row>
                      <Col style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textAlign: "right" }} span={20}>
                        <span>{this.$t("common.comment")}</span>
                      </Col>
                      <Col span={4} style={{ textAlign: "center" }}>:</Col>
                    </Row>
                  </Col>
                  <Col style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} span={22}>
                    <span title={data.remark} className="content">{data.remark}</span>
                  </Col>
                </Row>
              </Col>
            </Row>
          ...
          <Col span={8}>
            ...
            <div style={{ float: 'right', marginRight: '50px' }}>
              <div style={{ textAlign: "right", fontSize: 14 }} className="status-title">{this.$t('common.column.status')}</div>
              <div style={{ fontSize: "20px" }} className="status-content">{this.$statusList[data.statusCode] ? this.$statusList[data.statusCode].label : data.statusCode}</div>
            </div>
          </Col>
        </Row>
        <Modal bodyStyle={{ height: '50vh' }} visible={previewVisible} footer={null} onCancel={this.previewCancel} closable={false}>
