  ...
  render() {
    ...
    const uploadButton = (
      <div>
        ...
        <div className="ant-upload-text" style={{ marginTop: 8, color: '#666' }}>{this.$t('common.upload')/*上传*/}</div>
      </div>
    );
    ...
    return (
      <div className="image-upload">
        ...
        {this.props.showMaxNum && !this.props.disabled && (
          <div>({this.$t('common.max.upload.image', {max: this.props.maxNum})/*最多上传 {max} 张图片*/})</div>
        )}
        {fileList.length > 0 &&
...
export default connect(mapStateToProps, null, null, { withRef: true })(ImageUpload);
