...
class FileUpload extends React.Component{
  ...
  handleChange = (info) => {
    ...
    if (this.props.fileSize) {  //控制上传图片的大小
      ...
      if (!isLtSize && info.file.status !== 'removed') {
        //您上传的附件已超出大小限制，请重新上传！
        message.error(this.$t('components.file.upload.err1', {name: info.file.name, total: this.props.fileSize}));
        return
      }
    }
  ...
  getImage = (file) => {
    ...
    if(imageExtension.has(extension))
      return file.thumbnailUrl?file.thumbnailUrl:file.fileURL;
  ...
  deleteImage = () => {
    ...
    let required = false;
    let attachmentOid = info.response ? (info.response.attachmentOID ? info.response.attachmentOID : info.response.attachmentDTO.attachmentOID) : info.attachmentOID;
      ...
      <div className="file-upload">
        ...
        }
        {
          this.props.showMaxNum && !this.props.disabled && (
            <div>({this.$t('common.max.upload.attachment', {max: this.props.maxNum})/*最多上传 {max} 个附件*/})</div>
        )}
        {
          !this.props.disabled &&
            <div>
        ...
        {imageList.length > 0 &&
        <ImageViewer visible={previewVisible}
        ...
        <Modal onOk={this.deleteImage}
               onCancel={this.closeModal}
               okText={this.$t('common.ok')}
               cancelText={this.$t("common.cancel")}
               visible={modalShow}
               title={this.$t('components.file.upload.operate')}>
          <p style={{textAlign:'center',fontSize:'20px',margin:'10px 0px'}}>{this.$t('components.file.upload.ensureDelete')}</p>
        </Modal>
      </div>
    )
  }
