...
class ApproveHistoryWorkFlow extends React.Component {
  ...
  getHistoryRender(item, i) {
    if (item) {
      ...
      return (
        <Timeline.Item dot={model.dot ? <Icon type={model.dot}></Icon> : ""} color={model.color} key={i}>
          <Row>
            <Col span={3}>
              <div style={{ fontWeight: 'bold' }}>{model.text}</div>
            </Col>
            <Col span={4}>
              <div style={{ color: "rgba(0,0,0,0.5)" }}>{moment(item.lastModifiedDate).format('YYYY-MM-DD HH:mm:ss')}</div>
            </Col>
            <Col span={5}>
              ...
              <Collapse.Panel header={this.$t('expense.approval.history')} key="1">
                <div style={{ paddingTop: 10, paddingLeft: 15 }}>
                  {this.props.infoData.length ? (<Timeline>
                    {this.getHistory()}
                  </Timeline>) : (<div style={{ textAlign: "center" }}>{this.$t('acp.no.data')}</div>)}
                </div>
              </Collapse.Panel>
            </Collapse>
          </div>
