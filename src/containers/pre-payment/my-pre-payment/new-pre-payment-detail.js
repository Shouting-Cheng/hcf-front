...
import { Form, Button, Input, Row, Col, Select, InputNumber, DatePicker, message, Steps, Modal, Tag, Icon, Spin } from 'antd'
...
class NewPrePaymentDetail extends React.Component {
  ...
  componentDidMount() {
    console.log("1111");
      this.setState({
        ...
        selectedData: this.props.params.record.contractLineId ? [this.props.params.record.contractLineId] : [],
        contract: {
          lineNumber: this.props.params.record.contractLineNumber,
        ...
        if (this.state.params.id) {
          this.getReceivables(this.props.params.record.partnerId, this.props.params.record.partnerCategory);
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.params.flag && !nextProps.params.flag) {
      this.setState({ flag: false, contract: {}, bankInfos: [], partnerInfo: {}, contractValue: [], dueDate: "", lineNumber: "" });
    ...
    if (nextProps.params.flag && !this.state.flag) {
      this.setState({
        ...
        selectedData: nextProps.params.record.contractLineId ? [nextProps.params.record.contractLineId] : [],
        contract: {
          lineNumber: nextProps.params.record.contractLineNumber,
        ...
        if (this.state.params.id) {
          this.getReceivables(nextProps.params.record.partnerId, nextProps.params.record.partnerCategory);
          this.getCashTransactionList();
  ...
  getCashTransactionList = () => {
    ...
    httpFetch.get(`${config.prePaymentUrl}/api/cash/pay/requisition/types/queryTransactionClassByTypeId/${this.state.paymentReqTypeId}`).then(res => {
  ...
  handleSave = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        values.contractLineNumber = this.state.contract.lineNumber ? this.state.contract.lineNumber : "";
  ...
  onPartnerCategory = (value) => {
    ...
    this.setState({ selectedData: [], accountNumber: "", accountName: "", bankInfos: [], partnerInfo: {}, contractValue: [], contract: {} });
  }
  //获取收款方数据
    ...
    if (record) {
      ...
      if (payeeCategory === "EMPLOYEE") {
        reimburseService.getAccountByUserId(record.key).then(res => {
          ...
          accountList.length === 0 && (message.warning("该收款方没有银行信息，请先维护改收款方下银行信息！"));
          this.setState({ selectedData: [], contractValue: [], payeeId: record.key, payeeName: record.label, bankInfos: accountList });
        })
      }
  ...
  render() {
    ...
    const { fetching, partnerdSelectedData, bankInfos, loading, params, accountName, number, contract, contractValue, dueDate, lineNumber, availableAmount, amount, currency, showListSelector, partnerCategoryOptions, currencyList, payWayTypeList, contractParams, partnerInfo } = this.state;
    ...
    return (
      <div className="new-pay-plan">
        <Form onSubmit={this.handleSave}>
          ...
          <Row gutter={8}>
            <Col span={8} className="ant-form-item-label label-style">预付款金额：</Col>
              ...
              <FormItem className="ant-col-offset-1">
                {
                  getFieldDecorator('amount', {
                    ...
                    initialValue: params.id ? Number(this.filterMoney(params.amount, 2, true).replace(/,/g, "")) : ''
                  })(
                    <InputNumber precision={2} placeholder="请输入" style={{ width: '100%' }} onMouseOut={this.onAmountMouseMove} />
                    )
                }
              </FormItem>
            </Col>
          ...
          <FormItem  {...formItemLayout} label="收款方">
            {getFieldDecorator('partnerd', {
              ...
              initialValue: params.id ? { key: partnerInfo.code, label: partnerInfo.name } : { key: '', label: '' }
            })(
              <SelectReceivables onChange={this.handle} type={this.props.form.getFieldValue("partnerCategory")} disabled={!this.props.form.getFieldValue("partnerCategory")} />
              )
            }
          </FormItem>
                ...
                <Select onChange={this.accountNumberChange}>
                  {
                    bankInfos.map(item => {
                      return (
                        <Option key={item.bankAccountNo} value={item.bankAccountNo}>{item.bankAccountNo}</Option>
                      )
                    })
                  }
                </Select>)
          ...
          <FormItem {...formItemLayout} label="关联申请">
            {
              getFieldDecorator('application', {
                ...
                initialValue: (params.id && params.isApply) ? [{ businessCode: params.refDocumentCode, id: params.refDocumentId }] : []
              })(
            ...
            <Row gutter={8}>
              ...
              <Col span={16}>
                ...
                <div style={{ marginTop: '8px' }}>
                  {contractValue.length == 0 ? "注：根据收款方选择合同" : `付款计划序号：${lineNumber} | 付款计划日期：${moment(dueDate).format("YYYY-MM-DD")}`}
                </div>
              </Col>
              <Col span={4} style={{ textAlign: "left" }} className="ant-form-item-label">
                {contractValue.length > 0 && <a onClick={() => this.detail(contract.contractId)}>查看详情</a>}
              </Col>
            </Row>
          </div>
