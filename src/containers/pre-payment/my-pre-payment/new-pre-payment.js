...
class MyNewPrePayment extends React.Component {
  ...
  handleSave = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        prePaymentService.addPrepaymentHead(dataValue).then(res => {
          this.setState({
            loading: false
          });
  ...
  render() {
    ...
    const { model, loading, fileList, departmentId, pageLoading, user, contractTypeDisabled, isNew, data, unitIdOptions, partnerCategoryOptions, currencyOptions, companyIdOptions, contractCategoryOptions, selectorItem, extraParams } = this.state;
                ...
                <FormItem label="公司" {...formItemLayout}>
                  {
                    getFieldDecorator('companyId', {
                      ...
                      initialValue: isNew ? [{ id: this.props.user.companyId, name: this.props.user.companyName }] : [{ id: model.companyId, name: model.companyName }]
                    })(
                      <Chooser
                        type='company'
                        labelKey='name'
                ...
                <FormItem label='附件信息' {...formItemLayout}>
                  {
                    getFieldDecorator('attachmentOID')(
                      <Upload attachmentType="BUDGET_JOURNAL"
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedMyNewPrePayment)
