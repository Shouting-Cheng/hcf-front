...
class PrePaymentCommon extends React.Component {
    ...
    this.state = {
      ...
      columns:
        [
          ...
          {
            title: '预付款金额', dataIndex: 'amount', width: 120,
            render: (value, record) => {
              return (
                <div>
                  <div style={{ textAlign: "left", width: '20%' }}>{record.currency}&nbsp;&nbsp;{this.filterMoney(record.amount, 2)}</div>
                </div>
              )
            }
          },
          {
            title: '预付款类型', width: 110, dataIndex: 'cshTransactionClassName', align: "center",
            render: (cshTransactionClassName) => {
              return (
                <Popover content={cshTransactionClassName}>
          ...
          {
            title: '收款对象', width: 80, dataIndex: 'currency', align: "center",
            render: (value, record) => {
              return (
                <div>
          ...
          {
            title: '收款账户', width: 210, dataIndex: 'partnerId',
            render: (value, record) => {
              return (
                <div>
          ...
          {
            title: '付款属性', dataIndex: 'refDocumentCode', width: 200,
            render: (value, record) => {
              return (
                <Popover content={
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.params.status === 1001 || nextProps.params.status === 1003 || nextProps.params.status === 1005) {
  ...
  getApproveHistory = () => {
    ...
    prePaymentService.getApproveHistoryWorkflow(oid).then(res => {
  ...
  toContractDetail = (contractId) => {
    let url = this.state.ContractDetail.url.replace(':id', contractId).replace(':from', "pre-payment");
    window.open(url, "_blank");
  }
  //获取单据总金额
  getAmountByHeadId = () => {
    prePaymentService.getAmountByHeadId(this.props.id).then(res => {
  ...
  getList = () => {
    ...
    prePaymentService.getLineByHeadId(params).then(res => {
  ...
  back = () => {
    ...
    if (!formOid) {
      prePaymentService.back(id, this.props.user.id).then(res => {
        if (res.status === 200) {
  ...
  deleteItem = (e, record) => {
    e.preventDefault();
    let url = `${config.prePaymentUrl}/api/cash/prepayment/requisitionHead/deleteLineById?lineId=${record.id}`;
    this.setState({ planLoading: true });
    ...
    return (
      <div>
        <Row>
          ...
          <Col span={6} offset={1}>汇率日期：</Col>
          <Col span={6}>汇率：{record.exchangeRate}</Col>
          <Col span={5}> 本币金额：{record.currency}&nbsp; {this.filterMoney(record.functionAmount, 2, true)}</Col>
        </Row>
          ...
          record.refDocumentCode
            ...
            <div>
              ...
              <Row>
                ...
                <Col span={6}>申请单金额：{record.currency}&nbsp;{this.filterMoney(record.refDocumentTotalAmount, 2, true)}</Col>
              </Row>
            </div>
            :
            null
        ...
        {
          Number(record.returnAmount) === 0 && Number(record.payAmount) === 0 ?
            null
            :
            <div>
              <Divider />
              <Row>
                ...
                <Col span={6} offset={1}>已付款总金额：{record.currency}&nbsp;{this.filterMoney(record.payAmount, 2, true)}</Col>
                <Col span={6}>退款总金额：{record.currency}&nbsp;{this.filterMoney(record.returnAmount, 2, true)}</Col>
              </Row>
            </div>
        }
                ...
                return (
                  <div>
                    ...
                    <Row>
                      ...
                      <Col span={6}>被核销金额：{record.currency}&nbsp;{this.filterMoney(item.writeOffAmount, 2, true)}</Col>
                      <Col span={5}>交易日期：{moment(item.tranDate).format('YYYY-MM-DD')}</Col>
                      <Col span={4}>核销状态：{item.reportStatus === 'p' ? '核销中' : '核销完成'}</Col>
                    </Row>
                  </div>
                )
              } else {
  ...
  render() {
    ...
    if (headerData.status === 1001 || headerData.status === 1003 || headerData.status === 1005) {
      status =
        (
          <h3 className="header-title">
            <Button type="primary" onClick={this.edit}>编 辑</Button>
          </h3>
        );
      ...
      status = (<h3 className="header-title"></h3>);
    }
    let subContent = {};
    subContent = (
      <div>
          ...
          <Card style={{ marginTop: 20, boxShadow: "0 2px 8px rgba(0, 0, 0, 0.15)" }} title="付款信息">
            <div className="table-header">
              <div className="table-header-buttons" style={{ float: "left" }} >
                {
                  (headerData.status === 1001 || headerData.status === 1003 || headerData.status === 1005) && <Button type="primary" onClick={this.addItem}>新建付款信息</Button>
                }
              </div>
              {
                amountText !== ''
                  ...
                  <div style={{ float: 'right' }}>
                    <Breadcrumb style={{ marginBottom: '10px' }}>
                      <Breadcrumb.Item>金额:<span style={{ color: 'Green' }}>{amountText}</span></Breadcrumb.Item>
                      <Breadcrumb.Item>本币金额:<span style={{ color: 'Green' }}> {"CNY"} {this.filterMoney(functionAmount)}</span></Breadcrumb.Item>
                    </Breadcrumb>
                  </div>
                  :
                  null
    ...
    return (
      <div className="pre-payment-common">
        ...
        <SlideFrame
          ...
          <NewPrePaymentDetail onClose={this.handleCloseSlide} params={{ id: this.state.id, paymentReqTypeId: this.state.paymentReqTypeId, companyId: this.state.companyId, flag: this.state.flag, remark: this.state.headerData.description, record, headerData: this.state.headerData }} />
        </SlideFrame>
      </div>
    )
  }
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedPrePaymentCommon);
