...
class NewPrePaymentType extends React.Component {
        ...
        this.setState(
          {
            ...
            transactionClassList: res.data.transactionClassIdList
              ? res.data.transactionClassIdList
              : [],
            employeeList: res.data.departmentOrUserGroupIdList
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (!this.state.allClass && !this.state.transactionClassList.length) {
          message.warning(
            this.$t({ id: 'pre.payment.transactionClassList.warn' } /*请选择至少一个现金事务分类*/)
          );
          return;
        }
        ...
        if (values.applyEmployee == 'BASIS_02' && !values.selectEmployeeList.values.length) {
          message.warning(
            this.$t({ id: 'pre.payment.selectDepartmentList.warn' } /*请选择至少一个部门*/)
          );
          return;
        } else if (values.applyEmployee == 'BASIS_03' && !values.selectEmployeeList.values.length) {
          message.warning(
            this.$t({ id: 'pre.payment.selectEmployeeList.warn' } /*请选择至少一个人员组*/)
          );
          return;
        }
        if (this.state.allType == 'BASIS_02' && !this.state.requisitionTypeIdList.length) {
          message.warning(
            this.$t(
              { id: 'pre.payment.requisitionTypeIdList.warn' } /*请选择至少一个可关联申请类型*/
            )
          );
          return;
        }
        ...
        let params = {
          ...
          requisitionTypeIdList:
            this.state.allType == 'BASIS_02' ? this.state.requisitionTypeIdList : [],
          ...
          departmentOrUserGroupIdList:
            values.applyEmployee != 'BASIS_01'
              ? values.selectEmployeeList.values.map(o => o.value)
              : [],
        };

        this.setState({ loading: true });
        // PrePaymentTypeService.updatePrePaymentType(this.state.nowType.id,params).then((res) => {
        httpFetch[this.state.nowType.id ? 'put' : 'post'](
          `${config.prePaymentUrl}/api/cash/pay/requisition/types`,
          params
        )
          .then(res => {
          ...
          .catch(e => {
            if (e.response) {
              message.error(
                this.$t({ id: 'pre.payment.save.fail' } /*保存失败*/) + `${e.response.data.message}`
              );
            }
            this.setState({ loading: false });
          });
  ...
  handleListOk = values => {
    ...
    this.setState({
      showSelectEmployeeGroup: false,
      showSelectDepartment: false,
      employeeList: values.checkedKeys,
      selectEmployeeText: text,
    });
  };
    ...
    return (
      <div>
        <Form onSubmit={this.handleSave}>
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'pre.payment.typeCode' } /*预付款单类型代码*/)}
          >
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'pre.payment.typeName' } /*预付款单类型名称*/)}
          >
          ...
          <div className="common-item-title">
            {this.$t({ id: 'pre.payment.paymentMethodCategoryName' } /*付款方式类型*/)}
          </div>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'pre.payment.paymentMethodCategoryName' } /*付款方式类型*/)}
          >
          ...
          <div className="common-item-title">
            {this.$t({ id: 'pre.payment.association.requisition.set' }) /* 关联申请设置 */}
          </div>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'pre.payment.whether.associate.requisition' }) /* 是否关联申请 */}
          >
            <RadioGroup value={needApply} onChange={this.onNeedApply}>
              <RadioButton value={true}>
                {this.$t({ id: 'pre.payment.requisition.associate' }) /* 关联 */}
              </RadioButton>
              <RadioButton value={false}>
                {this.$t({ id: 'pre.payment.requisition.unassociate' }) /* 不关联 */}
              </RadioButton>
            </RadioGroup>
          </FormItem>
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'pre.payment.association.basis' }) /* 关联依据 */}
          >
            <RadioGroup
              value={applicationFormBasis}
              disabled={!needApply}
              onChange={this.onApplicationFormBasisChange}
            >
              <Radio style={radioStyle} value="BASIS_01">
                预付款单头公司+头部门=申请单头公司+头部门
              </Radio>
              <Radio style={radioStyle} value="BASIS_02">
                预付款单头申请人=申请单头申请人
              </Radio>
            </RadioGroup>
          </FormItem>
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'pre.payment.requisition.type.choose' }) /* 可关联申请类型 */}
          >
            <div>
              <RadioGroup value={allType} disabled={!needApply} onChange={this.onAllTypeChange}>
                <Radio value="BASIS_01">
                  {this.$t({ id: 'pre.payment.all.types' }) /* 全部类型 */}
                </Radio>
                <Radio value="BASIS_02">
                  {this.$t({ id: 'pre.payment.partial.types' }) /* 部分类型 */}
                </Radio>
              </RadioGroup>

              <Select
                ref="SelectRequisitionType"
                onFocus={() => this.showSelectRequisitionType(true)}
                disabled={!needApply || allType == 'BASIS_01'}
                value={
                  allType == 'BASIS_02'
                    ? this.$t(
          ...
          <div className="common-item-title">
            {this.$t({ id: 'pre.payment.cash.transaction.class.set' }) /* 现金事务分类设置 */}
          </div>
          <FormItem
            {...formItemLayout}
            label={
              this.$t({ id: 'pre.payment.available.cash.transaction.class' }) /* 可用现金事务分类 */
            }
          >
            <div>
              ...
              <Select
                ref="selectCashTransaction"
                onFocus={this.showSelectCashTransaction}
                onChange={this.onAllClassListChange}
                disabled={allClass}
                value={
                  allClass
                    ? this.$t({ id: 'pre.payment.all.types' }) /* 全部类型 */
          ...
          <div className="common-item-title">
            {this.$t({ id: 'pre.payment.authority.set' }) /* 权限设置 */}
          </div>

          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'pre.payment.applicable.personnel' }) /* 适用人员 */}
          >
            {getFieldDecorator('selectEmployeeList', {
              initialValue: this.state.permissions,
            })(<PermissionsAllocation params={{ setOfBooksId: nowType.setOfBookId }} />)}
          </FormItem>
        ...
        <SelectCashTransaction
          visible={showSelectCashTransaction}
        ...
        <SelectRequisitionType
          visible={showSelectRequisitionType}
