...
class PrePaymentType extends React.Component {
    ...
    this.state = {
      ...
      columns: [
        {
          ...
          render: (value, record) => {
            return (
              <Popover content={record.setOfBookName}>
                <span>
                  {record.setOfBookCode}-{record.setOfBookName}
                </span>
              </Popover>
            );
          },
        ...
        {
          title: this.$t({ id: 'pre.payment.formName' } /*关联表单类型*/),
          dataIndex: 'formName',
          width: 180,
          align: 'center',
        },
          ...
          render: (text, record) => (
            <span>
              ...
              <a onClick={() => this.handleDistribute(record)}>
                {this.$t({ id: 'pre.payment.company.distribution' } /*公司分配*/)}
              </a>
            </span>
          ),
        }, //操作
      ...
      searchForm: [
        ...
        },
        {
          ...
          label: '付款方式类型',
          options: [
            { value: 'ONLINE_PAYMENT', label: '线上' },
            { value: 'OFFLINE_PAYMENT', label: '线下' },
            { value: 'EBANK_PAYMENT', label: '落地文件' },
          ],
          labelKey: 'label',
          valueKey: 'value',
        },
  ...
  handleDistribute = record => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/document-type-manage/prepayment-type/distribution-company/${
          record.setOfBookId
        }/${record.id}`,
      })
    );
  ...
  getList() {
    ...
    return PrePaymentTypeService.getPrePaymentType(
      this.state.page,
      this.state.pageSize,
      params
    ).then(response => {
      ...
      <div className="budget-organization">
        ...
        >
          <NewPrePaymentType
            ...
            params={{ prePaymentType: nowType, flag: showSlideFrame }}
          />
        </SlideFrame>
      </div>
    );
