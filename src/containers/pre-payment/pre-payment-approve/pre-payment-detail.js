...
class PrePaymentDetail extends React.Component {
  ...
  //获取预付款头信息
  getInfo = () => {
    approvePrePaymentService.getHeadById(this.props.match.params.id).then(res => {
      this.setState({
        headerData: res.data,
      })
    }).catch(() => {
  ...
  handleApprovePass = (remark) => {
    ...
    approvePrePaymentService.approvePass(model).then(res => {
  ...
  handleApproveReject = (remark) => {
    ...
    approvePrePaymentService.approveReject(model).then(res => {
      if (res.status === 200) {
