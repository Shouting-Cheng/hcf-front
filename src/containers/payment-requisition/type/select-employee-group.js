  ...
  getList = nextProps => {
    httpFetch.get(`${config.baseUrl}/api/user/groups/company/all`).then(res => {
      if (this.props.selectedData && this.props.selectedData.length) {
        ...
        let list = [];
        res.data.map(item => {
          if (this.props.selectedData.findIndex(x => x == item.id) >= 0) {
        ...
        <Row gutter={10}>
          <Col span={10}>
            <Card
              ...
              extra={<span>代选区</span>}
            >
              <Search placeholder="请输入" onChange={e => this.toSearch(e.target.value)} />
              {
          ...
          <Col span={10}>
            <Card
              ...
              extra={<span>使用列表</span>}
            >
              <Search placeholder="请输入" onChange={e => this.useSearch(e.target.value)} />
              {
                <div style={{ margin: '10px 15px' }}>
                  <CheckboxGroup
                    value={useSelectList}
