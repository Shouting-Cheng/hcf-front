...
export default {
  ...
  submitOrCheckBudget(expenseReport){
    return httpFetch.post(`${config.baseUrl}/api/v2/expense/reports/custom/form/submit`, expenseReport)
  },

  //提交报销单
  submitExpenseReport(expenseReport){
    return httpFetch.post(`${config.baseUrl}/api/expense/reports/custom/form/submit`, expenseReport)
  },

  //获取默认借款单
  getDefaultLoanRequest(applicationOID, applicantOID){
    let params = {
      applicationOID, applicantOID
    };
  ...
  getDefaultApportionment(expenseReportOID, expenseTypeId){
    return httpFetch.get(`${config.baseUrl}/api/v2/expense/default/apportionment?expenseReportOID=${expenseReportOID}&expenseTypeId=${expenseTypeId}`)
  },
  ...
  getAllExpenseByExpenseReport(params){
    return httpFetch.post(`${config.baseUrl}/api/v2/invoices/currency`, params).then(res => {
      return httpFetch.post(`${config.baseUrl}/api/v2/invoices/currency?page=0&size=${Number(res.headers['x-total-count']) || 0}`, params)
    })
  },
