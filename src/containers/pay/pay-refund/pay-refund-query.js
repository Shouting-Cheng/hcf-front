...
class PayRefund extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            loading2: false,
            searchForm1: [
                {type: 'input', colSpan: 6, id: 'billcode', label: `${this.$t({id: "pay.refund.billCode"}/*付款流水号*/)}`},
                {
                    type: 'input',
                    colSpan: 6,
                    id: 'documentNumber',
                ...
                },
                {
                    type: 'items', colSpan: 6, id: 'amountRange', items: [
                        ...
                        {type: 'input', id: 'amountTo', label: `${this.$t({id: "pay.refund.paymentAmountTo"}/*付款金额至*/)}`}
                    ]
                },
                {
            ...
            searchForm2: [
                ...
                {
                  type: 'items', colSpan: 6,id: 'partner', items: [
                    { type: 'value_list',colSpan:1, id: 'partnerCategory', label: messages('pay.workbench.type')/*类型*/, valueListCode: 2107, options: [], event: 'PARTNER' },
                    {
                      type: 'list', colSpan:5, id: 'partnerId', label: `${this.$t({id: "pay.refund.partnerCategoryName"}/*退款方*/)}`,  listType: 'select_returnee', single: true,
                       disabled: true, labelKey: "name", valueKey: "id",listExtraParams:{ empFlag: "1001", name: "", pageFlag: true }
                    },
                  ]
                },
                ...
                {type: 'select', colSpan: 6, id: 'backFlashStatus', label: `${this.$t({id: "pay.refund.status"}/*状态*/)}`, options: [
            ...
            columns1: [
                {title: `${this.$t({id: "pay.refund.billCode"}/*付款流水号*/)}`, dataIndex: 'billcode',align:'center'},
                {title: `${this.$t({id: "pay.refund.documentNumber"}/*单据编号*/)}`, dataIndex: 'documentNumber',align:'center'},
                {title: `${this.$t({id: "pay.refund.documentTypeName"}/*单据类型*/)}`, dataIndex: 'documentTypeName',align:'center'},
                {
                    title: `${this.$t({id: "pay.refund.employeeName"}/*申请人*/)}`, dataIndex: 'employeeName',align:'center',
                    render: (value, record) => {
                        return (
                                <div>
                                    {record.employeeCode}
                ...
                {title: `${this.$t({id: "pay.refund.currency"}/*币种*/)}`, dataIndex: 'currency',align:'center'},
                {
                    title: `${this.$t({id: "pay.refund.amount"}/*付款金额*/)}`,align:'center',
                    dataIndex: 'amount',
                    render: this.filterMoney
                },
                {
                    title: `${this.$t({id: "pay.refund.abledRefundAmount"}/*可退款金额*/)}`,align:'center',
                    dataIndex: 'abledRefundAmount',
                    render: this.filterMoney
                },
                {
                    title: `${this.$t({id: "pay.refund.partnerName"}/*收款方*/)}`, dataIndex: 'partnerName',align:'center',
                    render: (value, record) => {
                        return (
                                <div>
                ...
                },
                {
                    ...
                    render: (id, record) => (
                            <a onClick={() => this.doRefund(record)}> {this.$t({id: "pay.refund.doRefund"}/*发起退款*/)}</a>)
                }
            ],
            columns2: [
    ...
    getUnRefundList = () => {
        ...
        payRefundService.queryUnRefundList(params,unRefundPage,unRefundPageSize).then((res) => {
            if (res.status === 200) {
                this.setState({
  ...
  onMyRefundChangePageSize =(page, pageSize) =>{
    ...
    getMyRefundList = () => {
        ...
        payRefundService.queryMyRefundList(params,myRefundPage,myRefundPageSize).then((res) => {
            if (res.status === 200) {
                this.setState({
    ...
    handleTabsChange = (value) =>{
      ...
      this.setState({unRefundSearchParams:{"employeeId":this.props.user.id},myRefundSearchParams:{}},()=>{
        if (value === 'myRefund'){
          this.searchEventHandle("PARTNER", "");
          this.getMyRefundList();
        }else{
    ...
    renderContent = () => {
      const { tabValue, loading1, loading2, searchForm1, searchForm2, columns1, unRefundData,deRecord,
        myRefundData, unRefundPagination, myRefundPagination,openWindowFlag,columns2,frameTitle,frameFlag,myRefundRecord} = this.state;
    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.renderContent()}
                <SlideFrame title={this.$t({id: "pay.refund.refundDetail"}/*退款详情*/)}
                             show={openWindowFlag}
                             onClose={this.cancelWindow}
                             afterClose={this.restFormFunc}>
                  <NewPayRefund params={{record:deRecord,flag:openWindowFlag}}
                                onClose={this.cancelWindow}/>
                </SlideFrame>

                <SlideFrame title={frameTitle}
                            show={frameFlag}
                            onClose={this.cancelWindow1}
                            afterClose={this.restFormFunc1}>
                  <PayRefundDetail params={{record:myRefundRecord,flag:frameFlag}}
                                   onClose={this.cancelWindow1}/>
                </SlideFrame>
            </div>
        )
