    ...
    return (
      <div className="booker-endorse-btn request-btn">
        ...
        {formType === 2003 && info.status === 1003 && !!travelOrders.length && (
          <Button type="primary" onClick={this.showModal}>{this.$t('request.detail.btn.endorse')/*改 签*/}</Button>
        )}

        <Modal title={this.$t('request.detail.btn.endorse')/*改 签*/}
               width={600}
          ...
          <Form className="form-container">
            <Row>
              <Col span={12}>
                <FormItem {...formItemLayout} label={this.$t('request.detail.btn.endorse.reason')/*改签原因*/}>
                  {getFieldDecorator('isPersonalReason', {
                    initialValue: false
                  })(
                    <RadioGroup onChange={e => this.setState({isPersonalReason: e.target.value})}>
                      <Radio value={false}>{this.$t('request.detail.btn.endorse.reason.project')/*项目*/}</Radio>
                      <Radio value={true}>{this.$t('request.detail.btn.endorse.reason.person')/*个人*/}</Radio>
                    </RadioGroup>
                  )}
                </FormItem>
              </Col>
                ...
                <FormItem label='' colon={false}>
                  {getFieldDecorator('remark', {
                    ...
                    <TextArea rows={2} style={{resize:'none'}}
                              placeholder={this.$t('request.detail.btn.endorse.reason.input')/*请输入改签人员姓名、改签理由等*/}/>
                  )}
                </FormItem>
              </Col>
            </Row>
