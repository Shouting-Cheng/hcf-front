    ...
    return (
      <div className="new-repayment-frame">
        <Form onSubmit={this.handleSave}>
          ...
          {loanRefund && (
            <div>
              <FormItem {...formItemLayout} label={this.$t('request.detail.loan.all.name')/*姓名*/} style={{marginBottom:20}}>
                <div>{applicant.fullName}</div>
              </FormItem>
              <FormItem {...formItemLayout} label={this.$t('request.detail.department.name'/*部门*/)}>
                <div>{info.departmentName || '-'}</div>
              </FormItem>
            </div>
          )}
          <Row>
            <Col span={6} className="form-define-title required">{this.$t('request.detail.loan.payment.amount'/*还款金额*/)}：</Col>
              ...
              <FormItem>
                {getFieldDecorator('repaymentValue', {
                  rules: [{
                    ...
                    pattern: /[1-9]/,
                    message: this.$t('request.detail.loan.pay.amount.greater.than.0'/*还款金额要大于 0*/)
                  }],
                  initialValue: amount
                })(
          ...
          {loanRefund && (
            <FormItem {...formItemLayout} label={this.$t('request.detail.loan.payment.method'/*还款方式*/)}>
              {getFieldDecorator('paymentType', {
                ...
                <Select onChange={this.handlePaymentTypeChange}>
                  {!this.props.profile['finance.cash.repaymemt.disabled'] && (
                    <Option key='cash'>{this.$t('request.detail.loan.payment.cash'/*现金还款*/)}</Option>
                  )}
                  <Option key='card'>{this.$t('request.detail.loan.payment.transfer'/*转账还款*/)}</Option>
                </Select>
              )}
            </FormItem>
          )}
          {(!loanRefund || paymentType === 'card') && (
            <div>
              <FormItem {...formItemLayout} label={this.$t('request.detail.loan.account.name')/*开户名*/}>
              ...
              <FormItem {...formItemLayout} label={this.$t('request.detail.loan.account.number')/*开户账号*/}>
              ...
              <FormItem {...formItemLayout} label={this.$t('request.detail.loan.account.bank')/*开户银行*/} className="pay-bank-container">
          ...
          {!loanRefund && (
            <FormItem {...formItemLayout} label={`${this.$t('request.detail.loan.image')/*流水转账凭证*/}（${this.$t('common.max.image.length', {max: 3})}）`}>
          ...
          {loanRefund && (
            <FormItem {...formItemLayout} label={this.$t('common.remark')}>
              {getFieldDecorator('remark', {
                ...
                <TextArea placeholder={this.$t('common.max.characters.length', {max: 200})} rows={3} style={{resize: 'none'}}/>
              )}
            </FormItem>
          )}
          {(!loanRefund || paymentType === 'card') && (
            <div>
              <h4 className="title">
                ...
                <span> {this.$t('request.detail.loan.receiver.notice')/*请按以下收款方账号打款*/}</span>
              </h4>
              <FormItem {...formItemLayout} label={this.$t('request.detail.loan.account.name')/*开户名*/} className="accept-account-container">
                {getFieldDecorator('acceptAccountName', {
                  ...
                  <Select placeholder={this.$t('common.please.select')}
                          ...
                          notFoundContent={accountFetching ? <Spin size="small" /> : this.$t('request.detail.loan.no.result'/*无匹配结果*/)}>
              ...
              <FormItem {...formItemLayout} label={this.$t('request.detail.loan.account.bank')/*开户银行*/}>
          ...
          <div className="slide-footer">
            <Button type="primary" htmlType="submit" loading={loading}>
              {loanRefund ? this.$t('request.detail.loan.confirm'/*确认*/) : this.$t('request.detail.loan.notification.finance'/*通知财务已还款*/)}
            </Button>
