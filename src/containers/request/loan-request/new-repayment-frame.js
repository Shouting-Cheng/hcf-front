...
class NewPaymentFrame extends React.Component{
  ...
  handleSave = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if(!err) {
        ...
        values.paymentType === 'card' && (values.acceptAccountName = this.state.acceptAccountName || this.state.accountOptions[0].companyName);
  ...
  render() {
    ...
    const { loading, accountFetching, accountOptions, acceptAccount, acceptBankName, currency, amount, info, paymentType, loanRefund, myBankInfo, isEditBankInfo, repayAttchmentImages } = this.state;
    ...
    return (
      <div className="new-repayment-frame">
        <Form onSubmit={this.handleSave}>
          <h4 className="title">{messages('request.detail.loan.payer.info')/*还款方信息*/}</h4>
          {loanRefund && (
            <div>
              <FormItem {...formItemLayout} label={messages('request.detail.loan.all.name')/*姓名*/} style={{marginBottom:20}}>
                <div>{applicant.fullName}</div>
              </FormItem>
              <FormItem {...formItemLayout} label={messages('request.detail.department.name'/*部门*/)}>
                <div>{info.departmentName || '-'}</div>
              </FormItem>
            </div>
          )}
          <Row>
            <Col span={6} className="form-define-title required">{messages('request.detail.loan.payment.amount'/*还款金额*/)}：</Col>
              ...
              <FormItem>
                {getFieldDecorator('repaymentValue', {
                  rules: [{
                    ...
                    pattern: /[1-9]/,
                    message: messages('request.detail.loan.pay.amount.greater.than.0'/*还款金额要大于 0*/)
                  }],
                  initialValue: amount
                })(
                  <InputNumber min={0} max={amount} precision={2} className="input-number"
                               disabled={!loanRefund && this.props.profile['ca.opt.didi.refund.amount.disabled']}/>
                )}
              </FormItem>
            </Col>
          </Row>
          {loanRefund && (
            <FormItem {...formItemLayout} label={messages('request.detail.loan.payment.method'/*还款方式*/)}>
              {getFieldDecorator('paymentType', {
                ...
                <Select onChange={this.handlePaymentTypeChange}>
                  {!this.props.profile['finance.cash.repaymemt.disabled'] && (
                    <Option key='cash'>{messages('request.detail.loan.payment.cash'/*现金还款*/)}</Option>
                  )}
                  <Option key='card'>{messages('request.detail.loan.payment.transfer'/*转账还款*/)}</Option>
                </Select>
              )}
            </FormItem>
          )}
          {(!loanRefund || paymentType === 'card') && (
            <div>
              <FormItem {...formItemLayout} label={messages('request.detail.loan.account.name')/*开户名*/}>
              ...
              <FormItem {...formItemLayout} label={messages('request.detail.loan.account.number')/*开户账号*/}>
              ...
              <FormItem {...formItemLayout} label={messages('request.detail.loan.account.bank')/*开户银行*/} className="pay-bank-container">
          ...
          {!loanRefund && (
            <FormItem {...formItemLayout} label={`${messages('request.detail.loan.image')/*流水转账凭证*/}（${messages('common.max.image.length', {max: 3})}）`}>
          ...
          {(!loanRefund || paymentType === 'card') && (
            <div>
              <h4 className="title">
                {messages('request.detail.loan.receiver.info')/*收款方信息*/}
                <span> {messages('request.detail.loan.receiver.notice')/*请按以下收款方账号打款*/}</span>
              </h4>
              <FormItem {...formItemLayout} label={messages('request.detail.loan.account.name')/*开户名*/} className="accept-account-container">
                {getFieldDecorator('acceptAccountName', {
                  ...
                  <Select placeholder={messages('common.please.select')}
                          ...
                          notFoundContent={accountFetching ? <Spin size="small" /> : messages('request.detail.loan.no.result'/*无匹配结果*/)}>
              ...
              <FormItem {...formItemLayout} label={messages('request.detail.loan.account.bank')/*开户银行*/}>
          ...
          <div className="slide-footer">
            <Button type="primary" htmlType="submit" loading={loading}>
              {loanRefund ? messages('request.detail.loan.confirm'/*确认*/) : messages('request.detail.loan.notification.finance'/*通知财务已还款*/)}
            </Button>
