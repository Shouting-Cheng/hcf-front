...
class LoanRepayment extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          title: this.$t('request.detail.loan.date' /*日期*/),
          dataIndex: 'createDate',
          render: value => moment(value).format('YYYY-MM-DD'),
        },
        {
          title: this.$t('request.detail.loan.payment.code' /*还款单号*/),
          dataIndex: 'businessCode',
        },
        {
          title: this.$t('request.detail.loan.payment.method' /*还款方式*/),
          dataIndex: 'type',
          render: value =>
            value === '0'
        ...
        {
          title: this.$t('request.detail.loan.payment.amount' /*还款金额*/),
          dataIndex: 'actRepayAmount',
          render: (value, record) => this.filterMoney(value || record.repaymentValue),
        },
        {
          ...
          render: value =>
            value === '1001' ? (
              <Badge
                text={this.$t('request.detail.loan.in.the.payment') /*还款中*/}
                status="processing"
              />
            ) : value === '1002' ? (
              <Badge
                text={this.$t('request.detail.loan.has.been.payment') /*已还款*/}
                status="success"
              />
            ) : (
        ...
        {
          title: this.$t('request.detail.loan.card.no' /*凭证编号*/),
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      newParams: {
        ...
        amount:
          info.writeoffArtificialDTO &&
          (info.writeoffArtificialDTO.stayWriteoffAmount || info.totalAmount),
        info: info,
        hasInit: false,
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      ...
      newParams: {
        ...
        amount:
          nextProps.info.writeoffArtificialDTO &&
          (nextProps.info.writeoffArtificialDTO.stayWriteoffAmount || nextProps.info.totalAmount),
        info: nextProps.info,
        hasInit: false,
        loanRefund: this.props.loanRefund,
      },
    ...
    this.setState(
      {
        newParams: {
          ...
          amount:
            this.props.info.writeoffArtificialDTO &&
            (this.props.info.writeoffArtificialDTO.stayWriteoffAmount ||
              this.props.info.totalAmount),
    ...
    return (
      <div className="loan-repayment tab-container">
        <h3 className="sub-header-title">
          {this.$t('request.detail.repayment.history' /*还款记录*/)}
        </h3>
          ...
          <div className="table-header-buttons">
            {repaymentInfo.stayWriteoffAmount && buttonRoleSwitch ? (
              <Button
                ...
                onClick={() => this.showNewSlide(true)}
              >
        ...
        />
        <SlideFrame
          title={this.$t('request.detail.loan.create.payment') /*新建还款*/}
          show={showNewSlide}
        ...
        />
        <SlideFrame
          title={this.$t('request.detail.loan.payment.detail') /*还款详情*/}
