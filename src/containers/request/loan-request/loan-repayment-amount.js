        ...
        <span className="loan-repayment-amount">
          {repaymentInfo.hasWriteoffAmount !== info.originCurrencyTotalAmount && (
            <span>
              {repaymentInfo.hasWriteoffAmount ?
                `${this.$t('request.detail.loan.has.been.payment')/*已还款*/}：${repaymentInfo.currencyCode}  ${this.renderMoney(repaymentInfo.hasWriteoffAmount)}` : ''}
              {repaymentInfo.hasWriteoffAmount ? <span className="ant-divider"/> : ''}
              {repaymentInfo.lockedWriteoffAmount ?
                <span style={{color: '#00a854'}}>
                  {this.$t('request.detail.loan.in.the.payment')/*还款中*/}：{repaymentInfo.currencyCode}&nbsp; {this.renderMoney(repaymentInfo.lockedWriteoffAmount)}
                </span> : ''}
              ...
              {repaymentInfo.stayWriteoffAmount ? (
                <span>
                  <span style={{color: '#108ee9', marginRight: 10}}>
                    {this.$t('request.detail.loan.balance')/*待还款*/}：{repaymentInfo.currencyCode}&nbsp; {this.renderMoney(repaymentInfo.stayWriteoffAmount)}
                  </span>
                  {buttonRoleSwitch && <a onClick={this.toRepayment}>{this.$t('request.detail.loan.create.payment')/*新建还款*/} ></a>}
                </span>
              ) : ''}
            </span>
          )}
          {repaymentInfo.hasWriteoffAmount === info.originCurrencyTotalAmount && (
            <span className="repayment-all">
              <Icon type="check-circle-o" className="repayment-all-icon" />{this.$t('request.detail.loan.all.repayment.completed')/*已全额还款*/}
            </span>
          )}
        </span>
      )
