...
class ExpenseAllocateForm extends React.Component {
  ...
  scaleChange = (value, index) => {
    if (this.judgeIsSBCCase(value)) {
      message.error(
        this.$t('itinerary.form.component.allocation.input.tip') /*'请切换输入法半角'*/
      );
  ...
  setValueDefaultItem = () => {
    ...
    if (this.value.length > 0 && defaultItem.costCenterItems[0].name) {
      ...
      if (isSortAndReplace.isSort) {
        ...
        if (isSortAndReplace.canReplace) {
          //是否需要替换
          this.value.splice(0, 1, defaultItem);
        }
      } else if (isSortAndReplace.canReplace) {
        //是否需要替换
        this.value.splice(0, 1, defaultItem);
      }
  ...
  initColumn = value => {
        ...
        if (item.defaultApportion) {
          columns.push({
            ...
            render: (value, record, index) =>
              this.renderColumn(value, record, index, cost.type, count),
          });
        }
      });
    ...
    columns.push({
      ...
      columns.push({
        title: this.$t('itinerary.form.component.allocation.operation') /*操作*/,
        key: 'operation',
        dataIndex: 'operation',
        render: (value, record, index) =>
          index === 0 ? (
            ...
            <Popconfirm
              title={this.$t('itinerary.form.component.allocation.delete.tip') /*'确认删除?'*/}
              onConfirm={e => this.deleteAllocate(index)}
            >
              <a
                onClick={e => {
                  e.stopPropagation();
              ...
              >
                {this.$t('itinerary.type.slide.and.modal.delete.btn') /*删除*/}
              </a>
            </Popconfirm>
          ),
      });
  ...
  render() {
    ...
    return (
      <div>
        ...
        />
        {isShowAddBtn && (
          <a onClick={() => this.addNewAllocate()}>
            {this.$t('itinerary.form.component.allocation.new') /*+新增分摊*/}
          </a>
        )}
      </div>
    );
