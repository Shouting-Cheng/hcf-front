...
class TravelType extends React.Component {
  ...
  refreshItinerary = (display) => {
    travelService.getItinerary(this.props.setInfo.oid).then(res=> {
      ...
      if(res.data['SUBSIDIES'] && res.data['SUBSIDIES'].length>0){
        ...
        || (res.data['HOTEL'] && res.data['HOTEL'].length > 0) || (res.data['OTHER'] && res.data['OTHER'].length > 0)){
  ...
  currencyRateChange = (value) => {
    ...
    subsidyItem.currencyRate = value.target.value ? parseFloat(value.target.value).toFixed(4) : subsidyItem.currencyRate;
  ...
  render() {
    const {isChangeVersion, itinerary, subCtrlObj,subsidyItem, isCanChangeRate,
      amountProfile, startDate, isShowHotelSlide, isShowTrainSlide, isShowPlaneSlide,
      isShowSubsidySlide, isShowRemarkSlide, isShowOtherSlide, setInfo, isShowTip, tipContent} = this.state;
                ...
                <FormItem {...formItemLayoutL} label={this.$t('itinerary.subsidy.edit.modal.rate')/*汇率*/}>
                  {getFieldDecorator('currencyRate',{
                    ...
                    initialValue:subsidyItem.currencyRate
                      ...
                      <Input type="number" disabled={!(!isCanChangeRate && subsidyItem.currencyCode!='CNY')} style={{width:90}} min={0} onChange={this.currencyRateChange}/>
                  )}
                  {
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedTravelType);
