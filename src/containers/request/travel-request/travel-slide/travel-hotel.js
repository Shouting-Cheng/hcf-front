...
class TravelHotel extends React.Component{
  ...
  selectSupply = (index) => {
    ...
    if (this.state.selectCity.code) {
      travelService.isCityInVendor(travelUtil.getSearchType(this.supply.serviceName,2003), this.state.selectCity.code).then(res => {
  ...
  disabledDateStart = (current) =>{
    ...
    return travelUtil.disabledDate(current, moment(this.baseStartDate), this.baseEndDate, floatLeaveDay, floatStartDay, this.dateArray) ;
  }

  disabledDateEnd = (current) =>{
    let floatLeaveDay =  this.state.formCtrl.leaveDate.floatDays + 1;
    let floatStartDay =  -this.state.formCtrl.fromDate.floatDays;
    return travelUtil.disabledDate(current, this.baseStartDate, this.baseEndDate, floatLeaveDay, floatStartDay, this.dateArray) ;
  }

  render(){
    const {formCtrl, maxRoom, minPrice, standardEnable, supplies, defaultDate, supplyId, currentIndex, isLoading, citySearchResult, nights} = this.state;
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedTravelHotel);
