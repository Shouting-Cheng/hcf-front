...
class TravelRemark extends React.Component {
  ...
  updateDateFromForm = () => {
    let start = "";
    let end = "";
    start = moment(travelUtil.getFormHeadValue(this.props.params.defaultValue,'start_date')).format("YYYY-MM-DD+hh:mm:ss");
    end = moment(travelUtil.getFormHeadValue(this.props.params.defaultValue,'end_date')).format("YYYY-MM-DD+hh:mm:ss") ;
  ...
  initRemark = (tempRemark) =>{
    ...
    let currentNum = 0;
    travelService.getDates(this.props.params.oid, this.baseStartDate, this.baseEndDate).then(res=> {
  ...
  getRemarks = (date,index) => {
    ...
    travelService.getRemarksByDate(this.props.params.oid, formatDate).then(res=> {
      this.setState({
        remarks: res.data,
      })
    }).catch(err=>{
        ...
        <Spin spinning={isLoading}>
          <Row>
                ...
                <RadioGroup defaultValue={0}>
                  {
                    dates.map((date, index)=> {
                      ...
                      types = types ? types : [];
                      return (
                        <RadioButton key={date.remarkDate}
                                     checked={index==currentIndex}
                                     onClick={()=>this.getRemarks(date,index)}
                                     className={(types.length>0 || date.remark)?'date-item-on':'date-item'}
                                     value={index}>{moment(date.remarkDate).format(this.props.language.code === 'zh_cn' ?  'MM月DD日' : 'MMMM DD') + `   D${index + 1}`}</RadioButton>
                      )
                    })
                  }
                </RadioGroup>
            ...
            <Col span={17}>
              ...
              <Form>
                <FormItem>
                  {getFieldDecorator("text-remark",travelUtil.createFormOption('',{type:'str',value:selectDate.remark},true))
                  (<TextArea className="remark-text-value" onChange={this.changeRemark} maxLength={201}></TextArea>)}
                </FormItem>
              </Form>
            </Col>
          </Row>
  ...
  createImgRow = (selectDate) => {
    ...
    let sle_dateStr = `${moment(selectDate.remarkDate).format(this.props.language.code === 'zh_cn' ? 'MM月DD日' : 'MMMM DD')}  ${travelUtil.getWeed(new Date(selectDate.remarkDate).getDay())}`;
    return (
      <Row className="row-title">
        <Col span={7}>{sle_dateStr}</Col>
        {
          sle_types.map(item=> {
            return (
              <Col key={item} span={2} className="col-img"><img src={travelUtil.setItineraryTypeImg(item)}/></Col>)
          })}
      </Row>
    )
  }
  ...
  createRecord = (remarks) => {
    ...
    for (let rem in remarks) {
      remarks[rem].map((item,index)=> {
        if (item.remark) {
          let div =  <div key={item.remark+index}>
              <Row className="row-city">
                <Col span={10} className="city-col">
                  <img src={travelUtil.setItineraryTypeImg(rem)}/>
                  <span className="city-margin">{item.fromCity?item.fromCity:item.cityName}</span>-
    ...
    return ( <div>{ divs.map( item =>{ return item }) }</div> )
  }
}
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedTravelRemark);
