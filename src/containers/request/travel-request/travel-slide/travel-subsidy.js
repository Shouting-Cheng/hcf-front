...
class TravelSubsidy extends React.Component {
  ...
  searchCityChange = (keyWord) => {
    ...
    travelService.searchCitys('standard', keyWord,country,this.props.language.code === 'zh_CN' ? 'zh_CN' : 'en_US').then(res => {
      this.setState({
        cityResult: res.data,
      })
    })
    ...
    this.props.form.validateFieldsAndScroll((err, values)=> {
      if (!err) {
        if (!this.currentCity.vendorAlias) {
          ...
          if (this.state.isEditing) {//如果是更新必填id
  ...
  onBlurCity = () => {
    if (this.currentCity.isChange) {//如果改动了却没有select 恢复原状态
      this.setHavedCity();
    }
  };
                  ...
                  subsidyTypeData.length > 0 ? <Row>
                    {
                      subsidyTypeData.map((item, index)=> {
                        return (
                          <Col span={5} className="subsidy-type" onClick={()=>this.selectSubsidyType(index)}
                               key={item.iconName+index}>
                            <img className="type-icon" src={item.iconURL}/>
                            <span className="type-name">{item.name}</span>
                            <Checkbox checked={item.isChecked} className="type-checked"></Checkbox>
                          </Col>
                        )
                      })
                    }
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedTravelSubsidy);
