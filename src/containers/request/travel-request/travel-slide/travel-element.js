...
class TravelElement extends React.Component {
  ...
  componentWillMount() {
    ...
    let obj = tempMap['application.property.control.fields']
      ? JSON.parse(tempMap['application.property.control.fields']) : travelUtil.getSetDataByTravelType('flight');
  ...
  render() {
    const {
      isLoading, travelElementCustomField
    } = this.state;
      ...
      <div className="travel-plane">
        <Spin spinning={isLoading}>
          <Form>
            ...
            {!isLoading && travelElementCustomField.customFormFields && travelElementCustomField.customFormFields.map(item => {
              let selectorItem = {
                url: `${config.baseUrl}/api/my/cost/center/items/${item.dataSource && JSON.parse(item.dataSource || '{}').costCenterOID}?page=0&size=1000`,
                label: 'name',
              ...
              let chooseItem = {
                ...
                url: `${config.baseUrl}/api/my/cost/center/items/${item.dataSource && JSON.parse(item.dataSource || '{}').costCenterOID}`,
                searchForm: [
                  {
                    type: 'input',
                    id: 'name',
              ...
              return (
                <FormItem {...formItemLayout} label={item.fieldName} key={item.fieldOID}>
                  {getFieldDecorator(item.fieldOID, {
                    ...
                    item.messageKey !== 'input' && item.messageKey !== '' ? (item.fieldCode === 'cust_customer' ?
                        <Chooser selectorItem={chooseItem}
                                 valueKey={'code'}
                                 labelKey={'name'}
                                 single={true}
                        /> :
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedTravelElement);
