...
class TravelPlane extends React.Component {
  ...
  selectSupply = index => {
    ...
    if (this.state.selectFromCity.code) {
      travelService
        .isCityInVendor(
          travelUtil.getSearchType(this.supply.serviceName, 2001),
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (travelElement && !err) {
        ...
        if (!err) {
          ...
          let takeOffEndTime = moment(this.props.form.getFieldValue('takeOffEndTime')).format(
            `HH:mm`
          );
          let arrivalBeginTime = moment(this.props.form.getFieldValue('arrivalBeginTime')).format(
            `HH:mm`
          );
          let arrivalEndTime = moment(this.props.form.getFieldValue('arrivalEndTime')).format(
            `HH:mm`
          );
          ...
          values.discount =
            values.discount === this.$t('itinerary.public.select.all') /*所有*/
              ? ''
              : values.discount;
          ...
          if (this.state.formCtrl.fromCities.required && !this.state.selectFromCity.vendorAlias) {
            ...
            this.state.formCtrl.toCities.required &&
            !this.state.selectToCity.vendorAlias
          ) {
            message.error(
          ...
          if (!this.state.editing) {
            travelService
              .travelPlaneSubmit(this.state.params.oid, [values])
              .then(res => {
        ...
        <Spin spinning={isLoading}>
          <Form>
            ...
            {!travelElement && (
              ...
              >
                <TabPane
                  tab={this.$t('itinerary.plane.slide.mainland') /*国内机票*/}
                  key={'1001'}
                />
            ...
            {!travelElement && (
              <FormItem
                {...formItemLayout}
                label={this.$t('itinerary.plane.slide.VoyageType') /*行程类型*/}
              >
                {getFieldDecorator(
                  'itineraryType',
                  this.CFO(this.$t('itinerary.plane.slide.VoyageType'), {
                    type: 'number',
                ...
                <Col span={12}>
                  ...
                  >
                    {getFieldDecorator(
                      'fromCity',
                      this.CFO(
                        this.$t('itinerary.public.slide.departureCity'),
                        { type: 'str', value: editData.fromCity },
                        !travelElement ? !formCtrl.fromCities.required : false
                      )
                    )(
                      <Select
                        mode="combobox"
                        ...
                        {cityFromSearchResult.map((city, index) => {
                          return (
                            <Option data-city={city} key={city.code} title={city.vendorAlias}>
                              {city.vendorAlias}&nbsp;&nbsp;<span style={{ color: '#ccc' }}>
                                ({city.country})
                              </span>
                            </Option>
                          );
                  ...
                  >
                    {getFieldDecorator(
                      'toCity',
                      this.CFO(
                        this.$t('itinerary.public.slide.arrivalCity'),
                        { type: 'str', value: editData.toCity },
                        !travelElement ? !formCtrl.toCities.required : false
                      )
                    )(
                      <Select
                        mode="combobox"
                ...
                <FormItem>
                  <Row>
                    <Col span={10}>
                      <FormItem
                        {...formItemLayoutTime}
                        label={this.$t('itinerary.plane.slide.leaveOn') /*出发起飞时间*/}
                      >
                        {getFieldDecorator(
                          'takeOffBeginTime',
                          this.CFO(
                ...
                <FormItem>
                  <Row>
                    <Col span={10}>
                      <FormItem
                        {...formItemLayoutTime}
                        label={this.$t('itinerary.plane.slide.returnOn') /*返回起飞时间*/}
                      >
                        {getFieldDecorator(
                          'arrivalBeginTime',
                          this.CFO(
                    ...
                    <Col span={10} className="plane-time">
                      <FormItem {...formItemLayoutTime}>
                        ...
                        )(<TimePicker disabled={!isDouble} format={'HH:mm'} />)}
                      </FormItem>
                    </Col>
                  </Row>
                </FormItem>
                ...
                <FormItem>
                  <Row>
                    <Col span={12}>
                      <FormItem
                        {...formItemLayoutCity}
                        label={this.$t('itinerary.public.slide.price') /*机票价格*/}
                      >
                        {getFieldDecorator(
                          'ticketPrice',
                          this.CFO(
                    ...
                    <Col span={12}>
                      {!standardEnable && (
                        <FormItem
                          {...formItemLayoutCity}
                          label={this.$t('itinerary.plane.slide.discount') /*折扣*/}
                        >
                          {getFieldDecorator(
                            'discount',
                            this.CFO(
                              this.$t('itinerary.plane.slide.discount'),
                              {
                                type: 'number',
                ...
                <FormItem className="plane-seat-margin">
                  <Row>
                    <Col span={12}>
                      <FormItem
                        {...formItemLayoutCity}
                        label={this.$t('itinerary.public.slide.class') /*舱等*/}
                      >
                        {getFieldDecorator(
                          'seatClass',
                          this.CFO(
                            this.$t('itinerary.public.slide.class'),
                            {
                              type: 'str',
                              value: editData.seatClass
                                ? editData.seatClass
                                : this.props.language.code === 'zh_cn'
                                  ? '经济舱'
                                  : 'Economy class',
                          ...
                          <Select>
                            {travelUtil
                              .getSeatClass(
                                this.props.language.code === 'zh_cn' ? 'plane' : 'plane_en'
                              )
                              .map((seat, index) => {
                                return (
            ...
            {!travelElement && (
              ...
              >
                {getFieldDecorator(
                  ...
                  <TextArea
                    maxLength={201}
                    placeholder={this.$t('itinerary.public.slide.remarkPlaceholder') /*请输入*/}
                    autosize={textAreaLayout}
                  />
                )}
              </FormItem>
            )}
