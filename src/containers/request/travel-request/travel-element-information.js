    ...
    return (
      <div className="travel-information-element tab-container">
        ...
        <Spin spinning={loading}>
          <Collapse>
            {
              info.travelApplication.travelItinerarys.length > 0 && info.travelApplication.travelItinerarys.map((item, mainIndex) => {
                ...
                return (<Panel header={header}
                               showArrow={!option}
                               disabled={option} key={item.itineraryOID ? item.itineraryOID : mainIndex + '-main'}>
                      ...
                      return (
                        <Row type="flex" className="element-item" align="middle" key={index}>
                          <Col span={23}>
                            {element.map(e => {
                              return (
                                <div className="value-item" key={e.fieldOID ? e.fieldOID : index + '-fieldOID'}>{e.fieldName}:{e.showValue}</div>)
                            })}
                          </Col>
                        </Row>)
                    })}
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedTravelInformationElement)
