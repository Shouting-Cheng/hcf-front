...
export default {
  ...
  getSearchType(serviceName,category){
    switch (serviceName) {
      case 'supplyCtripService'://携程1
        switch (category){
          case 2001: return 'ctrip_air';
          case 2002: return 'ctrip_train';
          case 2003: return 'ctrip_hotel';
      ...
      case 'supplyMeiYaService'://美亚
        switch (category){
          case 2001: return 'meiya_air';
          case 2002: return 'ctrip_train';
          case 2003: return 'meiya_hotel';
          default:return 'standard'
        }
  ...
  createFormOption (...args){
    let option = {
      rules: [{
        required: !args[2],
        message: !args[2] ? messages('itinerary.form.rule.isEmpty.tip',{formKey:args[0]}) : messages('itinerary.form.rule.textArea.tip',{maxNum:args[1].maxNum ? args[1].maxNum : 200}),
      }]
    }
   ...
   */
  getSeatClass(type){
    let train = ['硬座', '软座', '特等座', '一等座', '二等座', '高级软包上', '高级软包下', '商务座',
      '硬卧', '软卧', '动卧', '高级软卧', '一等软座', '二等软座', '一人软包', '高级动卧'];

    let train_en = ['Hard Seat', 'Soft Seat', 'Premier Class', 'First Class', 'Second Class', 'Deluxe Sleeper(Up)', 'Deluxe Sleeper(Down)', 'Business Class',
      'Hard Sleeper', 'Soft Sleeper', 'MU Sleeper', 'Deluxe Sleeper', '1st Soft Class', '2nd Soft Class', 'Soft Coach', 'MU Deluxe Sleeper'];
  ...
  getTravelRecord(title, datas, click, dele, update, stopAndStart, start, isChangeVersion){
    if (datas.length > 0 && (title === messages('itinerary.other.collapse.title')/*其他*/ || title === messages('itinerary.train.collapse.title')/*火车*/ || title === messages('itinerary.plane.collapse.title')/*机票*/)){
      let titleImgUrl = this.setItineraryTypeImg(title);
      let isOther = (title === messages('itinerary.other.collapse.title'))/*其他*/;
      return (<Panel header={<span><img style={{marginTop:-4}} src={titleImgUrl}/>&nbsp;{title}</span>} className="type-plane">
        {
          datas.map((res, index)=> {
            ...
            return (
              <div key={title + index} className="type-line-box" disabled={!res.loading} onClick={()=>click(res,title)}>
                <Row>
                  <Col span={8}><Tag color='#108ee9'>{messages('itinerary.record.public.days.tag',{days:day})/*第{day}天*/}</Tag>{date}</Col>
                  <Col span={10} className="type-supply">{res.approvalNum ? messages('request.detail.travel.itinerary.number')/*行程单号*/ + `:${res.approvalNum}` : ''}</Col>
                ...
                {
                  res.isExtend && res.disabled && <img className="stop-img"
                                                       src={messages('itinerary.record.public.disabled.img') === 'EN' ? disabledImgEN : disabledImgZH}/>
                }
                {
                  isChangeVersion && <Tag color={res.isExtend ? 'orange' : 'green'}>{res.isExtend ? messages('request.detail.travel.previous')/*原*/ : messages('request.detail.travel.latest')/*新*/}</Tag>
                }
                <Row>
                  <Col span={16} className="type-city-city">
                    <Row>
                      {isOther && <Col span={4}>{res.trafficTypeName}</Col>}
                      <Col span={6} style={{textAlign:'right',marginRight:6}}><span>{res.fromCity}</span></Col>
                      <Col span={4}><span><img style={{width:'100%'}} src={traffic_line}/></span></Col>
                      <Col span={9}style={{marginLeft:6}}><span>{res.toCity}</span></Col>
                    </Row>
                  </Col>
                  <Col span={8} className={res.isExtend ? 'type-stop' : 'type-city-city type-img-delete'}>
                    {
                      res.isExtend ?  (<Button size="small" loading={res.loading} type="primary" onClick={(e) => stopAndStart(e, res, title, index)}>{res.disabled ? messages('request.detail.btn.restart')/*重新启用*/ : messages('request.detail.btn.expire')/*停用*/}</Button>)
                      : ( isGoBack ? (!res.loading ? <Popconfirm title={messages('itinerary.record.public.delete.tip')/*"该行程为往返行程"*/}
                          onConfirm={()=>dele(res, title, index)}
                          onCancel={()=>update(res, title, index)}
                          okText={messages('itinerary.record.public.delete.round')/*"删除往返行程"*/}
                          cancelText={messages('itinerary.record.public.delete.single')/*"仅删除该行程"*/}>
                          <Icon type="close-circle" onClick={(e)=>{e.stopPropagation()}}/>
                          </Popconfirm> : <Button shape="circle" loading />) :
                ...
                return (
                  <div key={title + index} className="type-line-box" onClick={()=>click(res,title)}>
                    <Row className="remark-title">
                      <Col span={6}>
                        <Tag color='#108ee9'>{messages('itinerary.record.public.days.tag',{days:day})/*第{day}天*/}</Tag>{date}
                      </Col>
                      {types.map(item => {
                      ...
                      <Col span={8} className="remark-clear">
                        {
                          res.itineraryShowDetails.length > 0 ?
                            (res.loading ? <Button shape="circle" loading/>: <Popconfirm title={messages('itinerary.record.clear.remark.tip')/*"确定清空该行程备注吗"*/}
                                                                                         onConfirm={()=>clear(res,index)}
                                                                                         okText={messages('itinerary.type.slide.and.modal.ok.btn')/*确定*/}
                                                                                         cancleText={messages('itinerary.type.slide.and.modal.cancel.btn')/*取消*/}>
                              <span onClick={(e)=>{e.stopPropagation();}}>{messages('itinerary.remark.slide.clear')/*清空*/}</span>
                            </Popconfirm>)
                            :
                            (res.loading ? <Button shape="circle" loading/> : <Popconfirm title={messages('itinerary.record.public.delete.tip')/*"你确定删除这行内容吗?"*/}
                                                                                          onConfirm={()=>dele(res,title,index)}
                                                                                          okText={messages('itinerary.type.slide.and.modal.ok.btn')/*确定*/}
                                                                                          cancelText={messages('itinerary.type.slide.and.modal.cancel.btn')/*取消*/}>
                              <Icon type="close-circle" onClick={(e)=>{e.stopPropagation()}}/>
          ...
          datas.map((res, index)=> {
            ...
            return (
              <div key={title + index} className="type-line-box" onClick={()=>click(res,title)}>
                <Row>
                  <Col span={6}>
                    <Tag color="#108ee9">{messages('itinerary.record.public.days.tag',{days:startF})/*第{startF}天*/}</Tag>~
                    <Tag color="#108ee9">{messages('itinerary.record.public.days.tag',{days:endL})/*第{endL}天*/}</Tag>
                  </Col>
                  <Col span={11}>{`${dateF}~${dateL}`}</Col>
                  <Col span={3}>{`${messages('itinerary.hotel.slide.night',{nights:night})}`}</Col>
                ...
                {
                  res.isExtend && res.disabled && <img className="stop-img" src={messages('itinerary.record.public.disabled.img') === 'EN' ? disabledImgEN : disabledImgZH}/>
                }
                {
                  isChangeVersion && <Tag color={res.isExtend ? 'orange' : 'green'}>{res.isExtend ? messages('request.detail.travel.previous')/*原*/  : messages('request.detail.travel.latest')/*新*/ }</Tag>
                }
                <Row>
                  ...
                  <Col span={12}className={res.isExtend ? 'type-stop' : 'type-city-city type-img-delete'}>
                    {
                      res.isExtend ? (<Button size="small" type="primary" loading={res.loading} onClick={(e) => stopAndStart(e, res, title, index)}>{res.disabled ? messages('request.detail.btn.restart')/*重新启用*/ : messages('request.detail.btn.expire')/*停用*/}</Button>)
                        :
                        (res.loading ? <Button shape="circle" loading/>
                          :<Popconfirm title={messages('itinerary.record.public.delete.tip')/*"你确定删除这行内容吗?"*/}
                                       onConfirm={()=>dele(res,title,index)}
  ...
  disabledDateEnd(current, startDate){
    let boo = false;
    if(startDate && current < moment(startDate, 'YYYYMMDD hh:mm:ss'))
      boo = true;
    return current && boo;
  },
   ...
   * @param isCancelHiddenItem  取消隐藏回调
     ...
     * @returns {XML}
     */
  getSubsidyRecord(title, datas, click, dele, displayDetail, toBatchChange, amountChange, toSaveChange, toEditSubsidyItem, searchByDate, isHiddenItem, isCancelHiddenItem, subCtrlObj){
    if (datas.length > 0) {
      let titleImgUrl = this.setItineraryTypeImg(title);
      return (
        <Panel header={<span><img style={{marginTop:-4}} src={titleImgUrl}/>&nbsp;{title}</span>} className="type-plane">
          {
            datas.map((res, index)=> {
              ...
              let remark = res.remark ? `${messages('itinerary.public.slide.remark')/*备注*/}：${res.remark}` : '';
                ...
                <div key={title+index} className="subsidy-record-css">
                  <div className="subsidy-line-box" onClick={()=>click(res,title)}>
                    <Row className="set-font">
                      {`${startDate}~${endDate}`}<span style={{marginLeft:6,fontWeight: 'bold'}}>{res.cityName}</span>
                    </Row>
                    <Row>
                      <Col span={20}>
                      ...
                      <Col span={12}>
                        {
                          ...
                          <DatePicker disabledDate={(c)=>this.disabledDate(c,moment(res.startDate),moment(res.endDate),1)}
                                      onChange={(e)=>searchByDate(e,index)}></DatePicker>
                        }
                      </Col>
                      <Col span={12} className="save-cancel">
                         {
                          res.isBatch ? <div>
                            <Button onClick={()=>toBatchChange(index)}>{messages('itinerary.type.slide.and.modal.cancel.btn')/*取消*/}</Button>
                            <Button loading={subCtrlObj.isSaveChange} onClick={()=>toSaveChange(index)}>{messages('itinerary.type.slide.and.modal.save.btn')/*保存*/}</Button>
                          </div> : <Button onClick={()=>toBatchChange(index)}>{messages('itinerary.record.subsidy.batch.change.btn')/*批量更改*/}</Button>
                        }
                      </Col>
                    </Row>
                  </div>
                  {
                    requestDTOs.map((req, reqIndex)=> {
                      ...
                      for (let key in req.currencyCodeAmountMap) {
                        currentCodeMapChiled = currentCodeMapChiled + key + req.currencyCodeAmountMap[key] + " + ";
                        codeTypeCount++;
                        if(codeTypeCount === 1){
                          firstCodeType = key;
                        }
                      ...
                      for (let key in req.expenseTypeAmountMap) {
                        expenseTypeAmountMapChiled = expenseTypeAmountMapChiled + key + req.expenseTypeAmountMap[key].toFixed(2) + " + ";
                      }
                      ...
                      return (
                        <div className="second-box" key={title+index+reqIndex}>
                          {req.status == 1002 && <div className="to-hiden">
                            <span>{messages('itinerary.record.subsidy.hiddenAll.btn')/*!隐藏该员工该行程下的所有差补，提交后将自动删除隐藏的差补*/}</span>
                          ...
                          <Row className="row-child">
                            ...
                            <Col span={4} className="display-icon">
                              {
                                !res.isBatch && <img onClick={()=>isHiddenItem(req,false)}
                          ...
                          </Row>
                          {
                            req.isDisplay && detailMaps.map((det, detIndex)=> {/* TODO 需要一个控制显示的量 */
                              return det.travelSubsidiesRequestItemDetailDTOs.map((detail, indexOf)=> {
                                ...
                                let rateMount =  messages('request.detail.travel.enterprise.rate')/*企业汇率*/+ ": ";
                                let isRepeat = false;
                                ...
                                rateMount = rateMount + detail.baseCurrencyRate.toFixed(4) + calculateChar + tempRate.toFixed(1) + `% , ${messages('request.detail.travel.original.amount')/*原金额*/}: ` + detail.baseAmount.toFixed(2);
                                return (
                                  <div className="subsidy-detail" key={detail.travelSubsidiesDetailsOID}>
                                    {detail.status == 1002 && <div className="to-hiden">
                                      <span>{messages('itinerary.record.subsidy.hiddenSingle.btn')/*!隐藏的差补在提交后将自动删除*/}</span>
                                    ...
                                    <Spin spinning={subCtrlObj.isSaveChange}>
                                      <Row>
                                        ...
                                        <Col span={10} className="child-map-detail">
                                          {
                                            isRepeat && <Tag color="#f50">{messages('request.detail.travel.repeat')/*重复*/}</Tag>
                                          }
                                          {detail.expenseTypeName}</Col>
                                        <Col span={2} className="text-center">{detail.currencyRate.toFixed(4)}</Col>
                                        <Col span={2} className="text-center">{detail.currencyCode}</Col>
                                        {
                                          res.isBatch ? <Col span={5}>
                                            ...
                                            <Col span={2} className="text-center">{detail.amount.toFixed(2)}</Col>
                                        }
                                        {
                                        ...
                                        {
                                          !res.isBatch && <Col span={3} className="display-icon-eye-edit text-center">
                                            <img className="img-eye" onClick={()=>isHiddenItem(detail,true,displayItem)}
                                                 src={require('images/travel/subsidy-show.png')}/>
                                          </Col>
                                        }
                                      </Row>
                                      ...
                                      </Row>
                                      {
                                        detail.comment && <Row style={{marginTop:6}}>
                                          <Col span={3}></Col>
                                          <Col span={19} style={{wordBreak: 'break-all'}}>{messages('itinerary.public.slide.remark')/*备注*/+"：" + detail.comment}</Col>
                                        </Row>
                                      }
                                    </Spin>
                                  </div>
  ...
  setDefaultFormUtil(status, custValues, newValue,formOID, base, executeCall, isReplace){
    ...
    baseService.changeLoginInfo(newValue.value).then(()=>{
          ...
          res.data.map(item => {
            custValues.map(field =>{
              if(field.fieldOID === item.fieldOID && field.value !== item.value){
                ...
                if(setUpdateItem[key])
                  currentUpdate.push(setUpdateItem);
              }
            });
          });
          count++;
    ...
    if(isReplace){
      travelService.getPrincipals(formOID).then(res =>{
        ...
        custValues.map(field =>{
          if(field.messageKey === 'select_participant'){//参与人员
            key = status === 'edit' ? field.formValueOID : field.fieldOID;
            JSON.parse(field.value || '[]').map(v =>{//遍历参与人员
              if(v.userOID !== base.value){//参与人员的oid是否不等于原申请人oid
                newJoinPeople.push({userOID: v.userOID, fullName: v.fullName});
              }
              if(v.userOID === newValue.value){//是否新的申请人也在参与人之内
                isNewValueHave = true;//新的申请人也在参与人之内
              }
            })
          }
        });
        if(!isNewValueHave){//新的申请人不在参与人之内，新的申请人要替换掉原来参与人中与原申请人相同的人
          newJoinPeople.push({userOID: tempReuqestPeople.userOID, fullName: tempReuqestPeople.fullName})
        }
    ...
    for(let i=0;i<value.length-1;i++){
      for(let j=i+1;j<value.length;j++){
        if(value[i].hashStr === value[j].hashStr){
          ...
          result.reason = messages('itinerary.form.component.allocation.rowSame.tip',{x:i+1,y:j+1})/*`第${i+1}行与第${j+1}行分摊元素相同`*/;
          return result;
        }
      }
    }
  ...
  allocateColumnIsEmpty(value){
    ...
    for(let i = 1;i < value.length;i++){
        ...
        if(have){// 添加了
          if(columnName){//存在必填却未添加的列
            columnName = columnName.substr(0,columnName.length-1);
            returnItem.reason = messages('itinerary.form.component.allocation.column.tip',{y:columnName})/*`${columnName}列必填`*/;
            return returnItem;
          }
        }else{
          returnItem.reason = messages('itinerary.form.component.allocation.rowInfo.tip',{x:i+1})/*`第${i+1}行分摊信息不完整`*/;
          return returnItem;
        }
        if(value[i].scale === 0){
          if(i !== 0){
            returnItem.reason = messages('itinerary.form.component.allocation.row.tip',{x:i+1})/*`第${i+1}行分摊比例应大于0`*/;
            return returnItem;
          }else{
            returnItem.reason = messages('itinerary.form.component.allocation.noEnough.tip')/*`已无分摊比例`*/;
            return returnItem;
          }
        }
    };
  ...
  allocateColumnIsSort(value,defaultItem){
    ...
    value[0].costCenterItems.map((item,of)=>{
      if(item.fieldName === defaultItem.costCenterItems[of].fieldName){//顺序是否与初始化后的列顺序相同
  ...
  getSetDataByTravelType(type){
    ...
    switch (type){
      case 'train':return data.train;
      case 'flight':return data.flight;
