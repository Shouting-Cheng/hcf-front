...
class TravelInformation extends React.Component {
    ...
    requestService[
      ...
      if (this.state.subsidyVersion === 2) {
        ...
        res.data.SUBSIDIES = JSON.parse(
          (info.travelApplication && info.travelApplication.travelSubsidies) || '[]'
        );
      }
      let itinerary = {};
  ...
  getSubsidyVersion = () => {
    ...
    this.setState(
      {
        subsidyVersion:
          this.props.info.travelApplication && this.props.info.travelApplication.travelSubsidies
            ? 2
            : 3,
      },
      () => {
      ...
      case 'FLIGHT':
        header = (
          <Row className="itinerary-header">
            <Col span={5}>
              <img src={planeImg} />
              {this.$t('request.detail.travel.flight') /*机票*/}
            </Col>
            {info.travelApplication &&
              info.travelApplication.uniformBooking &&
              info.travelApplication.bookingClerkName && (
                <Col span={10}>
                  {this.$t('request.detail.travel.booker.ticket.by', {
                    name: info.travelApplication.bookingClerkName,
                  }) /*由 {name} 统一订机票*/}
                </Col>
              )}
          </Row>
        ...
        if (item) {
          ...
          let time = '';
          if (
            customFormPropertyMap['ca.travel.applypolicy.enable'] === 'false' ||
            !customFormPropertyMap['ca.travel.applypolicy.enable']
          ) {
            controlFields.discount.show &&
              details.push(
                item.discount
                  ? `${item.discount}${this.$t('request.detail.travel.discount') /*折*/}`
                  : this.$t('request.detail.travel.all.discount' /*所有折扣*/)
              );
            controlFields.seatClass.show &&
              details.push(
                item.seatClass
                  ? item.seatClass
                  : this.$t('request.detail.travel.all.class' /*所有舱位*/)
              );
          }
          controlFields.ticketPrice.show &&
            details.push(
              item.ticketPrice
            ...
            ${
              item.takeOffBeginTime
                ? `${item.takeOffBeginTime} ～ ${item.takeOffEndTime ||
                    this.$t('request.detail.travel.no.restriction' /*无限制*/)}`
          ...
          itemContent = (
            <div className="flight">
              ...
              <div>
                {(info.version > 0 || isPreVersion) && (
                  <Tag color={item.isExtend ? '#f5832b' : '#4cc648'}>
                    {item.isExtend
                      ? this.$t('request.detail.travel.previous' /*原*/)
                      : this.$t('request.detail.travel.latest' /*新*/)}
                  </Tag>
                )}
                <Tag color="#1890ff">
                  {this.$t('request.detail.travel.day.number', {
                    day: this.getDifferDays(sDate, item.startDate),
                  }) /*第 {day} 天*/}
                </Tag>
                <span className="date">
                  {new Date(item.startDate).format('yyyy.MM.dd')}{' '}
                  {this.getWeed(new Date(item.startDate).getDay())}
                </span>
                {item.supplierName && (
                  <div className="supplier">
                    <span>{item.supplierName}</span> <img src={item.supplierIconUrl} />
                  </div>
                )}
              </div>
              <Row className="location">
                ...
                {item.approvalNum && (
                  <Col span={10}>
                    {this.$t('request.detail.travel.itinerary.number') /*行程单号*/}：{
                      item.approvalNum
                    }
                  </Col>
                )}
              </Row>
              {(item.supplierServiceName === 'vendorCtripService' ||
                item.supplierServiceName === 'supplyCtripService') && ( //只有在携程机票下才有可能显示
                <Row>
                  <Col span={10}>
              ...
              {item.remark && (
                <div className="remark">
                  {this.$t('common.remark')}：{item.remark}
                </div>
              )}
            </div>
          );
      ...
      case 'TRAIN':
        header = (
          <Row className="itinerary-header">
            <Col span={5}>
              <img src={trainImg} />
              {this.$t('request.detail.travel.train') /*火车*/}
            </Col>
            {info.travelApplication &&
              info.travelApplication.trainUniformBooking &&
              info.travelApplication.trainBookingClerkName && (
                <Col span={10}>
                  {this.$t('request.detail.travel.booker.train.by', {
                    name: info.travelApplication.trainBookingClerkName,
                  }) /*由 {name} 统一订火车票*/}
                </Col>
              )}
          </Row>
        ...
        if (item) {
          ...
          controlFields.seatClass.show &&
            details.push(
              item.seatClass ? item.seatClass : this.$t('request.detail.travel.no.limit' /*不限制*/)
            );
          controlFields.ticketPrice.show &&
            details.push(
              item.ticketPrice
      ...
      case 'SUBSIDIES':
        header = (
          <Row className="itinerary-header">
            <Col span={5}>
              <img src={subsidyImg} />
              {this.$t('request.detail.travel.allowance') /*差补*/}
            </Col>
          </Row>
        );
            ...
            <div className="subsidy">
              ...
              <div>
                <Tag color="#1890ff">
                  ...
                  {' ～ '}
                  {this.$t('request.detail.travel.day.number', {
                    day: this.getDifferDays(sDate, item.endDate),
                  }) /*第 {day} 天*/}
                </Tag>
                <span className="date">
                  {new Date(item.startDate).format('yyyy.MM.dd')}{' '}
                  ...
                  {this.getWeed(new Date(item.endDate).getDay())}
                </span>
              </div>
              <Row className="location">
                <Col span={10}>
                  {this.$t('request.detail.travel.destination') /*目的地*/}：<span className="from">
                    {item.cityName || '-'}
                  </span>
                </Col>
                <Col span={10}>
                  {this.$t('request.detail.total.amount') /*总金额*/}：
                  <span className="amount">
                    {this.props.company.baseCurrency} {this.renderMoney(item.totalAmount || 0)}
                  </span>
                </Col>
              </Row>
                  ...
                  return (
                    <div key={expense.expenseTypeOID} className="expense-item">
                      <div>
                        ...
                        {expense.usernames.length}
                        {this.$t('request.detail.travel.people') /*人*/},
                        {this.$t('request.detail.travel.total.day', {
                          day: this.getDifferDays(item.startDate, item.endDate),
                        }) /*共{day}天*/}
                      </div>
            ...
            <div className="subsidy">
              ...
              <div className="location-and-amount">
                ...
                <span className="other-currency">
                  {Object.keys(item.currencyCodeAmountMap || {}).length > 1 &&
                    Object.keys(item.currencyCodeAmountMap).map(currency => {
                  ...
                  <div className="user-subsidy-items" key={itemIndex}>
                    <Row className="title-show">
                      <Col span={5} className="avatar-info">
                        <Row>
                          <Col span={5}>
                            <img src={subsidy.avatar || emptyAvatarImg} className="avatar-img" />
                          </Col>
                          <Col span={19} style={{ wordWrap: 'break-word' }}>
                            {subsidy.fullName}
                          </Col>
                        </Row>
                      </Col>
                      <Col span={17} className="user-total-info">
                        {subCurrency.length && (
                          <Tooltip placement="top" title={subCurrency.join(' + ')}>
                            {subCurrency.join(' + ')}
                          </Tooltip>
                        )}
                        <div>
                          {subsidy.baseCurrencyCode} {this.renderMoney(subsidy.baseCurrencyAmount)}
                        </div>
                        <div className="expense-type-amount">
                          {Object.keys(subsidy.expenseTypeAmountMap || {}).map(key => {
                            expenseTypeAmount.push(
                              `${key}: ${this.renderMoney(subsidy.expenseTypeAmountMap[key])}`
                            );
                          })}
                          <Tooltip placement="top" title={expenseTypeAmount.join('，')}>
                            {expenseTypeAmount.join('，')}
                          </Tooltip>
                        </div>
                      </Col>
                    ...
                    {subsidy.showItem &&
                      subsidy.travelSubsidiesItemMap.map(subsidiesItem => {
                        return subsidiesItem.travelSubsidiesRequestItemDetailDTOs.map(
                          subsidyItem => {
                            subsidyItemIndex++;
                            return (
                              <div
                              ...
                              >
                                <Row>
                                  <Col span={3}>
                                    {moment(subsidiesItem.subsidiesDate).format('YYYY-MM-DD')}
                                  </Col>
                                  <Col span={13}>
                                    <Tooltip
                                      placement="topLeft"
                                      title={subsidyItem.expenseTypeName}
                                    >
                                      <span className="expense-type-name">
                                    ...
                                    <div>
                                      {this.$t(
                                        'request.detail.travel.enterprise.rate'
                                      ) /*企业汇率*/}：{Number(
                                      ...
                                      {subsidyItem.currencyRate - subsidyItem.baseCurrencyRate >= 0
                                        ...
                                        : ' - '}
                                      {(
                                        subsidyItem.currencyRate - subsidyItem.baseCurrencyRate
                                      ).toFixed(4) * 100}% ，{this.$t(
                                        'request.detail.travel.original.amount'
                                      ) /*原金额*/}：{this.renderMoney(subsidyItem.baseAmount)}
                                    </div>
                                  </Col>
      ...
      case 'REMARK':
        header = (
          <Row className="itinerary-header">
            <Col span={5}>
              <img src={remarkImg} />
              {this.$t('request.detail.travel.itinerary.remark') /*行程备注*/}
            </Col>
          </Row>
        );
        if (item) {
            ...
            (itemContent = (
              <div className="travel-remark">
                ...
                <div>
                  <Tag color="#1890ff">
                    {this.$t('request.detail.travel.day.number', {
                      day: this.getDifferDays(sDate, item.remarkDate),
                    }) /*第 {day} 天*/}
                  </Tag>
                  <span className="date">
                    {new Date(item.remarkDate).format('yyyy.MM.dd')}{' '}
                    {this.getWeed(new Date(item.remarkDate).getDay())}
                  </span>
                </div>
                <div className="remark">
                  <div>{item.remark}</div>
                  {Object.keys(item.itineraryDetails || {}).map((key, index) => {
                    ...
                    if (item.itineraryDetails[key][0].remark) {
                      return (
                        <Row key={index}>
                          <Col span={2} className="itinerary-type">
                            {remark_title}
                          </Col>
                          <Col span={3}>
                            <span>
                              {item.itineraryDetails[key][0].fromCity ||
                                item.itineraryDetails[key][0].cityName}
                            </span>
      ...
      case 'HOTEL':
        header = (
          <Row className="itinerary-header">
            <Col span={5}>
              <img src={hotelImg} />
              {this.$t('request.detail.travel.hotel') /*酒店*/}
            </Col>
            {info.travelApplication &&
              info.travelApplication.hotelUniformBooking &&
              info.travelApplication.hotelBookingClerkName && (
                <Col span={10}>
                  {this.$t('request.detail.travel.booker.hotel.by', {
                    name: info.travelApplication.hotelBookingClerkName,
                  }) /*由 {name} 统一订酒店*/}
                </Col>
              )}
          </Row>
          ...
          (itemContent = (
            <div className="hotel">
              ...
              <div>
                ...
                <Tag color="#1890ff">
                  {this.$t('request.detail.travel.day.number', {
                    day: this.getDifferDays(sDate, item.fromDate),
                  }) /*第 {day} 天*/}
                  {' ～ '}
                  {this.$t('request.detail.travel.day.number', {
                    day: this.getDifferDays(sDate, item.leaveDate),
                  }) /*第 {day} 天*/}
                </Tag>
                <span className="date">
                  {new Date(item.fromDate).format('yyyy.MM.dd')}{' '}
                  {this.getWeed(new Date(item.fromDate).getDay())} ～{' '}
                  {new Date(item.leaveDate).format('yyyy.MM.dd')}{' '}
                  {this.getWeed(new Date(item.leaveDate).getDay())}
                </span>
                <span className="day-and-night">
                  {this.$t('request.detail.travel.day', {
                    day: this.getDifferDays(item.fromDate, item.leaveDate),
                  }) /*{day}天*/}
                  {this.$t('request.detail.travel.night', {
                    night: this.getDifferDays(item.fromDate, item.leaveDate) - 1,
                  }) /*{night}晚*/}
                </span>
                {item.supplierName && (
                  <div className="supplier">
              ...
              <Row className="location">
                <Col span={10}>
                  {this.$t('request.detail.travel.destination') /*目的地*/}：{item.cityName ? (
                    <span className="from">{item.cityName}</span>
                  ) : (
                    '-'
                  )}
                </Col>
              ...
              <div>
                {item.roomNumber}
                {this.$t('request.detail.travel.room') /*间*/}
                <span className="ant-divider" />
                {this.props.company.baseCurrency}{' '}
                {item.minPrice
                  ? this.renderMoney(item.minPrice)
                  : this.$t('request.detail.travel.no.restriction' /*无限制*/)}
                {item.maxPrice
                  ? ` -- ${this.renderMoney(item.maxPrice)}`
                  : item.minPrice
                    ? ` -- ${this.$t('request.detail.travel.no.restriction' /*无限制*/)}`
                    : ''}
                {` / `}
      ...
      case 'OTHER':
        header = (
          <Row className="itinerary-header">
            <Col span={5}>
              <img src={otherImg} />
              {this.$t('request.detail.travel.other.traffic') /*其他交通*/}
            </Col>
          </Row>
        );
        item &&
                    ...
                    if (item) {
                      return (
                        <div className="container" key={index}>
                          {this.renderItineraryItem(key, item)}
                        </div>
                      );
                    }
                  })}
