...
class SelectDateForm extends React.Component{
  ...
  componentDidMount(){
    ...
    let fieldConstraint = JSON.parse(this.props.field.fieldConstraint ? this.props.field.fieldConstraint : '{}');
  ...
  setStopDate = (dateMoment) => {
    ...
    let closeDate = moment(travelUtil.getAfterDate(this.state.closeDays,dateMoment.utc().format('YYYY-MM-DD')));
    closeDate.local();
    this.setState({
      closeDate:closeDate,
    });
  ...
  setIsShowCloseDate = () => {
    ...
    if(applicationData && !this.initCloseDate){
      ...
      if(applicationData.custFormValues){//有该值编辑
        isEdit = true;
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedSelectDateForm);
