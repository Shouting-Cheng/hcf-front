...
class TravelElementType extends React.Component {
  ...
  componentWillMount() {
    ...
    let isShowTip = false;
    let tipContent = "";
    let isSetTipes = this.props.setInfo['travelInfo']['customFormPropertyMap']['travel.subsidies.dimension'] ?
      JSON.parse(this.props.setInfo['travelInfo']['customFormPropertyMap']['travel.subsidies.dimension']) : false;
    if (isSetTipes) {
  ...
  getTravelElementsList = (formOID) => {
    travelService.getTravelElementsList(formOID)
      .then(res => {
  ...
  tirmTravelElements = (arr, params) => {
    arr.map((itinerarys, mainIndex) => {
      if (moment(itinerarys.itineraryDate).format('YYYY.MM.DD') === moment(params.startDate).format('YYYY.MM.DD')) {
      ...
      case 'other':
        if (params.oldDate) {
          info.travelItinerarys.map((mainitem, mainindex) => {
            if (moment(mainitem.itineraryDate).format('YYYY.MM.DD') === moment(params.oldDate).format('YYYY.MM.DD')) {
              mainitem.travelItineraryTraffics.map((item, index) => {
                if (item.fromCityCode === params.fromCityCode && item.toCityCode ===
                  params.toCityCode && item.trafficType === params.trafficType) {
        ...
        this.setState({isShowPlaneSlide: false, isShowOtherSlide: false, isShowTrainSlide: false, setInfo: info});
        break;
      case 'element':
        params.dateRange.map(data => {
          info.travelItinerarys.map((itinerarys, mainIndex) => {
            if (data === moment(info.travelItinerarys[mainIndex].itineraryDate).format('YYYY-MM-DD')) {
  ...
  render() {
    const {
      itinerary, startDate, isShowTrainSlide, isShowPlaneSlide, isShowOtherSlide, setInfo,
      travelElement, isShowElementSlide, travelElementsList, elementTitle
    } = this.state;
    ...
    return (
      <div className="travel-type-select" style={{width: '90%', paddingBottom: 6}}>
          ...
          <Col className="travel-type" span={17}>
            ...
            { travelElementsList && travelElementsList.length > 0 && travelElementsList.map(item => {
              return (<Button className="travel-type-btn" type="dashed" icon="plus"
                              ...
                              key={item.formOID}>{item.formName}</Button>)
            })}
          </Col>
        </Row>
        <Row>
          ...
          <Col span={17}>
            <Collapse bordered={false} className="type-collapse">
              {
                setInfo.travelItinerarys.length > 0 && setInfo.travelItinerarys.map((item, mainIndex) => {
                  ...
                  return (<Panel header={header}
                    ...
                    {showTraffics && item.travelItineraryTraffics.map((element, index) => {
                      let imgUrl = '';
                    ...
                    {showElement && (
                      <div>{item.travelElements.map((element, index) => {
                        element.startDate = item.itineraryDate;
                        return (
                            ...
                            <Col span={23}>
                              {element.map(e => {
                                return (<div style={{padding:'5px 0'}}
                                  key={e.fieldOID ? e.fieldOID : index + '-fieldOID'}>{e.fieldName}:{e.showValue}</div>)
                              })}
                            </Col>
                            <Col span={1}>
                              <Popconfirm title={messages('itinerary.record.public.delete.tip')/*"你确定删除这行内容吗?"*/}
        ...
        {
          travelElement && isShowElementSlide
          && <SlideFrame title={elementTitle}
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedTravelElementType);
