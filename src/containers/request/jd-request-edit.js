      ...
      if(!err) {
        ...
        this.setState({
          ...
          requestService[type === 'save' ? 'saveJDRequest' : 'submitJDRequest'](this.state.info).then(() => {
            ...
            message.success(type === 'save' ? this.$t('common.save.success', {name: ''}) : this.$t('common.operate.success'));
            this.handleCancel()
          }).catch(() => {
            this.setState({ saveLoading: false, submitLoading: false })
          })
    ...
    return (
      <div className="jd-request-edit">
        <Spin spinning={loading}>
          {isOutTime && <Alert message={this.$t('request.detail.jd.error.message')/*该京东订单已超时，如需购买商品，请重新提交订单*/}
                               type="error" showIcon />}
          <Form style={{width:'35%'}}>
          ...
          <Affix offsetBottom={0} className="bottom-bar">
            {!isOutTime && <Button type="primary" loading={submitLoading} onClick={() => this.handleSave('submit')}>{this.$t('common.submit')}</Button>}
            {!isOutTime && <Button loading={saveLoading} onClick={() => this.handleSave('save')}>{this.$t('common.save')}</Button>}
            <Button onClick={this.handleCancel}>{this.$t('common.back')}</Button>
            <Button className="delete-btn" loading={deleteLoading} onClick={this.handleDelete}>{this.$t('common.delete')}</Button>
          </Affix>
        </Spin>
      </div>
    )
