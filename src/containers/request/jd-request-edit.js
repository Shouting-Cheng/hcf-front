...
class JDRequestEdit extends React.Component {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if(!err) {
        ...
        this.setState({
          ...
          requestService[type === 'save' ? 'saveJDRequest' : 'submitJDRequest'](this.state.info).then(() => {
            this.setState({ saveLoading: false, submitLoading: false });
            message.success(type === 'save' ? messages('common.save.success', {name: ''}) : messages('common.operate.success'));
            this.handleCancel()
          }).catch(() => {
            this.setState({ saveLoading: false, submitLoading: false })
          })
  ...
  handleDelete = () => {
    ...
    requestService.deleteRequest(this.props.params.applicationOID).then(() => {
  ...
  render() {
    ...
    const { loading, saveLoading, submitLoading, deleteLoading, isOutTime, info, productInfo, columns, data } = this.state;
    let approveValue = [];
    info.custFormValues && info.custFormValues.map(item => {
      if (item.messageKey === 'select_approver') {
        item.showValue && item.showValue.split(',').map((name, index) => {
    ...
    return (
      <div className="jd-request-edit">
        <Spin spinning={loading}>
          {isOutTime && <Alert message={messages('request.detail.jd.error.message')/*该京东订单已超时，如需购买商品，请重新提交订单*/}
                               type="error" showIcon />}
          <Form style={{width:'35%'}}>
            <FormItem  label={messages('request.detail.jd.reason')/*事由*/}>
            ...
            <FormItem  label={messages('request.detail.jd.remark')/*备注*/}>
              {getFieldDecorator('remark', {
                ...
                <TextArea autosize={{minRows: 2}}
                          disabled={isOutTime}
                          style={{resize:'none'}}
                          placeholder={messages('common.max.characters.length', {max: 200})}/>
              )}
            </FormItem>
            <FormItem  label={messages('request.detail.jd.approve.person')/*审批人*/}>
          ...
          <div className="amount-info">
            <Row>
              <Col span={3} className="amount">CNY {this.renderMoney(productInfo.totalAmount || 0)}</Col>
              <Col span={2} className="amount-title">{messages('request.detail.jd.product.amount')/*商品总额*/}：</Col>
            </Row>
            <Row>
              <Col span={3} className="amount">CNY {this.renderMoney(productInfo.freight || 0)}</Col>
              <Col span={2} className="amount-title">{messages('request.detail.jd.freight')/*运费*/}：</Col>
            </Row>
            <Row>
              <Col span={3} className="amount total">CNY {this.renderMoney((productInfo.totalAmount || 0) + (productInfo.freight || 0))}</Col>
              <Col span={2} className="amount-title total">{messages('request.detail.jd.total.amount')/*总计金额*/}：</Col>
            </Row>
          </div>
          <Affix offsetBottom={0} className="bottom-bar">
            {!isOutTime && <Button type="primary" loading={submitLoading} onClick={() => this.handleSave('submit')}>{messages('common.submit')}</Button>}
            {!isOutTime && <Button loading={saveLoading} onClick={() => this.handleSave('save')}>{messages('common.save')}</Button>}
            <Button onClick={this.handleCancel}>{messages('common.back')}</Button>
            <Button className="delete-btn" loading={deleteLoading} onClick={this.handleDelete}>{messages('common.delete')}</Button>
          </Affix>
        </Spin>
      </div>
    )
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedJDRequestEdit)
