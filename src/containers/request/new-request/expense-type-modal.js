...
class ExpenseTypeModal extends React.Component{
  ...
  getCurrencyOptions = () => {
    ...
    (!this.state.currencyOptions.length || this.state.applicationOID !== this.props.user.userOID) && (
      this.service.getCurrencyList(this.props.formDetail.applicantOID || this.props.user.userOID).then(res => {
        let currencyOptions = [];
        //过滤掉禁用的企业币种
        res.data.map(item => {
        ...
        <Modal title={this.$t('request.edit.add.expense.and.amount')/*添加费用类型及金额*/}
          ...
          <div className="expense-type-container">
            <ExpenseTypeSelector onSelect={this.handleSelectExpenseType}
                                 source="formV2"
                                 param={{formOID: this.props.formOID, createManually: true, setOfBooksId: this.props.formDetail.setOfBooksId}}/>
          </div>
        </Modal>
      </div>
    )
