  ...
  render() {
    ...
    return (
      <div className="booker-ticket-info tab-container">
        <h3 className="sub-header-title">{this.$t('request.detail.ticket.info'/*机票信息*/)}
          <span> {moment(this.props.info.lastModifiedDate).format('YYYY-MM-DD HH:mm')} {this.$t('request.detail.booker.update')/*更新*/}</span>
        </h3>
        {travelOrders.map((item, index) => {
          return (
            <Card key={index} className={item.status === 1002 ? 'ticket-disabled' : ''}>
              ...
              <Row>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.departure.time'/*起飞时间*/), moment(item.beginDate).format('HH:mm'))}</Col>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.arrive.time'/*到达时间*/), moment(item.arriveDate).format('HH:mm'))}</Col>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.flight'/*航班号*/), item.travelNo)}</Col>
              </Row>
              <Row>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.departure.airport'/*出发机场*/), item.fromAirport)}</Col>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.arrive.airport'/*到达机场*/), item.toAirport)}</Col>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.seatClass'/*舱位*/), item.bunk)}</Col>
              </Row>
              <Row>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.ticket.price'/*机票价格*/), item.price.toFixed(2))}</Col>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.tax.price'/*税费*/), item.taxationPrice.toFixed(2))}</Col>
                <Col span={8}>{this.renderInfo(this.$t('request.detail.booker.total.price'/*价格合计*/), item.finalPrice.toFixed(2))}</Col>
              </Row>
              {(item.confirmStatus === 1002 || item.confirmStatus === 1003) && (
                <Row className="confirm-info-row">
                  ...
                  <span>#{item.name}#</span>
                  <span>{item.confirmStatus === 1002 ? this.$t('request.detail.booker.confirm.info'/*确认机票信息*/) :
                    `${this.$t('request.detail.booker.feedback.unsuitable')/*反馈机票信息不合适*/}: ${item.confirmRemark}`}</span>
                </Row>
              )}
            </Card>
          )
        ...
        <Modal title={this.$t('request.detail.booker.ticket.feedback')/*机票信息反馈*/}
            ...
            <FormItem label={this.$t('request.detail.booker.info.is.suitable'/*信息是否合适*/)}>
              {getFieldDecorator('flag', {
                ...
                <RadioGroup onChange={this.onRadioChange}>
                  ...
                  <Radio value={false}>{this.$t('request.detail.booker.unsuitable')/*不合适*/}</Radio>
                </RadioGroup>
              )}
            </FormItem>
            {!infoIsRight && (
              <FormItem label={this.$t('request.detail.booker.unsuitable.reason')/*信息不合适原因*/}>
