...
class NewRequest extends React.Component {
  ...
  checkedChange = (field, value, allValue) => {
    ...
    if (field.messageKey === 'applicant' && this.state.formType !== 2001) {
      ...
      if (this.state.total > 0) this.clearSubsidy();
      //更新副本中的默认值
  ...
  getDays = (day1, day2) => {
    ...
    travelItinerarys.map((item, index) => {
      ...
      this.state.travelItinerarys.map((oldItem, oldIndex) => {
        if (
          moment(item.itineraryDate).format('YYYY-MM-DD') ===
          moment(oldItem.itineraryDate).format('YYYY-MM-DD')
        ) {
  ...
  checkedIsSetSubsidyRule = res => {
    ...
    let subsidyDimension = maps['travel.subsidies.dimension']
        ...
        this.state.subsidyRulesFieldOID.map(oid => {
          if (item.fieldOID === oid) {
            ...
            if (
              isEditing &&
              baseStartData &&
              baseStartData.value !== item.value &&
              this.canGo &&
              this.state.total > 0
            ) {
  ...
  componentWillReceiveProps() {
    ...
    if (this.state.formType === 2001) {
      ...
      cust.map((item, index) => {
        ...
        if (item.messageKey === 'end_date') {
        ...
        } else if (item.messageKey === 'select_participant') {
          //参与人员
          let baseStartData = this.state.copyDefaultValues[index];
          if (isEditing && baseStartData && baseStartData.value !== item.value && this.canGo) {
            if (this.isReplace) {
        ...
        } else if (item.messageKey === 'applicant') {
          //申请人 改动
          let baseStartData = this.state.copyDefaultValues[index];
          if (baseStartData && baseStartData.value !== item.value && this.canGo) {
            ...
            if (isInParticipant) {
              ...
              this.baseModalShow(
                this.$t(
                  'request.edit.modal.application.change.info'
                ) /*'新的申请人将替换原参与人员中的默认数据'*/,
                tipMessage,
                baseStartData,
                currentKey,
  ...
  baseModalShow = (mes, sage, data, currentKey, index, newField, isSubsidyRule) => {
    Modal.confirm({
      ...
      onOk: () => {
        if (data.messageKey === 'applicant') {
          //更改的是申请人    参与人要替换掉；成本中心，公司，部门都改默认值
          // 单独处理
          this.isReplace = true;
          //参数中使用{value：data.value},而不是直接使用 data，是因为data是数组copyDefaultValue的一个项，当该项
          //被更新后，传入的值也就改变了，即会出现传入时是a,某一时刻就变成copyDefaultValue更新后的值b了
  ...
  processDisableTime = (applicationData, type) => {
    //新建时的处理
    if (applicationData.customFormProperties.enabled === 1 && type === 'create') {
      //是否和停用启用有关
  ...
  processEnableTime = field => {
    ...
    if (
      ...
      JSON.parse(field.fieldConstraint).enableTime
    ) {
      enableTime = true;
    }
    return enableTime;
  ...
  processDefaultTime = field => {
    ...
    if (
      field.fieldConstraint &&
      JSON.parse(field.fieldConstraint) &&
      JSON.parse(field.fieldConstraint).defaultTime
    ) {
      defaultTime = moment(
        moment(JSON.parse(field.fieldConstraint).defaultTime).format('HH:mm'),
        'HH:mm'
      );
    }
    return defaultTime;
  ...
  toSaveTravelRequest = (params, type) => {
    ...
    if (isSetSelectParticipant) {
      this.setState({ loading: true });
      travelService
        .travelValidate(params, this.props.language, this.customFormFieldsOrigin)
        .then(res => {
          //参与人权限校验
            ...
            if (this.state.info.sourceApplicationOID) {
              ...
              Modal.warn({
                ...
                content: (
                  <div>
                    <p>
                      {this.$t('itinerary.form.submit.power.noChange.tip', {
                        names: showName,
                      }) /*{showName}不符合数据权限，请检查或联系系统管理员*/}
                    </p>
                  </div>
                ),
              ...
              Modal.confirm({
                ...
                content: (
                  <div>
                    <p>
                      {this.$t('itinerary.form.submit.power.tip', {
                        names: showName,
                      }) /*{showName}不在可选人员范围内。是否删除以上人员？*/}
                    </p>
                    {this.state.total > 0 && (
                      <p style={{ fontSize: 12 }}>
                        {this.$t(
                          'itinerary.form.submit.power.clear.tip'
                        ) /**更改参与人将清空差补，您要重新添加差补**/}
                      </p>
                    )}
                  </div>
            ...
            if (error.validationErrors[0].externalPropertyName) {
              switch (error.validationErrors[0].externalPropertyName) {
                ...
                case '2012': //成本中心为空
                  ...
                  break;
                default:
                  message.error(this.$t('finance.view.search.errorAdmin'));
              }
            }
          } else {
            message.error(`${this.$t('common.save.filed')}，${error.message}`);
  ...
  cancelDelPartis = (params, type, status) => {
    ...
    if (type === 'save') {
      ...
      if (status === 'create') {
        ...
        //设置行程管控
        params.travelApplication.manageType =
          params['customFormPropertyMap']['application.property.manage.type'];
      }
      ...
      params.applicant = null;
      params.travelApplication.totalBudget = this.state.totalBudget;
      if (this.state.subsidyCtrl.isBudgetCheck) {
        //是否需要预算校验
    ...
    if (boo) {
      ...
      if (this.state.isHotel) {
        ...
        if (man && man.length === maxHotel.maleRoomNumber) {
          ...
          message.error(
            this.$t('itinerary.form.submit.noBooking.hotelMale.tip', {
              peo: maxHotel.maleRoomNumber,
            })
          );
          return;
        }
        if (women && women.length === maxHotel.femaleRoomNumber) {
          ...
          message.error(
            this.$t('itinerary.form.submit.noBooking.hotelFemale.tip', {
              peo: maxHotel.femaleRoomNumber,
            })
          );
          return;
        }
  ...
  executeTravelSubmit = () => {
    ...
    travelService
      .submitTravelRequest(this.travelParams)
      .then(res => {
  ...
  formItemChange(value) {
    ...
    custForm.map(item => {
      ...
      if (
        (item.messageKey === 'select_department' || item.messageKey === 'select_cost_center') &&
        value &&
        Object.prototype.toString.call(value) === '[object Object]' &&
        item[id] in value &&
        this.state.formType !== 2001
      ) {
  ...
  render() {
        ...
        <Form className="form-container">
          {info.sourceApplicationOID && (
            <FormItem
              ...
              label={
                <span>
                  <Icon type="exclamation-circle-o" />&nbsp;{this.$t(
                    'itinerary.form.versionNum'
                  ) /*版本号*/}
                </span>
              }
            >
          ...
          <div style={{ margin: '0 0 20px 30%' }}>
            {customField.instructionsTag(formInfo.customFormPropertyMap)}
          </div>
          <ExpectStopDate
          ...
          {signEnable && (
            <FormItem
              {...formItemLayout}
              label={this.$t('customField.special.signer')}
              key="addSign"
            >
              {getFieldDecorator('addSign', {
                ...
                <Chooser
                  ...
                  showArrow={
                    ...
                    formInfo.customFormPropertyMap.countersignType === '2'
                  }
                  newline
                />
              )}
          ...
          defaultValues.length > 0 && (
            <TravelType
              updateTotalBudget={(total, clear, isHaveRoute, isRepeatSubsidy) =>
                this.updateTotalBudget(total, clear, isHaveRoute, isRepeatSubsidy)
              }
          ...
          !loading && (
            <TravelElementType
              ...
              updateTravelItinerarys={travelItinerarys =>
                this.setState({ travelItinerarys: travelItinerarys })
              }
              infoDetail={info}
              formIsChange={formIsChange}
              setInfo={{
    ...
    return (
      <div className="new-request">
        <Spin spinning={loading}>
          {!!approvalHistory.length &&
            !loading && (
              <Tabs type="card">
                <TabPane
                  tab={this.$t('request.detail.request.info') /*申请单信息*/}
                  key="requestInfo"
                >
                  {requestInfo}
                </TabPane>
          ...
          {formType === 2001 && (
            <Row className="total-budget">
              <span className="total">
                {this.$t('itinerary.form.travel.info.total') /*总金额*/}:{currentCodeType}&nbsp;{React.Component.prototype.filterMoney(
                  totalBudget
                )}{' '}
              </span>
              <span className="budget">
                {this.$t('itinerary.form.travel.info.fee.total') /*费用总金额*/}:{React.Component.prototype.filterMoney(
                  amount
                )}
                + {this.$t('itinerary.form.travel.info.subsidy.total') /*差补总金额*/}:{React.Component.prototype.filterMoney(
                  total
                )}
              </span>
            </Row>
        ...
        <Modal
          title={this.$t('itinerary.form.submit.booking.title')} /*"统一订票"*/
          ...
          <Form>
            {subsidyCtrl.flight && (
              <Row>
                <Col span={6} style={{ marginTop: 9 }}>
                  <span>
                    {this.$t('itinerary.form.submit.booking.flight.title') /*统一订机票*/}：
                  </span>
                </Col>
            ...
            {subsidyCtrl.train && (
              <Row>
                <Col span={6} style={{ marginTop: 9 }}>
                  <span>
                    {this.$t('itinerary.form.submit.booking.train.title') /*统一订火车票*/}：
                  </span>
                </Col>
                <Col span={4} style={{ marginTop: 7 }}>
                  <Switch
            ...
            {subsidyCtrl.hotel && (
              <Row>
                <Col span={6} style={{ marginTop: 9 }}>
                  <span>
                    {this.$t('itinerary.form.submit.booking.hotel.title') /*统一订酒店*/}：
                  </span>
                </Col>
                <Col span={4} style={{ marginTop: 7 }}>
                  <Switch
        ...
        <Modal
          title={this.$t('itinerary.form.submit.budgeting.title') /*"预算校验"*/}
          visible={budgeting}
        >
