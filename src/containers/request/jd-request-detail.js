...
class JDRequestDetail extends React.Component {
    ...
    httpFetch
      ...
      .then(res => {
        if (res.status === 200) {
          ...
          message.success(
            this.props.intl.formatMessage({ id: 'common.operate.success' } /*操作成功*/)
          );
          this.goBack();
        }
      })
      .catch(e => {
        this.setState({ passLoading: false });
        message.error(
          `${this.props.intl.formatMessage({ id: 'common.operate.filed' } /*操作失败*/)}，${
  ...
  handleRecall = () => {
    ...
    httpFetch
      .post(`${config.baseUrl}/api/approvals/withdraw`, params)
      .then(res => {
        if (res.status === 200) {
  ...
  getRemainingTime = createDate => {
    ...
    if (remainDay < 0) {
      ...
      return (
        <Tag color="#ff9900">
          剩余付款时间：{remainDay}天{remainHour}小时
        </Tag>
      );
    }
  };
      ...
      <Spin spinning={loading}>
        <div className="top-info">
          <Row className="row-container">
            ...
            {!canApprove && (
              <Button type="primary" className="top-info-btn" onClick={this.handleRecall}>
                撤 回
              </Button>
            )}
          </Row>
          <Row className="row-container">
            ...
            <span className="detail-info">
              提交日期：{moment(info.submittedDate).format('YYYY-MM-DD')}，由 {} 代提
            </span>
            <span className="detail-info">
              当前状态：{status.map(item => {
                if (
          ...
          <Row className="row-container">
            <h3 className="amount">
              总金额：{info.currencyCode}{' '}
              {this.renderMoney((productInfo.totalAmount || 0) + (productInfo.freight || 0))}
            </h3>
          </Row>
        </div>
      </Spin>
            ...
            {approvalHistory.map(item => {
              return (
                <Timeline.Item
                  ...
                  dot={
                    item.operation === 1001 ? (
                      <Icon type="upload" style={{ color: '#ff9900' }} />
                    ) : (
                      ''
                    )
                  }
          ...
          <div className="amount-info">
            <Row>
              <Col span={3} className="amount">
                {info.currencyCode} {this.renderMoney(productInfo.totalAmount || 0)}
              </Col>
            ...
            <Row>
              <Col span={3} className="amount">
                {info.currencyCode} {this.renderMoney(productInfo.freight || 0)}
              </Col>
              <Col span={2} className="amount-title">
                运费：
              </Col>
      ...
      <div className="jd-request-detail background-transparent">
        <div className="tabs-info">
          <Tabs onChange={this.handleTabsChange}>
            <TabPane tab="申请单信息" key="requestInfo">
              {requestInfo}
            </TabPane>
            <TabPane tab="审批进度" key="approvals">
              {approvals}
            </TabPane>
          </Tabs>
        </div>
