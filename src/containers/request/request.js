...
import { Form, Table, Button, Badge, Input, Popover, Dropdown, Icon, Menu, message, Modal, Select } from 'antd'
...
class ApplicationList extends React.Component{
  constructor(props){
    ...
    this.state = {
      ...
      checkboxListForm: [{
        id: 'status',
        single: true,
        items: [{label: this.$t('request.status'/*状态*/), key: 'statusWithReject', checked: ['10011002100310041005100610071008'], options: [
          {label: this.$t('request.all'/*全部*/), value: '10011002100310041005100610071008'},
          {label: this.$t('constants.documentStatus.editing'/*编辑中*/), value: '10011000', state: 'processing'},
          {label: this.$t('constants.documentStatus.has.withdrawn'/*已撤回*/), value: '10011001', state: 'warning'},
          {label: this.$t('constants.documentStatus.has.been.rejected'/*已驳回*/), value: '10011002', state: 'error'},
          {label: this.$t('constants.documentStatus.audit.rejected'/*审核驳回*/), value: '10011003', state: 'error'},
          {label: this.$t('constants.documentStatus.auditing'/*审批中*/), value: '10021000', state: 'processing'},
          {label: this.$t('constants.documentStatus.yet.pass'/*已通过*/), value: '10031000', state: 'success'},
          {label: this.$t('constants.documentStatus.audit.pass'/*审核通过*/), value: '1004', state: 'success'},
          {label: this.$t('constants.documentStatus.yet.pay'/*已付款*/), value: '1005', state: 'success'},
          {label: this.$t('constants.documentStatus.repaying'/*还款中*/), value: '1006', state: 'processing'},
          {label: this.$t('constants.documentStatus.yet.repayment'/*已还款*/), value: '1007', state: 'success'},
          {label: this.$t('constants.documentStatus.paying'/*付款中*/), value: '1008', state: 'processing'},
          {label: this.$t('constants.documentStatus.yet.disable'/*已停用*/), value: '1009', state: 'default'}
        ]}]
      }, {
        id: 'formOID',
        single: true,
        items: [{label: this.$t('request.bill.name'/*单据名称*/), key: 'formOID',
          options: [{label: this.$t('request.all'/*全部*/), value: 'all'}], checked: ['all']}]
      }],
      searchForm: [
      ...
      columns: [
        {title: this.$t('common.sequence'), dataIndex: 'index', align:"center",render: (value, record, index) =>
        index + 1 + this.state.pageSize * this.state.page, width: '6%'},
        {title: this.$t('request.create.date'/*创建日期*/),align:"center",dataIndex: 'createdDate', width: '90',render: (value, record) =>
          moment(value).format('YYYY-MM-DD')},
        {title: this.$t('request.applicant'/*申请人*/), dataIndex: 'applicantName',width: '100'},
        {title: this.$t('request.bill.name'/*单据名称*/), dataIndex: 'formName', render: value =>
          value ? <Popover placement="topLeft" content={value}>{value}</Popover> : '-'},
        {title: this.$t('request.reason'/*事由*/), dataIndex: 'title', render: value => value ?
  ...
  getList = () => {
    ...
    requestService.getRequestList(page, pageSize, searchParams).then(res => {
      this.setState({
        loading: false,
        data: res.data,
        pagination: {
  ...
  expandedRowRender= (record) => {
    ...
    return expandedFlag ? (
      <div className="expanded-row">
        {!!record.warning && <div>{this.$t('request.over.budget')/*超预算*/}：{JSON.parse(record.warning).message}</div>}
        {record.rejectType === 1002 && <div>{this.$t('request.approve.reject')/*审批驳回*/}：{record.rejectReason}</div>}
        {record.rejectType === 1003 && <div>{this.$t('request.audit.reject')/*审核驳回*/}：{record.rejectReason}</div>}
        {!!(record.participantClosed || record.closed || (record.applicationParticipant && record.applicationParticipant.closed)) && (
  ...
  handleNewRequestProxy = () => {
    let redirect_url = this.state.newRequest.url.replace(':formOID', this.state.proxyFormOID).replace(':applicantOID', this.state.proxyApplicantOID);
  ...
  render(){
    const { loading, requestTypes, checkboxListForm, searchForm, columns, data, pagination } = this.state;
    const { proxyVisible, applicantList, proxyApplicantOID, proxyFormList, proxyFormOID, isShowProxy} = this.state;
    ...
    return(
      <div className="application-list">
        ...
        <div className="table-header">
          <div className="table-header-title">{this.$t('common.total', {total: pagination.total || 0})/*共搜索到 {total} 条数据*/}</div>
          <div className="table-header-buttons">
            <Dropdown overlay={menu}>
              <Button type="primary">{this.$t('request.create')/*新建申请单*/} <Icon type="down" /></Button>
            </Dropdown>
        ...
        <Table rowKey="applicationOID"
               ...
               rowClassName={record =>
                 ...
                 || (record.participantClosed || record.closed)) ? '' : 'row-expand-display-none'}
               onRow={record => ({onClick: () => this.handleRowClick(record)})}
               loading={loading}
               bordered
               size="middle"/>
        <Modal title={this.$t('request.proxy')/*代提申请单*/} visible={proxyVisible} onCancel={this.closeProxyModal} maskClosable={false} destroyOnClose={true}
               footer={[
                 <Button key="back" onClick={this.closeProxyModal}>{this.$t('common.cancel')/*取消*/}</Button>,
                 <Button key="submit" type="primary" disabled={!proxyApplicantOID || !proxyFormOID} onClick={this.handleNewRequestProxy}>{this.$t('common.ok')/*确定*/}</Button>
               ]}
        >
          <Form>
            <FormItem {...formItemLayout} label={this.$t('request.applicant')/*申请人*/}>
              <Select showSearch placeholder={this.$t('common.please.select')/*请选择*/} onChange={this.handleSelectApplicant} optionFilterProp="children"
                      filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
              >
                {applicantList.map(item=><Option value={item.userOID} key={item.userOID}>{`${item.fullName}  ${item.employeeID}`}</Option>)}
              </Select>
            </FormItem>
            {!!proxyApplicantOID &&
            <FormItem {...formItemLayout} label={this.$t('request.bill.name')/*单据名称*/}>
              <Select showSearch placeholder={this.$t('common.please.select')/*请选择*/} value={proxyFormOID ? proxyFormOID : undefined} onChange={this.handleSelectForm} optionFilterProp="children"
                      filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
              >
                {proxyFormList.map(item=><Option value={item.formOID} key={item.formOID}>{item.formName}</Option>)}
              </Select>
            </FormItem>
            }
          </Form>
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedApplicationList)
