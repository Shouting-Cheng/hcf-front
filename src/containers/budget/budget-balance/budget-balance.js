...
class BudgetBalance extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        ...
        //操作
        {
          ...
          render: (text, record, index) => (
            <span>
              <Popconfirm
                onConfirm={e => this.deleteItem(e, index)}
                title={
                  this.$t('budget.balance.are.you.sure.to.delete.this.data') /* 你确定要删除吗？*/
                }
              >
                <a
                  onClick={e => {
      ...
      costCenterSelectorItem: {
        //成本中心所需要的selectorItem，url需要在params的onChange里手动添加
  ...
  componentWillMount() {
    ...
    httpFetch
      .get(
        `${config.baseUrl}/api/periods/get/years/by/setOfBooksId?setOfBooksId=${
    ...
    let searchForm = [
      //预算版本
      {
        type: 'select',
        id: 'versionId',
        label: this.$t('budget.balance.budget.version'),
        isRequired: true,
        options: [],
        method: 'get',
        getUrl: `${config.budgetUrl}/api/budget/versions/queryAll`,
        getParams: organizationIdParams,
      ...
      //表
      {
        type: 'select',
        id: 'structureId',
        label: this.$t('budget.balance.budget.structure'),
        isRequired: true,
        options: [],
        method: 'get',
      ...
      //预算场景
      {
        type: 'select',
        id: 'scenarioId',
        label: this.$t('budget.balance.budget.scenarios'),
        isRequired: true,
        options: [],
        method: 'get',
      ...
      //年度
      {
        type: 'select',
        id: 'yearLimit',
        label: this.$t('budget.balance.year'),
        options: yearOptions,
        event: 'YEAR_CHANGE',
        isRequired: true,
      },
      {
        ...
        items: [
          //期间从
          {
            type: 'select',
            id: 'periodLowerLimit',
            label: this.$t('budget.balance.period.from'),
            options: [],
            method: 'get',
            disabled: true,
            getUrl: `${config.baseUrl}/api/company/group/assign/query/budget/periods`,
          ...
          //期间到
          {
            type: 'select',
            id: 'periodUpperLimit',
            label: this.$t('budget.balance.period.to'),
            options: [],
            method: 'get',
            disabled: true,
            getUrl: `${config.baseUrl}/api/company/group/assign/query/budget/periods`,
      ...
      },
      {
        ...
        items: [
          ...
          {
            type: 'value_list',
            id: 'quarterLowerLimit',
            label: this.$t('budget.balance.season.from'),
            options: [],
            valueListCode: 2021,
            disabled: true,
          },
          ...
          {
            type: 'value_list',
            id: 'quarterUpperLimit',
            label: this.$t('budget.balance.season.to'),
            options: [],
            valueListCode: 2021,
            disabled: true,
          },
      ...
      {
        type: 'value_list',
        id: 'periodSummaryFlag',
        label: this.$t('budget.balance.period.summary'),
        options: [],
        valueListCode: 2020,
        disabled: true,
      },
      ...
      {
        type: 'value_list',
        id: 'amountQuarterFlag',
        label: this.$t('budget.balance.money.or.number'),
        isRequired: true,
        options: [],
        valueListCode: 2019,
      },
    ...
    itemSelectorItem.searchForm[1].getUrl = itemSelectorItem.searchForm[2].getUrl = itemSelectorItem.searchForm[2].getUrl.replace(
      ':organizationId',
    ...
    switch (dataIndex) {
      ...
      case 'value': {
        ...
        let param = params[index].params
          ? params[index].type === 'BGT_RULE_PARAMETER_DIM'
            ? params[index].paramMap
            : paramValueMap[params[index].params]
          : null;
        return (
          <Row gutter={20}>
            <Col span={12}>
              <Select
                value={params[index].allFlag ? '1' : '2'}
                onChange={allFlag => {
                  this.handleChangeIsAll(allFlag, index);
                }}
                disabled={param === null}
              >
            ...
            <Col span={12}>
              <Chooser
                disabled={param === null || params[index].allFlag}
        ...
        structureId &&
          httpFetch
            .get(
              `${
                config.budgetUrl
              }/api/budget/structure/assign/layouts/queryAll?structureId=${structureId}`
            )
            .then(res => {
              let options = [];
              res.data.map(data => {
  ...
  search = e => {
    ...
    this.validate(values => {
      httpFetch
        .post(`${config.budgetUrl}/api/budget/balance/query/header/user`, values)
        .then(res => {
          this.setState({ searching: false });
          this.props.dispatch(
            routerRedux.replace({
              pathname: '/budget/budget-balance/budget-balance-result/:id'.replace(':id', res.data),
            })
          );
        })
        .catch(e => {
          if (e.response.data) {
            message.error(
              e.response.data.validationErrors
                ? e.response.data.validationErrors[0].message
  ...
  showSaveModal = () => {
    this.validate(() => {
      ...
      else this.setState({ conditionName: '', conditionCode: '', showSaveModal: true });
    });
  };
  ...
  handleSaveCondition = () => {
    this.validate(values => {
      ...
      let method;
      if (this.state.saveNewCondition || !this.state.condition) method = 'post';
      else {
        method = 'put';
        values.id = this.state.condition.id;
        values.versionNumber = this.state.condition.versionNumber;
      }
      httpFetch[method](`${config.budgetUrl}/api/budget/balance/query/header`, values)
        .then(() => {
        ...
        this.state.params.map(param => {
          ...
          !param.allFlag &&
            param.value.map(value => {
              let paramItem =
                param.type === 'BGT_RULE_PARAMETER_DIM'
                  ? costCenterSelectorItem
                  : paramValueMap[param.params];
              let queryParameter = {
                parameterValueId:
                  paramItem.listType === 'new_currency'
                    ? null
                    : paramItem.valueKey
                      ? value[paramItem.valueKey]
                      : null,
                parameterValueCode:
                  paramItem.listType === 'new_currency'
                    ? value[paramItem.valueKey]
                    : paramItem.codeKey
                      ? value[paramItem.codeKey]
                      : null,
  ...
  setValues = options => {
    Object.keys(options).map(key => {
      ...
      searchForm[4].items[0].getParams = searchForm[4].items[1].getParams = {
        setOfBooksId: this.props.company.setOfBooksId,
        periodYear: options.yearLimit,
      };
    ...
    this.setState({ showSlideFrame: false }, () => {
      if (condition) {
        ...
        this.setValues({
          ...
          periodLowerLimit: condition.periodLowerLimit
            ? { value: condition.periodLowerLimit, label: condition.periodLowerLimit }
            : null,
          periodUpperLimit: condition.periodUpperLimit
            ? { value: condition.periodUpperLimit, label: condition.periodUpperLimit }
            : null,
          periodSummaryFlag: {
          ...
          quarterLowerLimit: condition.quarterLowerLimit
            ...
            : null,
          quarterUpperLimit: condition.quarterUpperLimit
            ? { value: condition.quarterUpperLimit, label: condition.quarterUpperLimit }
            : null,
          amountQuarterFlag: {
            value: condition.amountQuarterFlag,
            label: condition.amountQuarterFlagName,
          },
        });
        this.setState({ structureId: condition.structureId, tableLoading: true }, () => {
          ...
          condition.queryLineList.map((item, index) => {
            let newParams = {
              ...
              params: item.parameterCode,
              value: [],
              key: paramsKey,
              allFlag: item.allFlag,
            };
            item.queryParameterList &&
              item.queryParameterList.map(queryParameter => {
                let val = {};
                let mapItem =
                  item.parameterType === 'BGT_RULE_PARAMETER_DIM'
                    ? this.state.costCenterSelectorItem
                    : paramValueMap[item.parameterCode];
                if (item.parameterCode !== 'CURRENCY')
                  val[mapItem.codeKey] = queryParameter.parameterValueCode;
                val[mapItem.valueKey] =
                  item.parameterCode === 'CURRENCY'
                    ? queryParameter.parameterValueCode
                    : queryParameter.parameterValueId;
          ...
          this.setState(
            { params, paramsKey, condition, saveNewCondition: true },
  ...
  setFieldsByStructureId = () => {
    ...
    structureId &&
      httpFetch.get(`${config.budgetUrl}/api/budget/structures/${structureId}`).then(res => {
        let periodStrategy = res.data.periodStrategy;
        let searchForm = this.state.searchForm;
        searchForm[4].items[0].isRequired = periodStrategy === 'MONTH';
        searchForm[4].items[0].disabled = searchForm[4].items[1].disabled =
          periodStrategy !== 'MONTH';
        searchForm[4].items[0].options = searchForm[4].items[1].options = [];
        searchForm[5].items[0].isRequired = searchForm[5].items[1].isRequired =
          periodStrategy === 'QUARTER';
        searchForm[5].items[0].disabled = searchForm[5].items[1].disabled =
          periodStrategy !== 'QUARTER';
          ...
          value &&
            httpFetch
              .get(
                `${
                  config.budgetUrl
                }/api/budget/structure/assign/layouts/queryAll?structureId=${value}`
              )
              .then(res => {
                let options = [];
                res.data.map(data => {
                ...
                this.setState({ queryLineListParamOptions }, () => {
                  ...
                  this.setState(
                    { structureId: value, params: targetParams, tableLoading: false },
                    () => {
                      this.setFieldsByStructureId();
                    }
                  );
      ...
      searchForm[index].items = searchForm[index].items.map(searchItem => {
        if (searchItem.id === item.id) {
          ...
          if (
            ...
            searchItem.options = [
              { label: value.label, key: value.value, value: dataOption, temp: true },
            ];
          }
        }
        return searchItem;
      ...
      <div className="budget-balance">
        <Form className="ant-advanced-search-form" onSubmit={this.search}>
          <div className="base-condition">
            ...
            <Row gutter={40} className="base-condition-content" type="flex" align="top">
              {this.getFields()}
            </Row>
          </div>
          <div className="footer-operate">
            ...
            <Button style={{ marginLeft: 10, marginRight: 20 }} onClick={this.clear}>
              {this.$t('budget.balance.reset') /* 重置 */}
            </Button>
            <Button style={{ marginRight: 10 }} onClick={this.showSaveModal}>
              {this.$t('budget.balance.save.condition') /* 保存方案 */}
            </Button>
            <Button
              onClick={() => {
            ...
            >
              {this.$t('budget.balance.use.condition') /* 使用现有方案 */}
            </Button>
            {condition ? (
              <div className="condition-name">
                {this.$t('budget.balance.using')}
          ...
          <div className="table-header">
            <div className="table-header-title">
              {this.$t('budget.balance.search.dimension') /* 查询维度 */}
            </div>
            <div className="table-header-buttons">
              <Button onClick={this.handleNew} disabled={params.length === 20}>
                {this.$t('budget.balance.add') /* 添加 */}
              </Button>
            </div>
          </div>
          <Table
        ...
        >
          <BudgetBalanceCondition onClose={this.useCondition} />
        </SlideFrame>
        <Modal
          title={this.$t('budget.balance.save.condition') /* 保存方案 */}
        ...
        >
          <div className="save-modal-content">
            ...
            <Input
              onChange={e => this.setState({ conditionName: e.target.value })}
              value={conditionName}
            />
            <br />
            {condition ? (
              <Checkbox
                checked={saveNewCondition}
                defaultValu={saveNewCondition}
                onChange={e => {
                  this.setState({ saveNewCondition: e.target.checked });
                }}
              >
                {this.$t('budget.balance.save.as.new.condition') /* 保存为新方案 */}
              </Checkbox>
            ) : null}
          </div>
        </Modal>
