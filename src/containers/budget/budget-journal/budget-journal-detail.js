...
import { Affix, Popover, Button, Table, Select, Modal, message, Popconfirm, notification, Icon, Spin, Collapse, Timeline } from 'antd';
...
class BudgetJournalDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      infoList: [
        /*状态*/
        { type: 'badge', label: formatMessage({ id: "budgetJournal.status" }), id: 'status' },
        /*预算日记账编号*/
        { type: 'input', label: formatMessage({ id: "budgetJournal.journalCode" }), id: 'journalCode', disabled: true },
        /*总金额*/
        { type: 'input', label: formatMessage({ id: "budgetJournal.total.amount" }), id: 'totalAmount', disabled: true },
        /*申请人*/
        { type: 'input', label: formatMessage({ id: "budgetJournal.employeeId" }), id: 'employeeName', disabled: true },
        /*公司*/
        { type: 'input', label: formatMessage({ id: "budgetJournal.companyId" }), id: 'companyName', disabled: true },
        /*部门*/
        { type: 'input', label: formatMessage({ id: "budgetJournal.unitId" }), id: 'unitName', disabled: true },
        /*创建时间*/
        { type: 'date', label: formatMessage({ id: "budgetJournal.createdDate" }), id: 'createdDate', disabled: true },
        /*预算日记账类型*/
        {
          ...
          label: formatMessage({ id: 'budgetJournal.journalTypeId' }),
          listExtraParams: { organizationId: this.props.organization.id },
          disabled: true
        },
        /*预算表*/
        {
          type: 'select', id: 'budgetStructure', label: formatMessage({ id: 'budgetJournal.structureId' }), options: [], method: 'get', disabled: true,
        ...
        /*预算版本*/
        {
          ...
          valueKey: 'id',
          single: true,
          label: formatMessage({ id: 'budgetJournal.versionId' }),
        ...
        /*预算场景*/
        {
          ...
          valueKey: 'id',
          single: true,
          label: formatMessage({ id: 'budgetJournal.scenarios' }),  /*预算场景*/
          listExtraParams: { "organizationId": this.props.organization.id, "enabled": true }
        },
        /*编辑期段*/
        { type: 'value_list', id: 'periodStrategy', label: formatMessage({ id: 'budgetJournal.periodStrategy' }), options: [], valueListCode: 2002, disabled: true },
        /*附件*/
        { type: 'file', label: formatMessage({ id: 'budgetJournal.attachment' }), id: 'file', disabled: true },
      ...
      columns: [
        {          /*公司*/
          title: formatMessage({ id: "budgetJournal.companyId" }), key: "companyName", dataIndex: 'companyName', width: '5%',
          render: companyName => (
        ...
        {          /*部门*/
          title: formatMessage({ id: "budgetJournal.unitId" }), key: "departmentName", dataIndex: 'departmentName', width: '5%',
        ...
        {          /*员工*/
          title: formatMessage({ id: "budgetJournal.employee" }), key: "employeeName", dataIndex: 'employeeName', width: '5%',
          render: recode => (
        ...
        {          /*预算项目*/
          title: formatMessage({ id: "budgetJournal.item" }), key: "itemName", dataIndex: 'itemName', width: '10%',
          render: itemName => (
        ...
        {          /*期间*/
          title: formatMessage({ id: "budgetJournal.periodName" }), key: "periodName", dataIndex: 'periodName',
        },
        {          /*季度*/
          title: formatMessage({ id: "budgetJournal.periodQuarter" }), key: "periodQuarterName", dataIndex: 'periodQuarterName',
        },
        {          /*年度*/
          title: formatMessage({ id: "budgetJournal.periodYear" }), key: "periodYear", dataIndex: 'periodYear',
        },
        {          /*币种*/
          title: formatMessage({ id: "budgetJournal.currency" }), key: "currency", dataIndex: 'currency',
        },
        {          /*汇率*/
          title: formatMessage({ id: "budgetJournal.rate" }), key: "rate", dataIndex: 'rate',
          render: rate => (
        ...
        {          /*金额*/
          title: formatMessage({ id: "budgetJournal.amount" }), key: "amount", dataIndex: 'amount', width: 180,
          render: recode => (
        ...
        {          /*本币今额*/
          title: formatMessage({ id: "budgetJournal.functionalAmount" }), key: "functionalAmount", dataIndex: 'functionalAmount', width: 180,
          render: recode => (
            <Popover content={this.filterMoney(recode)}>
              {this.filterMoney(recode)}
            </Popover>)
        ...
        {          /*数字*/
          title: formatMessage({ id: "budgetJournal.quantity" }), key: "quantity", dataIndex: 'quantity',
        },
        {          /*备注*/
  ...
  handleDeleteLine = () => {
    ...
    budgetJournalService.deleteBudgetJournalLine(selectedRowKeys).then((req) => {
      this.getBudgetJournalLine();
      this.getToleAmount();
      ...
      message.error(`${formatMessage({ id: 'common.operate.filed' })}`);
    })
  };
  ...
  getBudgetJournalHead() {
    ...
    budgetJournalService.getBudgetJournalHeaderDetil(journalCode).then((response) => {
      ...
      const periodStrategy = {
        "label": period == "YEAR" ? formatMessage({ id: 'budgetJournal.year' }) : (period == "QUARTER" ? formatMessage({ id: 'budgetJournal.quarter' }) : formatMessage({ id: 'budgetJournal.month' })),
        "value": period
      };
  ...
  handleSaveJournal() {
    ...
    budgetJournalService.addBudgetJournalHeaderLine(headerAndListData).then((req) => {
      message.success(`${formatMessage({ id: 'common.operate.success' })}`);
      this.getBudgetJournalHead();
      this.getToleAmount();
    }).catch((e) => {
  ...
  handleAfterCloseNewSlide = (value) => {
    if (value) {
      ...
      data.functionalAmount = data.functionalAmount?data.functionalAmount.toString().replace(/\,/g, ""):undefined;
      budgetJournalService.addBudgetJournalLine(data).then((req) => {
        message.success(`${formatMessage({ id: 'common.operate.success' })}`);
        this.getBudgetJournalLine();
        this.getToleAmount();
      }).catch((e) => {
  ...
  handleDeleteJournal = () => {
    ...
    budgetJournalService.deleteBudgetJournal(id).then((req) => {
      message.success(`${formatMessage({ id: 'common.operate.success' })}`);
      //删除完该预算日记账，跳转
  ...
  handlePut = () => {
    ...
    if (header.formOid) {
      ...
      if (this.state.commitFlag) {
        budgetJournalService.commitBudgetJournalWorkflow(data).then((req) => {
      ...
      if (this.state.commitFlag) {
        let header = this.state.headerAndListData.dto;
        budgetJournalService.commitBudgetJournal(header.id).then((res) => {
          message.success(`${formatMessage({ id: 'common.operate.success' })}`);
          this.setState({
            listData: [],
          });
        ...
        notification.open({
          message: formatMessage({ id: "budgetJournal.notEmpty" }),
          description: formatMessage({ id: "budgetJournal.andOrLead" }),
          icon: <Icon type="frown-circle" style={{ color: '#e93652' }} />,
        });
      }
    }
  ...
  render() {
    const { historyData, loading, data, templateUrl, errorUrl, uploadUrl, columns, pagination, formData, infoDate, infoList, updateState, showSlideFrameNew, rowSelection, showImportFrame, headerAndListData, commitLoading, headerData } = this.state;
    return (
        ...
        <Spin spinning={commitLoading}>
          <div className="budget-journal-cent">
            ...
            <div className="table-header">
              <div className="table-header-title">{formatMessage({ id: 'common.total' }, { total: `${this.state.pagination.total}` })}/{formatMessage({ id: 'common.total' }, { total: `${this.state.total}` })}</div>
              <div className="table-header-buttons">
                <Button type="primary" onClick={this.showSlideFrameNewData}>{formatMessage({ id: "common.add" })}</Button>
                ...
                <Importer
                  ...
                  uploadUrl={uploadUrl}
                  errorUrl={errorUrl}
                  title={formatMessage({ id: 'budgetJournal.leading' })}
                  fileName={formatMessage({ id: 'budgetJournal.budgetJournalLeading' })}
                  onOk={this.onLoadOk}
                  afterClose={() => this.showImport(false)}
                />
                <Popconfirm placement="topLeft" title={formatMessage({ id: 'common.delete' })} onConfirm={this.handleDeleteLine} okText={formatMessage({ id: 'common.ok' })} cancelText={formatMessage({ id: 'common.cancel' })}>
                  <Button className="delete" disabled={this.state.selectedRowKeys.length === 0} >{formatMessage({ id: "common.delete" })}</Button>
                </Popconfirm>
              </div>
            </div>
          ...
          <SlideFrame title={formatMessage({ id: 'budgetJournal.journal' })}
          ...
          <Affix offsetBottom={0}
            ...
            <Popconfirm style={{ width: 200 }} placement="topLeft" title={formatMessage({ id: 'budgetJournal.commit' })} onConfirm={this.handlePut} okText={formatMessage({ id: 'common.ok' })} cancelText={formatMessage({ id: 'common.cancel' })}>
              <Button type="primary" style={{ marginLeft: '20px', marginRight: '8px' }}>{formatMessage({ id: 'budgetJournal.commit' })}</Button>
            </Popconfirm>
            <Popconfirm placement="topLeft" title={formatMessage({ id: "budgetJournal.delete.journal" })} onConfirm={this.handleDeleteJournal} okText={formatMessage({ id: 'common.ok' })} cancelText={formatMessage({ id: 'common.cancel' })}>
              <Button className="delete" style={{ marginRight: '8px' }}>{formatMessage({ id: "budgetJournal.delete.journal" })}</Button>
            </Popconfirm>
            <Button onClick={this.handleReturn}>{formatMessage({ id: "budgetJournal.return" })}</Button>
          </Affix>
        </Spin>
      </div>
    )
...
export default connect(mapStateToProps, null, null, { withRef: true })((BudgetJournalDetail));
