...
import { Spin, Popover, Button, Collapse, Affix, Table, message, Icon, Badge, Row, Col, Steps, Popconfirm } from 'antd';
...
class BudgetJournalDetailSubmit extends React.Component {
  ...
  getStatus = () => {
    ...
    switch (infoData.status) {
      case 1001: { return <Badge status="processing" text={'编辑中'} /> }
      case 1003: { return <Badge status="default" text={'撤回'} /> }
      case 1005: { return <Badge status="error" text={'审批驳回'} /> }
      case 1004: { return < Badge status="success" text={'审批通过'} /> }
      case 1002: { return <Badge status="warning" text={'审批中'} /> }
      case 5001: { return <Badge status="default" text={'复核'} /> }
      case 5002: { return <Badge status="default" text={'反冲提交'} /> }
      case 5003: { return <Badge status="default" text={'反冲审核'} /> }
  ...
  //审批中返回，撤回按钮
  getCheckingButton() {
    return this.state.infoData.status === 1002 ? <Button className="button-Revocation" type="primary" onClick={this.handleRevocation}>{formatMessage({ id: "budgetJournal.returnCommit" })}</Button> : ''
  }
    ...
    return (
      <div className="budget-journal-detail-submit">
        ...
        <Spin spinning={this.state.spinLoading}>
          ...
          <Affix offsetBottom={0} style={{ position: 'fixed', bottom: 0, marginLeft: '-35px', width: '100%', height: '50px', boxShadow: '0px -5px 5px rgba(0, 0, 0, 0.067)', background: '#fff', lineHeight: '50px' }}>
            <div>
              <Button className="button-return" style={{ marginLeft: '20px', marginRight: '8px' }} onClick={this.HandleReturn}>{formatMessage({ id: "budgetJournal.return" })}</Button>
              {this.state.infoData.status === 1002 ?
                (<Popconfirm placement="topLeft" title={formatMessage({ id: "budgetJournal.returnCommit" })} onConfirm={this.handleRevocation} okText={formatMessage({ id: 'common.ok' })} cancelText={formatMessage({ id: 'common.cancel' })}>
                    <Button className="button-Revocation" type="primary" style={{ marginLeft: '20px', marginRight: '8px' }}>{formatMessage({ id: "budgetJournal.returnCommit" })}</Button>
                  </Popconfirm>
                ) : ''}
            </div>
          </Affix>
...
export default connect(mapStateToProps, null, null, { withRef: true })((BudgetJournalDetailSubmit));
