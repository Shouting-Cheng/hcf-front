...
class NewBudgetJournalDetail extends React.Component {
  ...
  componentWillMount() {
    ...
    let searchForm = [
      ...
      }, //部门
      {
        ...
        key: 'employee',
        id: 'employee',
        label: this.$t({ id: 'budgetJournal.employee' }),
        labelKey: 'userName',
        valueKey: 'userId',
        single: true,
        event: 'employee',
      ...
      }, //人员
      {
        ...
        key: 'item',
        id: 'item',
        listType: 'journal_item',
        label: this.$t({ id: 'budgetJournal.item' }),
        isRequired: true,
        options: [],
        labelKey: 'itemName',
        valueKey: 'id',
        disabled: true,
        single: true,
        listExtraParams: { journalTypeId: '', companyId: '' },
      ...
      }, //预算项目
      {
        ...
        id: 'periodName',
        method: 'get',
        label: this.$t({ id: 'budgetJournal.periodName' }),
        isRequired: true,
        options: [],
        labelKey: 'periodName',
        valueKey: 'periodName',
      ...
      }, //期间
      {
        ...
        key: 'periodQuarter',
        id: 'periodQuarter',
        label: this.$t({ id: 'budgetJournal.periodQuarter' }),
        isRequired: true,
        options: queryLineListTypeOptions,
        disabled: true,
        columnLabel: 'periodQuarterName',
        columnValue: 'periodQuarter',
      }, //季度
      {
        type: 'select_year',
        key: 'periodYear',
        id: 'periodYear',
        label: this.$t({ id: 'budgetJournal.periodYear' }),
        isRequired: true,
        disabled: true,
        options: yearOptions,
        event: 'YEAR_CHANGE',
      ...
      }, //年度
      {
        ...
        key: 'currency',
        id: 'currency',
        label: this.$t({ id: 'budgetJournal.currency' }),
        isRequired: true,
      ...
      }, //币种
      {
        ...
        key: 'rate',
        id: 'rate',
        label: this.$t({ id: 'budgetJournal.rate' }),
        defaultValue: 1,
        isRequired: true,
        event: 'rate',
        disabled: true,
      }, //汇率
      {
        ...
        key: 'amount',
        id: 'amount',
        label: this.$t({ id: 'budgetJournal.amount' }),
        isRequired: true,
        step: 10,
        defaultValue: 0,
        event: 'amount',
      }, //金额
      {
        ...
        id: 'functionalAmount',
        label: this.$t({ id: 'budgetJournal.functionalAmount' }),
        step: 10,
        isRequired: true,
        defaultValue: 0,
        disabled: true,
      }, //本位金额
      {
        ...
        id: 'quantity',
        precision: 0,
        label: this.$t({ id: 'budgetJournal.quantity' }),
        step: 1,
        defaultValue: 1,
        min: 0,
      }, //数量
      {
        ...
        key: 'remark',
        id: 'remark',
        label: this.$t({ id: 'budgetJournal.remark' }),
      }, //备注
    ];
  ...
  componentDidMount = () => {
    if (this.props.params && JSON.stringify(this.props.params) !== '{}') {
      if (this.props.params.isNew === false) {
        ...
        if (this.props.params.unit.length > 0) {
          this.getItemAbled(
            false,
            this.props.params.company[0].id,
            this.props.params.company[0].id
          );
        }
      } else if (this.props.params != this.props.params) {
      ...
      //获取编制期段的控制
      if (this.props.params.periodStrategy && this.state.periodStrategyFlag) {
        this.setState(
          {
      ...
      //获取维度表单,
      if (this.props.params.structureId && this.state.structureIdFlag) {
        this.setState(
          {
      ...
      //预算项目控制
      if (
        this.props.params.journalTypeId &&
        this.props.params.company &&
        this.state.journalTypeIdFlag
      ) {
        this.setState(
          {
          ...
          () => {
            this.getItemUrl(this.props.params.company[0].id, this.props.params.journalTypeId);
          }
        );
      }
  ...
  componentWillReceiveProps = nextProps => {
    if (nextProps.params && JSON.stringify(nextProps.params) !== '{}') {
      ...
      //预算项目控制
      if (
        nextProps.params.journalTypeId &&
        nextProps.params.company &&
        this.state.journalTypeIdFlag
      ) {
        this.setState(
          {
        ...
        return (
          ...
          >
            {item.options.map(option => {
              return (
                <Option
                  key={option.data[item.valueKey]}
                  value={option.data ? JSON.stringify(option.data) : ''}
                  lable={option.label}
                >
                  {option.label}
                </Option>
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
                ...
                if (value.indexOf(`":"`) > 1) {
                  ...
                  if (item.type == 'select_dimension') {
                    if (!this.props.params.isNew) {
                      ...
                      let name = this.state.dimensionDataLists[item.id].defaultDimValueName
                        ? this.state.dimensionDataLists[item.id].defaultDimValueName
                        : '';
                      let value = this.state.dimensionDataLists[item.id].defaultDimValueId
                        ? this.state.dimensionDataLists[item.id].defaultDimValueId
                        : '';
        ...
        this.props.onClose(valuesDataClose);
        this.props.form.resetFields();
      }
    });
  };
  ...
  getDimensionByStructureId = () => {
    ...
    budgetJournalService
      ...
      .catch(e => {
        message.error(`${this.$t({ id: 'budgetJournal.getDimensionFail' })}`);
      });
  };
  ...
  getSearchForm(dimension) {
    ...
    for (let i = 0; i < dimension.length; i++) {
      ...
      let dimensionListKey = [
        'dimension' + priority + 'Id',
        'dimension' + priority + 'Name',
        'dimension' + priority + 'ValueId',
        'dimension' + priority + 'ValueName',
        'dimension' + priority,
      ];
      ...
      dimensionList[dimensionListKey[3]] = item.defaultDimValueName
        ? item.defaultDimValueName
        : null;
