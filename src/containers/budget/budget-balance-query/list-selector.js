 ...
 */
class BudgetListSelector extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  search = (params) => {
    ...
    this.state.dataSource.map(o => {
      ...
      Object.keys(params).map(key => {
        if (params[key] && o[key].indexOf(params[key]) < 0) {
          flag = false;
        }
      })
      if (flag) {
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    this.setState({ dataSource: nextProps.dataSource, data: nextProps.dataSource });
  };
...
export default connect(mapStateToProps, null, null, { withRef: true })(BudgetListSelector);
