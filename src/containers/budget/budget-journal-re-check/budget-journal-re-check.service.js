...
export default {
  ...
  //获取预算日记账行上的总金额
  getToleAmount(id) {
    return httpFetch.get(`${config.budgetUrl}/api/budget/journals/get/Line/amount/by/header/id?id=${id}`)
  },
  ...
  getBudgetJournalLineDetil(headerId, params) {
    return httpFetch.get(`${config.budgetUrl}/api/budget/journals/query/line/by/header/id?headerId=${headerId}`, params)
  },
  ...
  passJournal(operationDetail,params) {
    return httpFetch.post(`${config.budgetUrl}/api/budget/journals/balance/create?operationDetail=${operationDetail}`, params)
  },

  //预算日记账驳回（非工作流）
  rejectJournal(operationDetail,params) {
    return httpFetch.post(`${config.budgetUrl}/api/budget/journals/rejectJournal?operationDetail=${operationDetail}`, params)
  },
  //计算总金额，拼接上币种的
  getTotalCurrencyAmount(id) {
    return httpFetch.get(`${config.budgetUrl}/api/budget/journals/get/line/amount/group/by/header/id?id=${id}`)
  }
}
