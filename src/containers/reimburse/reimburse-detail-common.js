...
class ContractDetailCommon extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      contractStatus: {
        6002: { label: this.$t('my.contract.state.cancel' /*已取消*/), state: 'default' },
        6003: { label: this.$t('my.contract.state.finish' /*已完成*/), state: 'success' },
        1001: { label: this.$t('my.contract.state.generate' /*编辑中*/), state: 'processing' },
        6001: { label: this.$t('my.contract.state.hold' /*暂挂*/), state: 'warning' },
        1002: { label: this.$t('my.contract.state.submitted' /*审批中*/), state: 'processing' },
        1005: { label: this.$t('my.contract.state.rejected' /*已驳回*/), state: 'error' },
        1004: { label: this.$t('my.contract.state.confirm' /*已通过*/), state: 'success' },
        1003: { label: this.$t('my.contract.state.withdrawal' /*已撤回*/), state: 'warning' },
        2004: { label: '支付成功', state: 'success' },
        2003: { label: '支付中', state: 'processing' },
        2002: { label: '审核通过', state: 'success' },
      },
      subTabsList: [{ label: this.$t('my.contract.detail' /*详情*/), key: 'DETAIL' }],
      infoList: {
        title: '报账单',
        headItems: [
      ...
      columns: [
        ...
        { title: this.$t('my.contract.currency' /*币种*/), dataIndex: 'currency' },
        {
          title: this.$t('my.contract.plan.amount' /*计划金额*/),
          dataIndex: 'amount',
          render: this.filterMoney,
        },
        {
          title: this.$t('my.contract.partner.category' /*合同方类型*/),
          dataIndex: 'partnerCategoryName',
        },
        { title: this.$t('my.contract.partner' /*合同方*/), dataIndex: 'partnerName' },
        {
          title: this.$t('my.contract.plan.pay.date' /*计划付款日期*/),
          dataIndex: 'dueDate',
