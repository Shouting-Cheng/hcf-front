...
import { Form, Input, Select, Affix, Button, message, Col, DatePicker, TimePicker, InputNumber, Tag, Spin } from 'antd';
...
class FormList extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (JSON.stringify(nextProps.formSetings) != "{}" && JSON.stringify(this.props.formSetings) == "{}") {
      // this.getReceivables("");
      if (!nextProps.isNew) {
        let sign = nextProps.formSetings.payeeId ? `${nextProps.formSetings.payeeId}_${nextProps.formSetings.payeeCategory == "EMPLOYEE" ? "true" : "false"}` : "";
        this.setState({ show: true, formSetings: nextProps.formSetings, payeeId: { key: sign, label: nextProps.formSetings.defaultPaymentInfo.partnerName }, currencyCode: nextProps.formSetings.currencyCode }, () => {
  ...
  getReimburseItem = () => {
              ...
              <Select onChange={this.accountNumberChange}>
                {
                  this.state.accountList.map(o => {
                    return (
                      <Select.Option key={o.number} value={o.number}>{o.number}</Select.Option>
                    )
                  })
                }
              </Select>
    ...
    if (this.props.isNew && formSetings.relatedContract && formSetings.contractPosition === "DOCUMENTS_HEAD") {
      forms.push(
        <FormItem
          {...formItemLayout}
          label="关联合同"
        ...
        >
          {
            getFieldDecorator("contarct", {
              ...
              <Chooser placeholder={formatMessage({ id: "common.please.select" })}
                ...
                single={true}
                disabled={!this.state.companyId || !this.state.currencyCode || (!formSetings.multipleReceivables && !this.state.payeeId.key)}
                listExtraParams={{ documentType: "PREPAYMENT_REQUISITION", companyId: this.state.companyId, currency: this.state.currencyCode, partnerId: this.state.payeeId.key ? this.state.payeeId.key.split("_")[0] : "" }}
              />
              )
          }
        </FormItem>
        ...
        >
          {
            getFieldDecorator("contarct", {
              rules: [{ required: true, message: "请关联合同" }],
              initialValue: [{ contractHeaderId: formSetings.contractHeaderId, contractName: formSetings.contractHeaderLineDTO.contractName }]
            })(
              <Chooser placeholder={formatMessage({ id: "common.please.select" })}
                type="select_contract"
                labelKey="contractName"
  ...
  formDesign = () => {
    ...
    const { showCompanySelector, companySelectedData, showDepartmentSelector, deparmentSelectedData, applyer, formItemLayout, costCenterSelectedData } = this.state;
    const form = [];
          ...
          >
            {
              getFieldDecorator(i.fieldOID, {
                ...
                initialValue: isNew ? [{ companyOID: this.props.user.companyOID, name: this.props.user.companyName, id: this.state.companyId }] : [
                  { companyOID: formSetings.companyOid, name: formSetings.companyName, id: formSetings.companyId }]
              })(
                <Chooser placeholder={formatMessage({ id: "common.please.select" })}
            ...
            >
              {
                getFieldDecorator(i.fieldOID, {
                  rules: [{ required: i.required, message: i.promptInfo }],
                  initialValue: isNew ? [{ departmentOid: user.departmentOID, name: user.departmentName }] : [{ departmentOid: i.value, name: i.showValue }]
                })(
                  <Chooser placeholder={formatMessage({ id: "common.please.select" })}
            ...
            >
              {
                getFieldDecorator(i.fieldOID, {
                  ...
                  <TextArea disabled={i.readonly} placeholder={i.promptInfo ? i.promptInfo : "请输入"} rows="4">
            ...
            >
              {
                getFieldDecorator(i.fieldOID, {
                  ...
                  <Select labelInValue placeholder={i.promptInfo ? i.promptInfo : "请选择"} disabled={i.readonly} onFocus={() => this.handleFocus(i, "costCenterOID", i.fieldOID)}>
                    {
                      this.state.costData[i.fieldOID] && this.state.costData[i.fieldOID].map(o => {
                        return <Option key={o.costCenterItemOID} value={o.costCenterItemOID}>{o.name}</Option>
                      })
                    }
                  </Select>
                  )
            ...
            >
              {
                getFieldDecorator(i.fieldOID, {
                  ...
                  <Select disabled={!isNew} onFocus={this.getCurrencyList}
                    onChange={this.handleCurrencyChange}
                  >
                    {
                      currencyCodeList.map(value => {
                        return <Option key={value.currency} value={value.currency}>{value.currency}</Option>
                      })
                    }
                  </Select>
                  )
                  ...
                  <Select onFocus={this.selectApprover}>
                    {
                      this.state.approverList.map(o => {
                        return <Option key={o.userId} value={o.userId}>{o.userName}</Option>
                      })
                    }
                  </Select>
                  )
            ...
            >
              {
                getFieldDecorator(i.fieldOID, {
                  ...
                  <Upload attachmentType="PREPAYMENT"
  ...
  getReceivables = (value, payeeId) => {
    ...
    if (this.state.payeeId.key) {
      this.setState({ receivables: [{ sign: this.state.payeeId.key, name: this.state.payeeId.label }] }, () => {
        reimburseService.getReceivables(value, type).then(res => {
  ...
  onSelect = (value) => {
    ...
    if (data) {
      ...
      this.setState({ value: data.name, accountList: data.bankInfos, payeeId: value.key ? value : this.state.payeeId });
    }
  }

  getCurrencyList = () => {
    reimburseService.getCurrencyCode().then(res => {
