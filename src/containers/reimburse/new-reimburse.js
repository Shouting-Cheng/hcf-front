...
class NewReimburse extends React.Component {
  ...
  //判断当前用户是否有代理的关系
  getPrincipals() {
    httpFetch.get(`${config.baseUrl}/api/bill/proxy/principals`).then(res => {
      //存在代理人
  ...
  handleApplyerChange = (value) => {
    ...
    reimburseService.getSetOfBooks(value).then(res => {
      //设置新的账套
    ...
    reimburseService.getUserInfo(value).then(res => {
      this.setState({
        companySelectedData: [{
  ...
  getCustomList = (customEnumerationOID) => {
    reimburseService.getCustomEnumeration(customEnumerationOID).then(res => {
    }).catch(err => {
      message.error("网络错误！请稍后重试");
    })
  }
  ...
  getCustomFormFields() {
    ...
    reimburseService.getFormSet(this.props.routeParams.formOID).then(res => {
      this.setState({
    ...
    this.refs.formList.validateFieldsAndScroll((err, values) => {
      ...
      if (!err) {
        ...
        let data = {
          ...
          contractHeaderId: (values.contarct && values.contarct.length) ? values.contarct[0].contractHeaderId : "",
        };
        ...
        reimburseService.newReimburse(data).then(res => {
          if (200 === res.status) {
  ...
  render() {
    ...
    const { showCompanySelector, companySelectedData, showDepartmentSelector, costCenterSelectedData, deparmentSelectedData, applyer, formItemLayout, costCenterSelectorItem, showCostCenterSelector } = this.state;
      ...
      <div className="container reimburse">
        <Form onSubmit={this.handleSubmit}>
          ...
          <Spin spinning={this.state.loading}>
            <FormList isNew={this.state.isNew} formSetings={this.state.formSetings} customFormFields={this.state.customFormFields} user={this.props.user} company={this.props.company} routeParams={this.props.routeParams} ref="formList"></FormList>
          </Spin>
        </Form>
        <ListSelector visible={showCompanySelector}
          ...
          single={true} />
        <ListSelector visible={showDepartmentSelector}
          type='reimburse_department'
          onCancel={this.handleListCancel}
          onOk={this.handleDepartmentListOk}
          selectedData={deparmentSelectedData}
          single={true} />
        <ListSelector visible={this.state.showReceiverSelector}
        ...
        <Affix offsetBottom={0} className="bottom-bar">
          <Button style={{ marginLeft: 20, marginRight: 20 }} disabled={this.state.loading} loading={this.state.saveLoading} type="primary" onClick={this.handleSubmit}  >下一步</Button>
...
export default connect(mapStateToProps, null, null, { withRef: true })(NewReimburse);
