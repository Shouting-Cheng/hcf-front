...
import { Alert, Form, Switch, Icon, Input, Select, Button, Row, Col, message, Card, Popover, InputNumber, DatePicker, Spin, Popconfirm, Affix, Badge } from 'antd'
...
class NewExpense extends React.Component {
        ...
        this.state = {
            ...
            expenseTypeItem: {
                ...
                columns: [
                    ...
                    {
                        title: formatMessage({ id: "common.column.status" }), dataIndex: 'enable',
                        render: isEnabled => (
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (params.visible && !this.props.params.visible && !params.record.id) {
            ...
            expenseTypeItem.url = `${config.baseUrl}/api/v2/custom/forms/${params.headerData.formOid}/selected/expense/types`;

            this.setState({ isCopy: false, defaultApportion: params.defaultApportion, headerData: params.headerData, isShowInvoice: false, editModel: {}, expenseType: {}, nowPage: 'type', shareParams, expenseTypeItem }, () => {
        ...
        //显示并且是编辑
            ...
            this.setState({ nowPage: 'form', isCopy: params.isCopy, defaultApportion: params.defaultApportion, headerData: params.headerData, isShowInvoice: params.record.vatInvoice, loading: true, shareParams, expenseTypeItem }, () => {
    ...
    resetForm = (flag) => {
        ...
        var editModel = this.state.editModel.id ? { id: this.state.editModel.id, invoiceOID: this.state.editModel.invoiceOID, receiptGoodsID: this.state.editModel.receiptGoodsID, receiptID: this.state.editModel.receiptID } : {};

        // this.props.form.setFieldsValue({costType: [], vatInvoice: false, amount: 0, actualAmount: 0, comment: ""});
        this.upload.reset();

        this.setState({ applicationData: [], isShowInvoice: false, editModel: flag ? editModel : {}, expenseType: {}, nowPage: 'type', currentRate: "", isRefreshShareTabel: !this.state.isRefreshShareTabel, invoiceData: {}, attachmentOID: [], fileList: [] });
    }
    ...
    save = (callback) => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
                ...
                if (this.state.defaultApportion && this.state.defaultApportion.costCenterItems) {
                    costCenterItems = this.state.defaultApportion.costCenterItems.map(item => {
                        if (!o[item.costCenterOID] || !o[item.costCenterOID].key) {
                            ...
                            return { costCenterItemId: o[item.costCenterOID].key, costCenterItemName: o[item.costCenterOID].label }
                        }
                    })
                }
            ...
            reimburseService.newReportLine(data).then(res => {
                callback();
            }).catch(err => {
    ...
    newShareByApplication = () => {
        ...
        this.setState({ showSelectApplication: true, applincationParams: this.props.headerData, selectedData: [] });
    }
    ...
    newShare = (value) => {
        ...
        this.setState({ applicationData, shareVisible: false, isRefreshShareTabel: !this.state.isRefreshShareTabel });
    }
    ...
    render() {
        ...
        const { nowPage, expenseType, loading, saving, attachments, currencyList, nowCurrency,attachmentOID,
            businessCardConsumptions, nowBusinessCardConsumptionIndex, shareVisible, isCreateByApplication, saveLoading, editModel, fileList, invoiceData, applicationData, showSelectApplication, applincationParams, selectedData } = this.state;
        ...
        return (
            <div style={{ padding: 20 }}>
                {this.props.params.visible && <Form onSubmit={this.handleSave}>
                    <Spin spinning={loading}>
                        <div style={{ paddingBottom: 50 }}>
                                ...
                                <Col span={12} >
                                    {
                                        <FormItem {...formItemLayout} label="报账金额">
                                            {getFieldDecorator('amount', {
                                                ...
                                                <InputNumber step={0.01} onBlur={this.amountBlur} onChange={this.reimburseAmountChange} style={{ width: '100%' }} percision={2} />
                                                )}
                                        </FormItem>
                                    }
                                </Col>
                            ...
                            {this.props.params.visible && <FormItem
                                label="附件"
                                labelCol={{ span: 3 }}
                                wrapperCol={{ span: 20 }}
                            >
                                {
                                    getFieldDecorator("attachmentOID")(
                                        <Upload wrappedComponentRef={(upload) => this.upload = upload} attachmentType="BUDGET_JOURNAL"
                            ...
                            <FormItem labelCol={{ span: 3 }}
                                wrapperCol={{ span: 20 }} label="分摊费用">
                                <div>
                                    {this.state.headerData.relatedApplication && <Button onClick={() => { this.newShareByApplication() }} icon="plus" style={{ marginRight: 10 }}>从申请单新建分摊</Button>}
                                    <Button onClick={() => { this.newShare() }} icon="plus">新建分摊</Button>
                                </div>
                            </FormItem>
                            <NewShare handleOk={this.getShareData} isRefresh={this.state.isRefreshShareTabel} edit={this.editShare} deleteShare={this.deleteShare} params={this.state.shareParams} data={applicationData}></NewShare>
                        </div>
                        <Affix style={{ textAlign: "center" }} offsetBottom={0} className="bottom-bar">
                            ...
                            <Button onClick={this.againSave} loading={this.state.againLoading} style={{ margin: '0 10px' }}>再记一笔</Button>
                            <Button onClick={this.copy} loading={this.state.copyLoading} style={{ margin: '0 10px', marginLeft: 0 }}>复制</Button>
                ...
                <SelectApplication
                    ...
                    single={true}
                    params={{ applincationParams: this.state.applincationParams, show: showSelectApplication, type: expenseType.id }}
