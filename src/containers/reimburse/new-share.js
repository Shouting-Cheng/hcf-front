...
class NewShare extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                {
                    title: "公司", dataIndex: "company", width: 200, render: (value, record, index) => {
                        return (record.status == "edit" || record.status == "new") ? (<Select disabled={record.isCreateByApplication} ref={(ref) => { this["company" + index] = ref }} value={record.company ? record.company.name : ""} onFocus={() => { this.showSelector(index, "company", "select_company_reimburse") }}></Select>) : (
                ...
                {
                    title: "部门", dataIndex: "department", width: 200, render: (value, record, index) => {
                        return (record.status == "edit" || record.status == "new") ? (<Select disabled={record.isCreateByApplication} ref={(ref) => { this["department" + index] = ref }} value={record.department ? record.department.name : ""} onFocus={() => { this.showSelector(index, "department", "select_department_reimburse") }}></Select>) : (
                ...
                {
                    title: "分摊金额", dataIndex: "cost", width: 160, key: "cost", render: (value, record, index) => {
                        return (record.status == "edit" || record.status == "new") ? (
                            <div style={{ textAlign: "right" }}>
                                <InputNumber precision={2} value={value} onChange={(val) => this.costChange(index, val)} />
                            </div>
            ...
            costCenterData: {},
            applicationCol: {
                title: "关联申请单", width: 240, dataIndex: "applicationCode", key: "applicationCode", render: (value, record) => {
                    return <a>{record.application && record.application.businessCode}</a>
                }
            },
            optionCol: {
                title: "操作", dataIndex: "id", key: "id", fixed: "right", width: 120, render: (value, record, index) => {
                    return (
                        (record.status == "edit" || record.status == "new") ?
                            ...
                            (
                                <div>
                                    ...
                                    <Popconfirm placement="top" title={"确认删除？"} onConfirm={() => { this.delete(index) }} okText="确定" cancelText="取消">
