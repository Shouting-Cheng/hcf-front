...
import { Button, Table, Menu, Dropdown, Icon, Row, Col, Badge, Popconfirm, Popover, Input, message, Divider } from 'antd';
...
class MyReimburse extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      searchForm: [
        { type: 'select', id: 'formId', label: '单据类型', getUrl: `${config.baseUrl}/api/custom/forms/company/my/available/all/?formType=105`, options: [], method: "get", valueKey: "formId", labelKey: "formName", colSpan: 6 },
        {
        ...
        {
          type: 'list', listType: "select_authorization_user", options: [], id: 'applicationId', label: "申请人", labelKey: "userName",
          valueKey: "userId", single: true, colSpan: 6
        },
        { type: 'select', id: 'status', label: '状态', options: statusList, colSpan: 6 },
        {
          type: 'select', key: 'currency', id: 'currencyCode', label: "币种", getUrl: `${config.baseUrl}/api/company/standard/currency/getAll`, options: [], method: "get",
        ...
        {
          type: "input", id: "remark", colSpan: 12, label: "备注"
        }
      ],
      columns: [
        {
          /*单号*/
          title: formatMessage({ id: "myReimburse.businessCode" }), dataIndex: 'businessCode', width: 180, align: "left",
          render: recode => (
        ...
        {          /*单据名称*/
          title: "单据类型", dataIndex: 'formName', width: 150, align: "center",
          render: recode => (
            <Popover content={recode}>
              {recode}
            </Popover>)
        ...
        {          /*申请人*/
          title: formatMessage({ id: "myReimburse.applicationName" }), dataIndex: 'applicantName', width: 90, align: "center",
        },
        {          /*提交日期*/
          title: "申请日期", dataIndex: 'reportDate', width: 90, align: "center",
          render: value => (
        ...
        {
          /*币种*/
          title: formatMessage({ id: "myReimburse.currencyCode" }), dataIndex: 'currencyCode', width: 80, align: "center",
          render: recode => (
            <Popover content={recode}>
              {recode}
            </Popover>)
        ...
        {          /*金额*/
          title: formatMessage({ id: "myReimburse.totalAmount" }), dataIndex: 'totalAmount', width: 110, align: "center",
          render: this.filterMoney
        },
        {
          /*本币金额*/
          title: formatMessage({ id: "myReimburse.functionalAmount" }), dataIndex: 'functionalAmount', width: 110, align: "center",
          render: this.filterMoney
        },
        {
          /*事由*/
          title: formatMessage({ id: "myReimburse.remark" }), dataIndex: 'remark', align: "center",
          render: recode => (
            <Popover content={recode}>
              {recode}
            </Popover>)
        ...
        {
          title: '状态', dataIndex: 'status', width: 110, align: "center", render: (value, record) => {
            return (
  ...
  getForms = () => {
    ...
    httpFetch.get(`${config.baseUrl}/api/custom/forms/company/my/available/all/?formType=105`).then(res => {
  ...
  getList = () => {
    ...
    httpFetch.get(`${config.baseUrl}/api/expReportHeader/selectByInput`, params).then(res => {
  ...
  searchNumber = (e) => {
    ...
    this.setState({ searchParams: { ...this.state.searchParams, businessCode: e, allForm: false } }, () => {
      this.refs.table.search(this.state.searchParams);
    });
    ...
    return (
      <div className="reimburse-container">
        <SearchArea searchForm={searchForm}
          submitHandle={this.search} maxLength={4} />
        ...
        <CustomTable onClick={this.handleRowClick} ref="table" columns={columns} url={`${config.baseUrl}/api/expReportHeader/selectByInput`}></CustomTable>
      </div>
    )
  }
}
...
export default connect(mapStateToProps, null, null, { withRef: true })(MyReimburse);
