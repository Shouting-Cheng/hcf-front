...
class ContractDetailCommon extends React.Component {
  ...
  getHistory = () => {
    ...
    contractService
      .getContractHistory(this.state.headerData.documentOid)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            historyLoading: false,
    ...
    let headerInfo = {
      ...
      infoList: [
        ...
        headerData.contractHeaderId
          ? {
              label: '合同',
              value: headerData.contractHeaderLineDTO.contractNumber,
      ...
      <div className="tab-container">
        <Spin spinning={planLoading}>
          <h3 className="sub-header-title">
            {this.$t({ id: 'my.contract.payment.plan' } /*付款计划*/)}
          </h3>
          <div className="table-header">
            <div className="table-header-buttons">
              {contractEdit && (
                <Button type="primary" onClick={this.addItem}>
                  {this.$t({ id: 'common.add' } /*添加*/)}
                </Button>
              )}
            </div>
            <div style={{ marginBottom: '10px' }}>
              {this.$t(
                { id: 'common.total' },
                { total: pagination.total } /*共搜索到 {total} 条数据*/
              )}
              <span className="ant-divider" />
              {this.$t({ id: 'my.contract.amount' } /*合同金额*/)}: {headerData.currency}{' '}
              {this.filterMoney(headerData.amount)}
            </div>
          </div>
          <Table
