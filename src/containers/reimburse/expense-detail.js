...
class NewExpense extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        //显示并且是编辑
            ...
            this.setState({ nowPage: 'form', isCopy: params.isCopy, defaultApportion: params.defaultApportion, headerData: params.headerData, isShowInvoice: params.record.vatInvoice, loading: true, shareParams }, () => {
    ...
    render() {
        ...
        const { nowPage, expenseType, loading, saving, attachments, currencyList, nowCurrency,
        ...
        return (
            <div className="new-expense">
                {this.props.params.visible && <Form onSubmit={this.handleSave}>
                    <div className={`expense-container page-form`}>
                        <Row>
                            <Col span={12}>
                            ...
                            <Col span={12} style={{ padding: "20px 40px" }}>
                                <Spin spinning={loading}>
                                    <div style={{ overflow: "hidden" }} className="expense-type-container">
                                        <Row>
                                            <Col span={12}>
                                            ...
                                            <Col span={12} >
                                                {
                                                    <FormItem {...formItemLayout} label="报账金额">
                                                        {getFieldDecorator('amount', {
                                                            ...
                                                            <InputNumber disabled step={0.01} onBlur={this.amountBlur} onChange={this.reimburseAmountChange} style={{ width: '100%' }} percision={2} />
                                                            )}
                                                    </FormItem>
                                                }
                                            </Col>
                                        ...
                                        <ShareDetail isRefresh={this.state.isRefreshShareTabel} edit={this.editShare} deleteShare={this.deleteShare} params={this.state.shareParams} data={applicationData}></ShareDetail>
                                    </div>
                                </Spin>
                            </Col>
                        </Row>
                    ...
                    <Affix offsetBottom={0}
                        style={{
                            position: 'fixed', bottom: 0, marginLeft: '-15px', width: '100%', height: '50px',
                            boxShadow: '0px -5px 5px rgba(0, 0, 0, 0.067)', background: '#fff', lineHeight: '50px', textAlign: "center"
                        }}>
                        <Button type="primary" htmlType="submit" loading={saveLoading} style={{ margin: '0 10px' }}>保存</Button>
                        <Button onClick={this.againSave} loading={this.state.againLoading} style={{ margin: '0 10px' }}>再记一笔</Button>
                        <Button onClick={this.copy} loading={this.state.copyLoading} style={{ margin: '0 40px', marginLeft: 0 }}>复制</Button>
                ...
                <SelectApplication
                    ...
                    single={true}
                    params={{ applincationParams: this.state.applincationParams, show: showSelectApplication, type: this.state.expenseType.id }}
