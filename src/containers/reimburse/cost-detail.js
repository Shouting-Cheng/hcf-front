...
class CostDetail extends React.Component {
    constructor(props) {
        ...
        this.state = {
            columns: [
                { title: "序号", width: 80, align: "center", dataIndex: "index", key: "index", render: (value, record, index) => { return <span>{index + 1}</span> } },
                { title: "费用类型", width: 100,align: "center", dataIndex: "expenseTypeName", key: "expenseTypeName" },
                { title: "发生日期", width: 120,align: "center", dataIndex: "createTime", key: "createdDate", render: (value) => moment(value).format('YYYY-MM-DD') },
                {
                    title: "金额", width: 100,align: "center", dataIndex: "amount", key: "amount", render: this.filterMoney
                },
                {
                    title: "本位币金额", width: 120,align: "center", dataIndex: "baseAmount", key: "baseAmount", render: this.filterMoney
                },
                {
                    title: "备注", dataIndex: "comment",align: "center", key: "comment", render: (value) => {
                        return (<Popover content={value}>{value}</Popover>)
                    }
                },
                {
                    title: "查看信息", dataIndex: "option", width: 160, align: "center", render: (value, record) => {
                        return (
                            <div>
                                {<a disabled={!record.vatInvoice} onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                ...
                {
                    title: "操作", dataIndex: "id", width: 160, align: "center", render: (value, record) => {
                        return (
                            <div>
                                <a onClick={(e) => {
                                    e.preventDefault();
                                ...
                                <Divider type="vertical"></Divider>
                                <Popconfirm placement="top" title={"确认删除？"} onConfirm={(e) => {
                                    e.preventDefault();
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.headerData.id && !this.state.headerData.id) {
            this.setState({ headerData: nextProps.headerData }, () => {
                let expenseTypeList = [];
                if (this.state.headerData.summaryView && this.state.headerData.summaryView.expenseTypeList) {
    ...
    getList = (typeId = "") => {
        ...
        reimburseService.getCostLineInfo(this.state.headerData.id, this.state.page, typeId).then(res => {
            this.setState({
                data: res.data,
                pagination: {
  ...
  expandedRowRender = (record) => {
    return (
      <Row>
        <Col style={{ textAlign: 'right' }} span={2}><h3>{formatMessage({ id: "my.contract.enclosure.information" })}：</h3></Col>
        <Col span={20}>
          <Row>
            {record.attachments&&record.attachments.map(item => {
              return (
                <Col span={6} style={{textAlign: "left", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} key={item.id}>
    ...
    render() {
        let { loading, data, columns, pagination, headerData, expenseTypeList, previewVisible, previewImage } = this.state;
        return (
            <div>
  ...
  <span style={{ color: "green" }}>{this.props.headerData.currencyCode} {this.filterMoney(this.props.headerData.totalAmount)}</span>
                ...
                <ApprotionInfo close={() => { this.setState({ showApprotion: false }) }} headerData={this.state.headerData} visible={this.state.showApprotion} id={this.state.invoiceId}></ApprotionInfo>
            </div>
        )
    }
}
