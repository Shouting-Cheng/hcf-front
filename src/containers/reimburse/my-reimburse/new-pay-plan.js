...
class NewPayPlan extends React.Component {
  ...
  queryCashTransactionClassForForm = () => {
    reimburseService
      .queryCashTransactionClassForForm(this.state.headerData.formOid)
      .then(res => {
      ...
      <div className="new-pay-plan">
        <Row>
          <Col span={20} offset={2} style={{ marginTop: -20, marginBottom: 20 }}>
            <Alert
              message={`报账总金额: ${headerData.summaryView &&
          ...
          <Form onSubmit={this.handleSave}>
            ...
            <FormItem {...formItemLayout} label="收款方类型">
              {getFieldDecorator('payeeCategory', {
                ...
                initialValue:
                  model.payeeCategory || headerData.payeeCategory == 'EMPLOYEE_VENDER'
                    ? 'EMPLOYEE'
            ...
            <FormItem {...formItemLayout} label="收款方银行账号">
              {getFieldDecorator('accountNumber', {
                rules: [{ message: '请输入', required: true }],
                initialValue: isNew
                  ? headerData.multipleReceivables === false
                    ? headerData.defaultPaymentInfo.accountNumber
                    : ''
                  : model.accountNumber,
              })(
                <Select
            ...
            <FormItem {...formItemLayout} label="收款方户名">
              {getFieldDecorator('accountName', {
                initialValue: isNew
                  ? headerData.multipleReceivables === false
                    ? headerData.defaultPaymentInfo.accountName
                    : ''
                  : model.accountName,
                rules: [{ message: '请输入', required: true }],
              })(<Input disabled />)}
            ...
            <FormItem {...formItemLayout} label="付款方式类型">
              {getFieldDecorator('paymentMethod', {
                initialValue: isNew
                  ? headerData.defaultPaymentInfo
                    ? headerData.defaultPaymentInfo.paymentMethod
                    : ''
                  : model.paymentMethod,
                rules: [{ message: '请输入', required: true }],
              })(
            ...
            >
              {getFieldDecorator('schedulePaymentDate', {
                ...
                initialValue: isNew
                  ...
                  : moment(new Date(model.schedulePaymentDate), 'YYYY-MM-DD'),
