...
class SelectContract extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            expandedRowKeys: [],
            searchForm: [
                { type: "input", label: "申请单编号", id: "contractNumber" }
            ],
            columns: [
                {
                    title: '提交日期', dataIndex: 'submittedDate', render: value => {
                ...
                {
                    title: '相关人员', dataIndex: 'applicationParticipants', render: value => {
                        let text = "";
    ...
    getList() {
        ...
        let url = `${config.baseUrl}/api/application/related/expense?expenseTypeId=${type}&currencyCode=${applincationParams.currencyCode}&expReportHeaderId=${applincationParams.id}&page=${page}&size=${pageSize}`;
                ...
                if (item.apportionmentList) {
                    item.apportionmentList.map(o => {
                        if (o.id == selected) {
                            ...
                            this.setState({ expandedRowKeys: [item.id], selectedData: [{ ...o, businessCode: item.businessCode }] });
    ...
    expandedRowRender = (record, index) => {
        ...
        const columns = [
            {
                title: '序号', dataIndex: 'index', key: 'lineNumber', render: (value, record, index) => {
                    return <span>{index + 1}</span>
                }
            },
            { title: '费用类型', dataIndex: 'expenseTypeName', key: 'expenseTypeName', },
            {
                title: '分摊金额', dataIndex: 'amount', key: 'amount', render: (value, record) => {
                    return (
                        <span>{record.currencyCode} {this.formatMoney(value)}</span>
                    )
                }
            },
            {
                title: '已报账金额', dataIndex: 'usedAmount', key: 'usedAmount', render: (value, record) => {
                    return (
                        <span>{record.currencyCode} {this.formatMoney(value)}</span>
                    )
                }
            ...
            {
                title: '可报账金额', dataIndex: 'usableAmount', key: 'usableAmount', render: (value, record) => {
                    return (
                        <span>{record.currencyCode} {this.formatMoney(value)}</span>
                    )
                }
    ...
    render() {
        ...
        const { data, pagination, loading, columns, selectorItem, selectedData, rowSelection, inputValue, searchForm, expandedRowKeys } = this.state;
        return (
            <Modal title={"选择申请单"} visible={visible} onCancel={onCancel} afterClose={afterClose} width={1000} onOk={this.handleOk} className="list-selector select-contract">
                ...
                <div className="table-header">
                    <div className="table-header-title">
                        ...
                        &nbsp;<span>/</span>&nbsp;
                        {formatMessage({ id: "common.total.selected" }, { total: selectedData.length === 0 ? '0' : selectedData.length })}
                    </div>
                </div>
