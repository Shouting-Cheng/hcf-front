...
import { Button, Table, Badge, Input, Switch, Select, Form, Row, Col, Icon, Cascader, message, notification  } from 'antd';
...
class NewUpdateBankAccount extends React.Component{
      ...
      if (!err) {
        ...
        method.then(response => {
          if (response.data.constraintFlag === 'false') {
            //弹框出提示
            notification.open({
              message: <span>{messages('supplier.vendor')}<span style={{ color: '#2292dd' }}>{response.data.venInfoName}</span>{messages('vendor.notification.left')}
                <span style={{ color: '#2292dd' }}>{response.data.mesBankName}</span>{messages('vendor.notification.bank')}
                <span style={{ color: '#2292dd' }}>{response.data.mesVenBankNumberName}</span>
              </span>,
              description: <span>{messages('vendor.notification.right')}<span style={{ color: '#2292dd' }}>{response.data.mesVenBankNumberName}</span>
              ...
              btn:
                <div>
                  <Button size="small" onClick={() => { this.setState({ loading: false }); notification.close('section') }}>
                    {messages('common.cancel')}
                  </Button>
                  <Button type="primary" style={{ marginLeft: 10 }} size="small" onClick={() => this.handleNotification(response.data)}>
  ...
  handleBankChange = (value)=>{
    if(typeof value[0].id !== 'undefined'){
      ...
      this.props.form.setFieldsValue({country: value[0].countryName,bankAddress: value[0].province+value[0].city})
    }
  };
  ...
  render(){
    ...
    const {loading, enabled, isMainAccount,validateAccount, bank, bankInfo, country, address} = this.state;
      ...
      <div className="new-update-bank-account">
        <Form onSubmit={this.handleSubmit}>
            ...
            <Col offset={4} span={7}>
              <FormItem {...formItemLayout}
                ...
                {getFieldDecorator('enabled', {
                  ...
                  <div>
                    <Switch checked={enabled}  checkedChildren={<Icon type="check"/>} unCheckedChildren={<Icon type="cross" />} onChange={this.statusChange}/>
                    <span className="enabled-type" style={{marginLeft:20,width:100}}>{ enabled ? messages('common.status.enable') : messages('common.disabled') }</span>
                  </div>)}
              </FormItem>
            </Col>
            <Col span={8} offset={1}>
              <FormItem {...formItemLayout}
                        label={messages('supplier.main.account')} colon={true}>
                {getFieldDecorator('primaryFlag', {
                  valuePropName:"checked",
                  initialValue: isMainAccount
                })(
                  <div>
                    <Switch checked={isMainAccount}  checkedChildren={<Icon type="check"/>} unCheckedChildren={<Icon type="cross" />} onChange={this.mainAccountChange}/>
                    <span className="enabled-type" style={{marginLeft:20,width:100}}>{ isMainAccount ? messages('supplier.bank.yes') : messages('supplier.bank.no') }</span>
                  </div>)}
              </FormItem>
            </Col>
          </Row>
          <Row gutter={22}>
            <Col span={22}>
              <FormItem {...formItemLayout}
                        label={messages('bank.bankName')} colon={true}>
          ...
          <Row gutter={22}>
            <Col span={22}>
              <FormItem {...formItemLayout}
                        label={messages('supplier.management.bank.accountName')} colon={true}>
          ...
          <Row gutter={22}>
            <Col span={22}>
              <FormItem {...formItemLayout}
                        ...
                        label={messages('supplier.bank.account')} colon={true}>
                {getFieldDecorator('bankAccount', {
                  ...
                  rules:[{
                    ...
                    {
                      validator:(item,value,callback)=>this.validateAccount(item,value,callback)
                    }
                  ]
                })(
                  <Input placeholder={messages('common.please.enter')}/>
          ...
          <Row gutter={22}>
            <Col span={22}>
              <FormItem {...formItemLayout}
                        label={messages('bank.country')} colon={true}>
                {getFieldDecorator('country', {
                  initialValue: bankInfo.country,
                })
                (<Select disabled allowClear showSearch onChange={this.countryChange} placeholder={messages('common.please.select')}>
          ...
          <Row gutter={22}>
            <Col span={22}>
              <FormItem {...formItemLayout}
                        label={messages('bank.address')} colon={true}>
          ...
          <Row gutter={22}>
            <Col span={22}>
              <FormItem {...formItemLayout}
                        label={messages('supplier.management.remark')} colon={true}>
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedNewUpdateBankAccount);
