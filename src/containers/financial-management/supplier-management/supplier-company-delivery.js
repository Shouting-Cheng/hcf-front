...
class SupplierCompanyDelivery extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      infoList: [
        { type: 'input', id: 'venderTypeName', disabled: true, label: messages('supplier.management.type') }, /*供应商类型*/
        { type: 'input', id: 'venderCode', disabled: true, label: messages('supplier.management.code') }, /*供应商代码*/
        { type: 'input', id: 'venNickname', disabled: true, label: messages('supplier.management.name') }, /*供应商名称*/
        { type: 'switch', id: 'venType', defaultValue: true, label: messages('common.column.status') }, /*状态*/
      ],
      columns: [
        {                         /*公司代码*/
          title: messages('supplier.company.code'), key: "companyCode", dataIndex: 'companyCode',
          render: desc => <span><Popover content={desc}>{desc ? desc : "-"}</Popover></span>
        },
        {                         /*公司名称*/
          title: messages('supplier.company.name'), key: "name", dataIndex: 'name',
          render: desc => <span><Popover content={desc}>{desc ? desc : "-"}</Popover></span>
        },
        {                         /*公司类型*/
          title: messages('supplier.company.type'), key: "companyTypeName", dataIndex: 'companyTypeName',
          render: desc => <span><Popover content={desc}>{desc ? desc : "-"}</Popover></span>
        },
        {                         /*账套*/
          title: messages('supplier.company.setOfBook'), key: "setOfBooksName", dataIndex: 'setOfBooksName'
        },
        {                        /*启用*/
          title: messages('common.status.enable'), key: "enabled", dataIndex: 'enabled', width: '10%',
      ...
      <div className="supplier-company-delivery">
        ...
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            <Button type="primary" onClick={() => this.showListSelector(true)}>{messages('budget.item.batchCompany')}</Button>  {/*批量分配公司*/}
          </div>
        </div>
        <Table
          loading={loading}
