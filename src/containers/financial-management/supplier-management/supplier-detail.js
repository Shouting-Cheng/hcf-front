...
class SupplierDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          /*序号*/ title: messages('supplier.management.index'),
          width: '5%',
          dataIndex: 'id',
        ...
        {
          /*银行开户行*/
          title: messages('supplier.bank.branch'),
          dataIndex: 'bankOpeningBank',
          render: (value, record, index) =>
            this.renderColumns(value, record, 'bankOpeningBank', index),
        },
        {
          /*银行开户行号*/
          title: messages('supplier.bank.branch.number'),
          dataIndex: 'bankOperatorNumber',
          render: (value, record, index) =>
            this.renderColumns(value, record, 'bankOperatorNumber', index),
        },
        {
          /*开户行城市*/
          title: messages('supplier.bank.city'),
          dataIndex: 'bankOpeningCity',
          render: (value, record, index) =>
            this.renderColumns(value, record, 'bankOpeningCity', index),
        },
        {
          /*状态*/
          ...
          render: record =>
            record === 1001 ? (
              <Badge status="success" text={messages('supplier.management.using')} />
            ) : record === 1002 ? (
              <Badge status="error" text={messages('supplier.management.disuse')} />
            ) : (
              <Badge status="processing" text={messages('supplier.management.newAdd')} />
            ),
        },
        {
          /*最近更新时间*/
          title: messages('supplier.management.lastUpdate'),
          key: 'updateTime',
        ...
        },
        {
          ...
          render: (text, record, index) =>
            this.checkPageRole('VENDORMAINTENANCE', 2) && (
              <span>
                {record.editable ? (
                  <a href="#" onClick={e => this.saveItem(e, record, index)}>
                    {messages('supplier.a.save')}
                  </a>
                ) : (
                  <a href="#" onClick={e => this.editItem(record.id, index)}>
                    {messages('common.edit')}
                  </a>
                )}
                <span className="ant-divider" />
                {record.editable ? (
                  <a href="#" onClick={e => this.cancel(record.id, index)}>
                    {messages('supplier.a.cancel')}
                  </a>
                ) : record.venType === 1002 ? (
                  <a onClick={e => this.disableBank(e, record, index)} href="#">
                    {messages('supplier.a.enable')}
                  </a>
                ) : (
                  <a onClick={e => this.disableBank(e, record, index)} href="#">
                    {messages('supplier.a.disable')}
                  </a>
                )}
              </span>
            ),
  ...
  disableBank = (e, record, index) => {
    ...
    record.updateTime = new Date().getTime();
    supplierManagementService
      .disableSupplierBank(record)
      .then(res => {
        if (res.data.code === '0000') {
          record.venType === 1001
            ? (newData[index].venType = 1002)
            : (newData[index].venType = 1001);
        ...
        <Row>
          ...
          <Col span={6}>
            <span className="supplier-title">
              {messages('supplier.management.supplierStatus')}:
            </span>
            <span className="supplier-value">
              {headInfo.venType === 1001
                ? messages('supplier.management.using')
                : messages('supplier.management.disuse')}
            </span>
          </Col>
        </Row>
        <Row>
          <Col span={6}>
            <span className="supplier-title">
              {messages('supplier.management.operatorNumber')}:
            </span>
          ...
          <Col span={12}>
            ...
            <span className="supplier-value">
              {moment(headInfo.updateTime).format('YYYY-MM-DD HH:mm:ss')}
            </span>
          </Col>
        </Row>
        <Row type="flex" align="middle">
          <Col span={8}>
            ...
            <Input
              ...
              value={venNickOid}
              disabled={editFlag}
              onChange={this.onChangeVenNickOid}
            />
          </Col>
          {buttonRoleSwitch && (
            <Col span={6}>
              {editFlag ? (
                <span className="supplier-title">
                  <a onClick={this.openEdit}>{messages('common.edit')}</a>
                </span>
              ) : (
                <span className="supplier-title">
                  <a onClick={this.updateInfo}>{messages('common.save')}</a>
                  <a onClick={this.closeEdit}>{messages('common.cancel')}</a>
                </span>
              )}
            </Col>
          )}
          ...
          <div className="table-header">
            <div className="table-header-buttons">
              <Button type="primary" onClick={this.handleAddRow}>
                {messages('supplier.add.bank.account')}
              </Button>
              <Button onClick={this.disableSupplier}>
                {headInfo.venType === 1001
                  ? messages('supplier.detail.disableSuppier')
                  : messages('supplier.detail.enableSuppier')}
              </Button>
            </div>
          </div>
        )}
