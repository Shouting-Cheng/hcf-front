...
class BankPicker extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      searchForm: [
        {type: 'input', id: 'bankBranchName', label:this.$t('supplier.bank.bankName')}, /*银行名称*/
      ],
      pagination: {
        current: 1,
        page: 0,
      ...
      customBankColumns:[
        {
          /*国家*/
          title: this.$t('bank.country'), key: "countryName", dataIndex: 'countryName', width: '10%',
          render: desc => <span>{desc ? <Popover placement="topLeft" content={desc}>{desc}</Popover> : '-'}</span>
        },
        {
          /*银行代码*/
          width: '15%',
          title: this.$t('bank.bankCode'), key: "bankCode", dataIndex: 'bankCode'
        },
        {
          /*银行名称*/
          title: this.$t('bank.bankName'), key: "bankName", dataIndex: 'bankName',
          render: desc => <span>{desc ? <Popover placement="topLeft" content={desc}>{desc}</Popover> : '-'}</span>
        },
        {
          /*支行名称*/
          title: this.$t('bank.bankBranchName'), key: "bankBranchName", dataIndex: 'bankBranchName',
        },
        {
          /*开户地*/
          title: this.$t('bank.openAccount'), key: "openAccount", dataIndex: 'openAccount'
        },
        {
          /*详细地址*/
          title: this.$t('bank.detailAddress'), key: "detailAddress", dataIndex: 'detailAddress',
        }
      ],
      // 通用银行表格
      commonBankColumns: [
...
export default connect(mapStateToProps, null, null, { withRef: true })(BankPicker);
