...
class CompanyBankSelector extends React.Component {
  ...
  handleOk = () => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        confirmPaymentService.confirmPayment(this.props.data.status, result).then(res => {
  ...
  render() {
    ...
    return (
      <Modal visible={visible} onCancel={onCancel} onOk={this.handleOk} title={messages('payment.batch.company.paySure')} confirmLoading={loading}>
        <Form>
          <FormItem {...formItemLayout} label={messages('payment.batch.search.account')}>
            {getFieldDecorator('paymentBankAccount', {
              rules: [{
                required: true,
                message: messages('common.name.is.required', {name: messages('payment.batch.search.account')})
              }]
            })(
              <Chooser listExtraParams={{setOfBooksId: this.props.company.setOfBooksId,userOID:this.props.user.userOID}}
        ...
        <Form>
          <FormItem {...formItemLayout} label={messages('payment.batch.company.payWay')}>
            {getFieldDecorator('paymentType', {
              rules: [{
                required: true,
                message: messages('common.name.is.required', {name: messages('payment.batch.company.payWay')})
              }]
            })(
            ...
            {getFieldDecorator('paymentChequeNo',
              {
                rules: [{
                  ...
                  message: messages('common.name.is.required', {name: messages('payment.batch.company.paymentChequeNo')})
                }]
              }
            )(
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedCompanyBankSelector);
