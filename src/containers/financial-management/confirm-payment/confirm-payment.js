...
class ConfirmPayment extends React.Component {
    ...
    this.state = {
      searchForm: [
          ...
          items: [
            {
              ...
              defaultValue: moment().subtract(3, 'month'),
            },
            {
              type: 'date',
              id: 'dateTo',
              label: messages('finance.audit.endDate' /*日期到*/),
              defaultValue: moment(),
            },
          ],
        },
        ...
        },
        {
          ...
          valueKey: 'userOID',
          renderOption: option =>
            `${option.employeeID}-${option.fullName}${option.status != 1001 ? '(已离职)' : ''}`,
        },
        {
          type: 'multiple',
          id: 'legalEntity',
      ...
      columns: [
        ...
        {
          title: messages('finance.view.search.documentType' /*单据名称*/),
          dataIndex: 'formName',
          width: '13%',
          render: formName => <Popover content={formName}>{formName}</Popover>,
          sorter: true,
        },
        {
          title: messages('finance.audit.reimbursement' /*报销单号*/),
          dataIndex: 'childBusinessCode',
          width: '15%',
          render: (text, record, index) => {
            let showText =
              (!record.parentBusinessCode && record.businessCode) ||
              (record.parentBusinessCode !== record.childBusinessCode &&
                record.parentBusinessCode + '-' + record.childBusinessCode) ||
              record.childBusinessCode;
            return <Popover content={showText}>{showText}</Popover>;
          },
          sorter: true,
        },
        {
        ...
        {
          title: messages('finance.view.search.totalAmount' /*总金额*/),
        ...
        },
        {
          title: (
            <Popover
              content={messages('finance.audit.payCurrency' /*支付币种*/)}
          ...
          key: 'realCurrencyCode',
          width: '7%',
        },
        {
          title: messages('finance.audit.duePay' /*待支付金额*/),
          dataIndex: 'baseCurrencyRealPaymentAmount',
        ...
        },
        {
          title: (
            <Popover
              content={messages('finance.audit.journalNo' /*凭证编号*/)}
              overlayStyle={{ width: 100 }}
            >
          ...
          render: origDocumentSequence => (
            <Popover content={origDocumentSequence}>{origDocumentSequence}</Popover>
          ),
        },
      ],
      borrowColumns: [
        ...
        {
          title: messages('bookingManagement.businessCode' /*申请单号*/),
  ...
  tableHeaderDeal = () => {
    ...
    columns.filter(item => item.dataIndex === 'baseCurrencyRealPaymentAmount')[0].title =
      this.state.status === 'pay_finished'
        ? messages('finance.audit.finishPay' /*已支付金额*/)
        : messages('finance.audit.duePay' /*待支付金额*/);
    borrowColumns.filter(item => item.dataIndex === 'paymentAmount')[0].title =
      this.state.status === 'pay_finished'
        ? messages('finance.audit.finishPay' /*已支付金额*/)
        : messages('finance.audit.duePay' /*待支付金额*/);
  ...
  getList(firstRequest) {
    ...
    return httpFetch
      .post(
        `${config.baseUrl}/api/${
          this.state.nowType === 'INVOICE' ? 'v2/expense/reports' : 'loan/application'
        }/finance/admin/search?page=${this.state.page}&size=${this.state.pageSize}&sort=${
          this.state.sort
        }`,
        searchParams
      )
  ...
  getCheckboxListForm() {
    ...
    financeViewService.getExpenseTypeList().then(res => {
      ...
      if (this.state.nowType === 'INVOICE')
        checkboxListForm.items = [
          {
            label: messages('finance.audit.formType' /*表单类型*/),
            key: 'form',
            options: expenseForms,
            checked: checkboxListForm.defaultValue,
          },
        ];
  ...
  search = result => {
    ...
    result.dateFromTrans = result.dateFrom
      ? result.dateFrom.format('YYYY-MM-DD 00:00:00')
      : undefined;
  ...
  searchEventHandle = (event, value, valuesTmp) => {
    switch (event) {
      case 'CHANGE_TYPE': {
        if (value === this.state.nowType) return;
        let { checkboxListForm, expenseForms, loanForms } = this.state;
        checkboxListForm.items = [
          {
            label: messages('finance.audit.formType' /*表单类型*/),
            key: 'form',
            options: value === 'INVOICE' ? expenseForms : loanForms,
            checked: [],
          },
        ];
    ...
    if (dropDownSelectedKeys !== '1') {
      if (!selected) {
        ...
        selectedDataItems.map((selected, index) => {
          if (
            record.expenseReportOID
              ? selected.expenseReportOID === record.expenseReportOID
              : selected.applicationOID === record.expenseReportOID
          ) {
  ...
  onSelectAll = (selected, selectedRows, changeRows) => {
    ...
    this.setState({
      selectedDataNum: selectedDataNum,
      selectedDataOids: selectedDataOids,
      selectedDataItems,
    });
  };
  ...
  renderAllExpandedRow = record => {
    ...
    if (record.warningList) {
      ...
      content &&
        result.push(
          this.renderExpandedRow(messages('common.label'), content.substr(0, content.length - 1))
        );
    }
    if (record.printFree) {
      result.push(
        this.renderExpandedRow(messages('common.print.free'), messages('common.print.require'))
      );
    }
    if (result.length > 0) {
      return result;
  ...
  confirmSuccess() {
    notification.open({
      ...
      description: messages(
        this.state.status === 'prending_pay'
          ? 'confirm.payment.paySuccess'
          : 'confirm.payment.confirmPaySuccess',
        { total: this.state.selectedDataNum } /*确认付款成功！*/
      ), // `您有${this.state.selectedDataNum}笔单据确认付款成功:)`,
  ...
  submit = () => {
    confirmPaymentService
      .confirmPayment(this.state.status, this.getSubmitData())
      .then(() => {
      ...
      .catch(e => {
        ...
        if (e.response.data.errorCode) {
          ...
          if (e.name === 'SyntaxError') this.confirmSuccess();
          else
            notification.open({
              message: messages('confirm.payment.payFailure' /*确认付款失败！*/),
  ...
  handleConfirm = () => {
    ...
    if (
      this.state.status === 'prending_pay' &&
      this.checkFunctionProfiles('pay.choice.banknum', true)
    ) {
  ...
  render() {
    ...
    selectedDataItems.map(item => {
      totalObject[item.currencyCode]
        ? (totalObject[item.currencyCode] += parseFloat(
            this.filterMoney(item.totalAmount || item.paymentAmount, 2, true).replace(',', '')
          ))
        : (totalObject[item.currencyCode] = parseFloat(
            this.filterMoney(item.totalAmount || item.paymentAmount, 2, true).replace(',', '')
          ));
      ...
      <div className="confirm-payment">
        ...
        <div className="table-header">
          <div className="table-header-title">
            {messages(
              'common.total1',
              { total: pagination.total } /*共 {pagination.total} 条数据*/
            )}
            {status !== 'pay_finished' && (
              <span>
                /{messages(
                  'common.total.selected',
                  { total: selectedDataNum } /*已选{selectedDataNum}条*/
                )}
              </span>
            )}
            {selectedDataNum > 0 &&
                  ...
                  {Object.keys(totalObject).map(item => {
                    return (
                      <span className="money-style">
                        {item}
                        {this.filterMoney(totalObject[item], 2, true)}&nbsp;&nbsp;
                      </span>
                    );
                  })}
                </span>
            ...
            {status !== 'pay_finished' &&
              buttonRoleSwitch && (
                <Dropdown overlay={this.renderDropDown()} placement="bottomCenter">
                  <Button>
                    {(dropDownSelectedKeys === '1' &&
                      messages('confirm.payment.choosePage' /*全选当页！*/)) ||
                      messages('confirm.payment.chooseAll' /*全选全部！*/)}
                  </Button>
                </Dropdown>
              )}
            {status !== 'pay_finished' &&
              ...
              this.checkFunctionProfiles(
                ['web.financial.approval.sure.pay.disabled'],
                [[false, undefined]]
              ) && (
                <Button
                  type="primary"
                  onClick={this.handleConfirm}
                  disabled={selectedDataNum === 0}
                  loading={confirmLoading}
                >
                  {status === 'pay_in_process'
                    ? messages('confirm.payment.confirmPaid' /*确认已付款！*/)
                    : messages('confirm.payment.confirmPay' /*确认付款！*/)}
                </Button>
              )}
            {status === 'pay_in_process' &&
              buttonRoleSwitch &&
              this.checkFunctionProfiles(
                ['web.financial.approval.import.offer.disabled'],
                [[false, undefined]]
              ) && (
                <Button onClick={() => this.openOfferImport(true)}>
                  {messages('confirm.payment.importOffer' /*导入报盘文件！*/)}
                </Button>
              )}
          </div>
        </div>
        <Table
          columns={(nowType === 'INVOICE' && columns) || borrowColumns}
          dataSource={data}
          ...
          rowClassName={record =>
            record.printFree || record.warningList ? '' : 'finance-payment-reject'
          }
