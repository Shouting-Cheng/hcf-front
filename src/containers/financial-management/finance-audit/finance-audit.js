...
class FinanceAudit extends React.Component {
          ...
          items: [
            {
              ...
              label: messages('finance.view.search.approvalDateFrom'),
            }, //审批日期从
            {
              type: 'date',
              id: 'approvalEndDate',
              label: messages('finance.view.search.approvalDateTo'),
            }, //审批日期至
          ],
        },
        {
          ...
          method: 'get',
          searchKey: 'keyword',
          getParams: { status: 'prending_audit' },
          labelKey: '_self',
          valueKey: '_self',
        },
        {
          ...
          placeholder:
            ...
            messages('finance.view.search.application' /*请输入姓名／工号*/),
          options: [],
  ...
  componentWillMount() {
    ...
    httpFetch
      .post(
        `${config.baseUrl}/config/api/config/hit/EXPENSE_REPORT_PRINT_FREE?tenantId=${
  ...
  analyseData = () => {
    ...
    if ('AUTH_SUCCESS' === expense.body.type) {
      ...
      let url = expenseDetailAudit.url
        .replace(':expenseReportOID', content[0])
  ...
  getForms = () => {
    ...
    Promise.all([
      ...
      httpFetch.get(
        `${config.baseUrl}/api/custom/forms/company/loan/application/all?enabledFlag=2`
      ),
    ]).then(res => {
      let expenseForms = [];
      let loanForms = [];
      ...
      checkboxListForm.items = [
        {
          ...
          key: 'form',
          options: this.state.nowType === 'INVOICE' ? expenseForms : loanForms,
          checked: checkboxListForm.defaultValue,
        },
      ];
      defaultCheckboxListForm.items = [
        {
          label: messages('finance.audit.formType' /*表单类型*/),
          key: 'form',
          options: this.state.nowType === 'INVOICE' ? expenseForms : loanForms,
  ...
  handleData = () => {
    ...
    if (status === 'audit_pass') {
      ...
      removeArryItem(
        invoiceColumns,
        invoiceColumns.filter(item => item.dataIndex === 'origDocumentSequence')[0]
      );
      removeArryItem(
        borrowColumns,
        borrowColumns.filter(item => item.dataIndex === 'origDocumentSequence')[0]
      );
    }
  ...
  search = result => {
    ...
    result.approvalStartDateTrans = result.approvalStartDate
      ? result.approvalStartDate.format('YYYY-MM-DD 00:00:00')
      : undefined;
    result.approvalEndDateTrans = result.approvalEndDate
      ? result.approvalEndDate.format('YYYY-MM-DD 23:59:59')
      : undefined;
    let searchParams = {
  ...
  handleRowClick = record => {
    ...
    let url = '';
    if (nowType === 'INVOICE')
      url = expenseDetailAudit.url
        .replace(':expenseReportOID', record.expenseReportOID)
        .replace(':backType', 'history');
    else
      url = loanDetailAudit.url
      ...
      <div className="finance-audit">
        {this.checkFunctionProfiles(['finance.scan.disabled'], [[false, undefined]]) &&
          this.checkPageRole('EXPENSEAUDIT', 2) && (
            ...
            >
              {messages('finance.audit.sweepQRcode' /*扫码审核*/)}
            </Button>
          )}
        {this.checkFunctionProfiles('document.review.enable', [true]) &&
          this.checkPageRole('EXPENSEAUDIT', 2) && (
            <Button type="primary" icon="qrcode" onClick={this.handleClickSwitchGun}>
              {messages('finance.audit.sweepGun' /*扫码枪审核*/)}
            </Button>
          )}
        ...
        <Table
          columns={nowType === 'INVOICE' ? invoiceColumns : borrowColumns}
          dataSource={data}
          bordered
          ...
          rowClassName={record =>
            record.printFree || record.noticeFlag || record.warningList
              ? ''
