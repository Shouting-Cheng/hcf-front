...
class AuditApplicationDetail extends React.Component{
  ...
  //返回
  goBack = () => {
    this.context.router.push(this.state.financeAuditList.url + `?tab=${location.search.indexOf('prending_audit') > -1 ? 'prending_audit' : 'audit_pass'}`)
  };
  ...
  handleApprovePass = (value) => {
    ...
    financeAuditService.auditPass(params).then(res => {
      ...
      this.setState({ passLoading: false });
      e.response.data.message ? message.error(`${this.$t('common.operate.filed')}，${e.response.data.message}`) : message.error(this.$t("common.operate.filed")/*操作失败*/);
    })
  };
  ...
  render() {
    const { passLoading, rejectLoading, noticeLoading, printLoading, financeAuditList, invoiceNumber, checkLoading, modalShow, paperReject, paperLoading } = this.state;
    const {profile,status} = this.props;
    let moreButtons = [];
    moreButtons.push('noticeBtn');
    invoiceNumber > 0 && moreButtons.push('auditCheck');
    return status === 1003 &&  location.search.indexOf('prending_audit') > -1 && !(profile['er.disabled'] && 1002 === this.props.entityType) ? (
      <Affix offsetBottom={0} className="bottom-bar bottom-bar-approve">
        <ApproveBar backUrl={financeAuditList.url + '?tab=prending_audit'}
                    ...
                    audit/>
        <Modal onOk={this.handleApprovePass}
               ...
               closable={false}>
          <div>
            <Icon type="exclamation-circle" style={{color:"#F7C243",fontSize:"30",float:"left"}}/><span style={{lineHeight:"30px",fontSize:"20",marginLeft:"10px"}}>{this.$t('expense.date.fail.info')/*有查验失败的发票信息*/}</span>
            <div style={{marginLeft:"40px",fontSize:"16px",color:"red",lineHeight:"30px"}}>{this.$t('expense.date.still.approved')/*是否仍然审核通过*/}</div>
          </div>
        </Modal>
      </Affix>
    ) : (
      <Affix offsetBottom={0} className="bottom-bar">
        <Button type="primary" className="back-btn" onClick={this.goBack}>{this.$t('common.back')/*返回*/}</Button>
      </Affix>
    )
  }
}
...
export default connect(mapStateToProps, null, null, { withRef: true })(AuditApplicationDetail)
