  ...
  analyseData = () => {
    ...
    if (expense.body.type === 'AUTH_SUCCESS'){
      ...
      this.setState({ entityList }, () => {
        message.success(`${entityList[0].name}${messages('finance.audit.expense.report.passed')/*的报销单已通过*/}`)
      })
  ...
  render(){
    ...
    return (
      <div className="scan-audit background-transparent">
        <Button type="primary" icon="layout" onClick={this.handleClickSwitch}>{messages('finance.audit.normal.audit')/*普通审核*/}</Button>
        <div className="reject-audit">
          {rejectEntity.businessCode ? (
            <div className="reject-audit-area">
              ...
              <Form onSubmit={this.handleReject}>
                ...
                <FormItem label={messages('finance.audit.quick.reply')/*快捷回复*/}>
                  { quickReply.length > 0 ? (
                    quickReply.map(item => <Tag color="red" key={item.id} onClick={() => this.handleClickReply(item.reply)}>{item.reply}</Tag>)
                  ) : <div className="no-reply">{messages('finance.audit.no.quick.reply.info')/*尚未设置快捷回复，请联系管理员在值列表中添加*/}</div> }
                </FormItem>
            ...
            <div className="scan-alert">
              ...
              <h3>{messages('finance.audit.scan.audit.info')/*请打开汇联易app，扫描报销单上的二维码*/}</h3>
            </div>
          )}
        </div>
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedScanAudit);
