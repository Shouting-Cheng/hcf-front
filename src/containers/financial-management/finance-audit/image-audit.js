...
class ImageAudit extends React.Component{
  ...
  handleCheckAttachment = (checked, invoiceIndex, attachmentIndex) => {
    ...
    financeAuditService.checkAttachment({
      ...
      currentInvoices && currentInvoices.map(item=>{
        if(item.invoiceOID === invoices[invoiceIndex].invoiceOID){
          let target=item.invoiceView;
          target.attachments && target.attachments.map(i=>{
            if(i.attachmentOID === invoices[invoiceIndex].attachments[attachmentIndex].attachmentOID){
              i.checked =checked;
            }
          })
        }
            ...
            <div className="invoice-container">
              {invoices.map((invoice, index) => (
                <Row key={invoice.invoiceOID} className={`invoice-item ${nowInvoiceIndex === index && 'selected-invoice'}`} onClick={() => this.handleSelectInvoice(index)}>
                  ...
                  <Col span={4}><Popover content={this.filterMoney(invoice.amount)}>{this.filterMoney(invoice.amount)}</Popover></Col>
                </Row>
              ))}
            </div>
          </Col>
          <Col span={16} className="invoice-image">
            <Icon type="left" className="arrow arrow-left"
                  onClick={() => this.handleSelectAttachment(nowAttachmentIndex === 0 ? (attachments.length - 1) : (nowAttachmentIndex - 1))}/>
            <Icon type="right" className="arrow arrow-right"
                  onClick={() => this.handleSelectAttachment(nowAttachmentIndex + 1 === attachments.length ? 0 : (nowAttachmentIndex + 1))}/>/>
            {nowInvoice && (
              <div>
                <div className="invoice-title">
                  ...
                  {nowInvoice.invoiceCurrencyCode}{nowInvoice.amount.toFixed(2)}
                </div>
                <div className="attachment-area">
                  <img src={attachment.fileURL} ref="imageRef"
                       draggable={false}
                ...
                <div className="attachment-list">
                  <div className="attachment-list-box">
                    {attachments.map((file, index) => (
                      <Badge status={isEnableCheck?(file.checked ? 'success' : 'error'):''} key={file.id} count={0} dot={isEnableCheck} offset={[0, -10]} >
                        <img src={file.thumbnailUrl?file.thumbnailUrl:file.fileURL}
...
export default connect(mapStateToProps, null, null, { withRef: true })(ImageAudit)
