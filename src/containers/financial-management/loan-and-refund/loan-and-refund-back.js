...
class LoanAndRefundBack extends React.Component{
  ...
  goBack = () => {
    if (this.props.tab) {
      ...
      this.context.router.push(this.state.overallList.url.replace(':employeeId', this.props.info.applicant.employeeID).replace(':currencyCode', this.props.info.originCurrencyCode))
    }
  };
  ...
  render() {
    return (
      <div className="loan-and-refund-back">
        <Button type="primary" onClick={this.goBack}>{this.$t('common.back')}</Button>
      </div>
    )
  }
}
