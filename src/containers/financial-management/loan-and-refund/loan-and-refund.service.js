...
export default {
  ...
  getLoanInfoByEmployeeId(employeeId,currencyCode, repaymentStatus = 0) {
    return httpFetch.get(`${config.baseUrl}/api/global/list?employeeId=${employeeId}&currencyCode=${currencyCode}&repaymentStatus=${repaymentStatus}&page=0&size=1`)
  },

  //根据employeeId查询员工借款列表
  getLoanListByEmployeeId(page, size, employeeId, searchParams) {
    let url = `${config.baseUrl}/api/global/loan/list?employeeId=${employeeId}&page=${page}&size=${size}`;
    for(let searchName in searchParams) {
      url += searchParams[searchName] ? `&${searchName}=${searchParams[searchName]}` : ''
    }
    return httpFetch.get(url)
