...
class ReportList extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      searchForm: [
        // 报销单ID
        {type: 'input', id: 'businessCode', label: this.$t("train.business.code")},
        // 报销单状态
        {
          type: 'value_list',
          label: this.$t('train.report.status'),
          id: 'reportStatus',
          options: [],
          valueListCode: 2028
        }
      ...
      */
      ...
      columns: [
        ...
        {title: this.$t('train.report.id'), dataIndex: 'id', width: '20%', align: 'center'},
        {title: this.$t('train.business.code'), dataIndex: 'businessCode', width: '15%', align: 'center'},
        {
          title: this.$t('train.application.name'),
          dataIndex: 'applicationName',
          width: '15%',
          align: 'center'
        },
        {
          title: this.$t('train.total.amount'),
          dataIndex: 'totalAmount',
          width: '10%',
          className: 'rightClass'
        },
        {
          title: this.$t('train.remark'),
          dataIndex: 'remark',
          width: '25%',
          align: 'center',
          render: remark => (
        ...
        {
          title: this.$t('train.report.date'),
            ...
            <div>
              <span>
                ...
                <Popconfirm onConfirm={e => this.deleteItem(e, record)}
                            title={this.$t('common.confirm.delete')}>{/* 你确定要删除organizationName吗 */}
                  <a href="#">{this.$t('common.delete')}</a>
                </Popconfirm>
              </span>
            </div>)
        }
  ...
  editItem = (e, record) => {
    ...
    this.props.dispatch(
      routerRedux.push({
          pathname: `/train-report/tra-report-maintain/${record.id}`,
      })
  );
  }
  ...
  handleCreateClick = (e, value) => {
    this.props.dispatch(
      routerRedux.push({
          pathname: `/train-report/tra-report-maintain/create`,
      })
  );
  }
    ...
    return (
      <div className="train">
        ...
        <div className="table-header">
          <div
            className="table-header-title">{this.$t('common.total', {total: pagination.total ? pagination.total : '0'})}</div>
          {/* 共total条数据 */}
          <div className="table-header-buttons">
            <Button
              type="primary"
              onClick={this.handleCreateClick}>{this.$t('common.create')}</Button> {/* 新建 */}
            <Popconfirm onConfirm={e => this.batchDeleteItem(e)} title={this.$t('common.confirm.delete')}>
 ...
 const WrappedReportList = ReportList;
