        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                if (!this.props.params.id) {
                    ...
                    SupplierTypeService.addSupplierType(toValue).then((res) => {
                    ...
                    SupplierTypeService.updateSupplierType(toValue).then((res) => {
                      if(res.data.code === "0000"){
                        this.setState({ loading: false });
                        this.props.form.resetFields();
                        this.props.onClose(true);
                ...
                <Form onSubmit={this.handleSave} onChange={this.handleFormChange}>
                    <FormItem {...formItemLayout} label={this.$t('supplier.type.code1')}>
                        {getFieldDecorator('vendorTypeCode', {
                            ...
                            <Input disabled={typeof this.props.params.id !== 'undefined' ? true : false} placeholder={this.$t('common.please.enter')} />
                            )}
                    </FormItem>
                    ...
                    <FormItem {...formItemLayout} label={this.$t('common.column.status')}>
                        {getFieldDecorator('enabled', {
                            ...
                            <Switch checked={this.props.params.id ? this.props.form.getFieldValue('enabled') : true} checkedChildren={<Icon type="check" />} unCheckedChildren={<Icon type="cross" />} />
                            )}&nbsp;&nbsp;&nbsp;&nbsp;{this.props.form.getFieldValue('enabled') ? this.$t('common.status.enable') : this.$t('common.status.disable')}
                    </FormItem>

                    <div className="slide-footer">
                        <Button type="primary" htmlType="submit"
                            loading={this.state.loading}>{this.$t('common.save')}</Button>
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedNewSupplierType);
