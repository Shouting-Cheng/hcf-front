...
class CustomExpenseType extends React.Component {
    ...
    return (
      <div style={{ padding: 20 }}>
        <Button type="primary" onClick={this.handleNewExpenseType}>
          {messages('expense.type.new.expense.type') /*新增费用类型*/}
        </Button>
          ...
          return (
            ...
            >
              <div className="expense-type-category-title">
                {expenseTypeCategory.name}&nbsp;({expenseTypeCategory.expenseTypes
                  ? expenseTypeCategory.expenseTypes.length
                  : 0})
                {typeSorting && typeSortingIndex === index ? (
                  ...
                  tenantMode && (
                    <div className="expense-type-category-operate">
                      ...
                      <Button
                        onClick={() =>
                          this.setState({ typeSorting: true, typeSortingIndex: index })
                        }
                      >
                        {messages('expense.type.sort')}
                      </Button>
                    </div>
                  )
                )}
  ...
  renderApplicationTypePanel = () => {
    ...
    return (
      <div style={{ padding: 20 }}>
        <Button type="primary" onClick={this.handleNewApplicationType}>
          {messages('新增申请类型')}
        </Button>
