...
class DepartmentGroupDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      infoList: [
        {
          ...
          isRequired: true,
          disabled: true,
          label: this.$t({ id: 'setting.deptGroup.code' }) + ' :',
        },
        {
          type: 'input',
          id: 'description',
          isRequired: true,
          disabled: true,
          label: this.$t({ id: 'setting.deptGroup.name' }) + ' :',
        },
        {
          type: 'switch',
          id: 'enabled',
        ...
        searchForm: [
          {
            type: 'input',
            id: 'deptGroupCode',
            label: this.$t({ id: 'setting.deptCode' }),
            defaultValue: '',
          },
          {
            type: 'input',
            id: 'description',
            label: this.$t({ id: 'setting.deptName' }),
            defaultValue: '',
          },
        ],
        columns: [
      ...
      columns: [
        {
          title: this.$t({ id: 'setting.deptCode' }),
          key: 'custDeptNumber',
  ...
  deleteItem = (e, record) => {
    ...
    typeof record === 'undefined'
      ? (param = this.state.selectedEntityOIDs)
      : param.push(record.departmentDetailId);
    deptGroupService
      .deleteDeptGroupById(param)
      .then(response => {
  ...
  handleListOk = result => {
    ...
    result.result.map(item => {
      param.push({
        departmentGroupId: this.props.match.params.id,
  ...
  onSelectRow = (record, selected) => {
    ...
    if (selected) temp.push(record.departmentDetailId);
    else temp.delete(record.departmentDetailId);
    this.setState({
  ...
  refreshRowSelection() {
    ...
    this.state.selectedEntityOIDs.map(selectedEntityOID => {
      this.state.data.map((item, index) => {
        if (item.departmentDetailId === selectedEntityOID) selectedRowKeys.push(index);
      });
    });
    this.setState({ selectedRowKeys });
  }
      ...
      <div className="budget-item-detail">
        ...
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            ...
            {/*添加公司*/}
            <Button disabled={batchDelete} onClick={this.deleteItem}>
              {this.$t({ id: 'common.delete' })}
            </Button>
          </div>
        </div>
