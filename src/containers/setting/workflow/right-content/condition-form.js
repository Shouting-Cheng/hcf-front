...
import { Form, Button, Icon, Select, InputNumber, DatePicker, Input, message, Row, Col, Modal } from 'antd'
...
import workflowService from 'widget/setting/workflow/workflow.service'
...
class ConditionForm extends React.Component {
  ...
  getDefaultValues = (ruleItem) => {
    ...
    this.setState({ condition, depLevelValue, depRoleValue, expenseTypeValue, companyValue, entityValue, costCenterValue, currencyValue })
  };
  ...
  renderSymbolsType = () => {
    ...
    let lessThanAndEqualRuleList = [{name: this.$t('workflow.detail.symbol.none'/*无*/), key: '0'}];
    let booleanRuleList = [];
  ...
  handleSave = () => {
    ...
    condition.map(item => {
      ...
      if (saveAble && type === 'custList' && item.remark !== 'cust_list' &&
        ...
        message.error(this.$t('workflow.detail.tip.select.value', {name: item.name}/*请选择{name}的条件值*/));
        saveAble = false
      }
      ...
      if (saveAble && type === 'text' && !item.value) {
        message.error(this.$t('workflow.detail.tip.input.value', {name: item.name}/*请输入{name}的条件值*/));
        saveAble = false
      }
      if (saveAble && (type === 'long' || type === 'double' || type === 'date')) {
        if (!item.valueDetail) {
          message.error(this.$t('workflow.detail.tip.select.symbol.and.value', {name: item.name}/*请选择{name}的条件符，条件值*/));
          saveAble = false
        } else {
          ...
          list.map((listItem, listIndex) => {
            ...
            if (saveAble && (listItem.value || listItem.value === 0) && !listItemSymbol) {
              message.error(this.$t('workflow.detail.tip.input.index.symbol',
                {name: item.name, index: this.props.language.local === 'zh_CN' ? listIndex + 1 : (listIndex ? '2nd' : '1st')}/*请输入{name}的第{index}个条件符*/));
              saveAble = false
            }
            if (saveAble && !(listItem.value || listItem.value === 0) && listItemSymbol) {
              message.error(this.$t('workflow.detail.tip.input.index.value',
                {name: item.name, index: this.props.language.local === 'zh_CN' ? listIndex + 1 : (listIndex ? '2nd' : '1st')}/*请输入{name}的第{index}个条件值*/));
              saveAble = false
            }
          });
          ...
          if (type === 'date') {
            if (new Date(leftValue).getTime() > new Date(rightValue).getTime()) {
              message.error(this.$t('workflow.detail.tip.input.date.error', {name: item.name})/*请输入合法的{name}条件值，第一个时间不能比第二个时间晚*/);
              saveAble = false
            }
          } else {
            if (saveAble && leftValue && rightValue && (leftValue > rightValue || (leftValue === rightValue && (leftSymbol === 9001 || rightSymbol === 9003)))) {
              message.error(this.$t('workflow.detail.tip.input.value.error', {name: item.name})/*请输入合法的{name}条件值，表达式不合法*/);
              saveAble = false
            }
          }
        }
  ...
  render() {
    ...
    const { loading, condition, containRuleList, inRuleList, moreThanRuleList, lessThanAndEqualRuleList, booleanRuleList,
            ...
            entityValue, currencyValue, costCenterValue } = this.state;
    return (
      <div className='condition-form'>
        ...
        <div className="form-container">
                  ...
                  <Row className="type-boolean" key={index}>
                    <Col span={2}>
                      {item.remark === 'judge_cost_center' ? (
                        <span>{this.$t('workflow.detail.applicant')/*申请人*/} = {item.name}{this.$t('workflow.detail.manager')/*经理*/}</span>
                      ) : (
                        <span>{item.name}</span>
                      )}
                    </Col>
                  ...
                  <Row className="type-cust-list" key={index}>
                    ...
                    <Col span={language.local === 'zh_CN' ? 3 : 4}>
                      <Select size="small"
                        ...
                        {(item.remark === 'default_department_path' ? containRuleList : inRuleList).map(symbolsItem => {
                          return <Option key={symbolsItem.key}>{symbolsItem.name}</Option>
                        })}
                      </Select>
                    </Col>
                    <Col span={item.remark === 'cust_list' ? 4 : 17}>
                      ...
                      {String(item.symbol) === '9015' || String(item.symbol) === '9016'? '' : (
                        (item.remark === 'default_department_path' || item.remark === 'select_department') ? (
          ...
          <a onClick={this.props.addCondition}><Icon type="plus-circle-o" className="add-icon"/>{this.$t('workflow.add.approve.condition')/*添加审批条件*/}</a>
        </div>
        ...
        />
        <Modal title={this.$t('workflow.select.expense.type')/*选择费用类型*/}
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedConditionForm)
