...
class CustomApproveNode extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      customWidget: [
        {
          type: 1001,
          name: messages('workflow.detail.approval'/*审批*/),
          remark: messages('workflow.detail.approval'/*审批*/),
          code: null,
          level: 1,
        ...
        {
          type: 1003,
          name: messages('workflow.detail.node.robot'/*机器人*/),
          remark: messages('workflow.detail.node.robot'/*机器人*/),
          code: null,
          level: 1,
          ruleApprovalChainOID: this.props.ruleApprovalChainOID,
          ruleApprovalNodeOID: null,
        ...
        {
          type: 1002,
          name: messages('workflow.detail.node.inform'/*知会*/),
          remark: messages('workflow.detail.node.inform'/*知会*/),
          code: null,
          level: 1,
          ruleApprovalChainOID: this.props.ruleApprovalChainOID,
          ruleApprovalNodeOID: null,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    //只有报销单和借款单显示打印节点
    if (nextProps.formInfo && (String(nextProps.formInfo.formType).charAt(0) === '3' || nextProps.formInfo.formType === 2005)) {
      let customWidget = this.state.customWidget;
      let hasPrint = false;
      customWidget.map(item => {
        item.type === 1004 && (hasPrint = true);
      });
      !hasPrint && customWidget.push({
        type: 1004,
        name: messages('workflow.detail.node.print'/*打印*/),
        remark: messages('workflow.detail.node.print'/*打印*/),
        code: null,
        level: 1,
        ruleApprovalChainOID: this.props.ruleApprovalChainOID,
        ruleApprovalNodeOID: null,
  ...
  handleDelete = (hasDelete) => {
    ...
    hasDelete && workflowService.getApprovalChainDetail(this.props.formOID).then(res => {
      ...
      this.setState({
        ...
        nowWidget.length ? this.props.onSelect(nowWidget[0].type, nowWidget[0], 0) :
  ...
  render() {
    const { loading, customWidget, nowWidget, nowSelectedIndex, endNodeSelected, endNodeWidget } = this.state;
    ...
    return (
      <div className="custom-approve-node">
        ...
        <Spin spinning={loading}>
          <NodeContent widgetList={widgetList}
...
CustomApproveNode.propTypes = {
  ...
  ruleApprovalChainOID: React.PropTypes.string,
  formInfo: React.PropTypes.object,
  formOID: React.PropTypes.string,
  isRuleInEdit: React.PropTypes.bool, //是否有审批条件处于编辑状态
...
CustomApproveNode.contextTypes = {
...
export default connect(mapStateToProps, null, null, { withRef: true })(DragDropContext(HTML5Backend)(wrappedWorkflow))
