...
import {Form, Icon, Row, Col, Button, Spin, Checkbox, message, Popover, Tooltip, Affix} from 'antd'
...
class WorkflowDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    workflowService.getFormFields(this.props.params.formOid).then(res => {
  ...
  //返回
  goBack = () => {
    let url = '/admin-setting/form-list';
    this.props.dispatch(
      routerRedux.replace({
        pathname: url
      })
