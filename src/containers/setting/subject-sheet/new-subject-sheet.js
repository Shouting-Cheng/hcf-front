...
class NewSubjectSheet extends React.Component {
    ...
    newAccount = account => {
        ...
        SSService.newAccount(account).then(res => {
            ...
            message.success(
                this.$t({ id: 'common.create.success' }, { name: this.$t({ id: 'subject.sheet' }) })
            );
        });
    ...
    editAccount = account => {
        this.setState({ loading: true });
        SSService.editAccount(account).then(res => {
            this.setState({
                loading: false,
            });
            this.props.form.resetFields();
        ...
        return (
            <div className="new-payment-method">
                <Form onSubmit={this.handleSave} onChange={this.formChange}>
                    <FormItem
                        {...formItemLayout}
                        label={this.$t({ id: 'subject.sheet.code' }) /*科目表代码*/}
                    >
                ...
                <Row style={{ marginBottom: 20, lineHeight: "39px" }}>
                    <Col span={6} offset={1} style={{ textAlign: 'right' }}>
                        <span style={{ color: 'red' }}>&nbsp;*</span>
                        {/*科目表名称*/}&nbsp;{this.$t({ id: 'subject.sheet.describe' })}：
          </Col>
                    <Col span={14} offset={1}>
                        <LanguageInput
                            ...
                            i18nName={
                                newSheet.i18n && newSheet.i18n.accountSetDesc
                                    ? [...newSheet.i18n.accountSetDesc]
                                    : null
                            }
