...
class CodingRuleValue extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      infoList: [
        {
          type: 'input',
          label: messages('code.rule.code') /*编码规则代码*/,
          id: 'codingRuleCode',
          disabled: true,
        },
        {
          type: 'input',
          label: messages('code.rule.name') /*编码规则名称*/,
          id: 'codingRuleName',
          isRequired: true,
        },
        {
          type: 'value_list',
          label: messages('code.rule.reset.frequency') /*重置频率*/,
          id: 'resetFrequence',
          options: [],
          valueListCode: 2024,
        },
  ...
  deleteItem = (e, record) => {
    ...
    codingRuleService
      .deleteCodingRuleObject(record.id)
      .then(res => {
        this.getList();
  ...
  getList() {
    ...
    return codingRuleService
      .getCodingRuleValueList(this.state.page, this.state.pageSize, this.props.match.params.ruleId)
      .then(response => {
        response.data.map(item => {
          ...
          switch (item.segmentType) {
            ...
            case '20':
              ...
              break;
            case '50':
              item.value = `${messages('code.rule.value.digit') /*位数*/}：${item.length}、  ${
                messages('code.rule.value.step.length') /*步长*/
              }：${item.incremental}、  ${messages('code.rule.value.start.value') /*开始值*/}：${
                item.startValue
              }`;
              break;
            default:
  ...
  updateInfo = params => {
    ...
    codingRuleService
      .updateCodingRule(Object.assign({}, this.state.infoData, params))
      .then(res => {
        codingRuleService.getCodingRule(this.props.match.params.ruleId).then(res => {
          res.data.resetFrequence = {
            label: res.data.resetFrequenceName,
      ...
      <div>
        ...
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            <Row gutter={20}>
              <Col span={4}>
                <Button type="primary" style={{ height: 39 }} onClick={this.handleNew}>
                  {messages('code.rule.value.new.rules.and.details') /*新建规则明细*/}
                </Button>
              </Col>
              <Col span={20}>
                <Alert
                  message={
                    messages(
                      'code.rule.value.segment.tips'
                    ) /*段值必须维护序列号，否则编码无法生成。不同规则的日期格式尽量相同。*/
                  }
                  type="info"
                  showIcon
                />
        ...
        >
          <NewCodingRuleValue
            ...
            params={{
              codingRuleId: this.props.match.params.ruleId,
              nowCodingRuleValue,
              nowSequence,
            }}
          />
