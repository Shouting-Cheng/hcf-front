...
class ValueList extends React.Component {
  ...
  componentDidMount() {
    ...
    } else if (!this.props.params.record) { //新建
      this.setState({
  ...
  getList = () => {
    ...
    valueListService.getEmployeeList(page, pageSize, customEnumerationItemOID)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            data: res.data,
  ...
  deleteEmployees = () => {
    ...
    valueListService.deleteEmployees(params).then(res => {
      if (res.status === 200) {
  ...
  render() {
    ...
    const {
      loading, tableLoading, deleteLoading, columns, data, common,
      pagination, record, deleteUserOIDs, showListSelector, customEnumerationItemOID, systemInit
    } = this.state;
                    ...
                    <div className="table-header-buttons">
                      <div className="f-left">
                        <SelectDepOrPerson
                          ...
                          title={messages('value.list.add.employee.by.organization'/*按组织添加员工*/)}
                          onlyPerson={true}
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedValueList);
