...
class CurrencySettingEdit extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          title: this.$t('cost.center.detail.no') /*编号*/,
          dataIndex: 'index',
        ...
        {
          title: this.$t('currency.setting.add.foreign.currency') /*外币*/,
        ...
        {
          title: this.$t('currency.setting.add.edit.time') /*修改时间*/,
        ...
        {
          title: this.props.tenantMode ? '操作' : '',
          dataIndex: 'operation',
          width: this.props.language.local === 'zh_cn' ? '7%' : '10%',
  ...
  getRateHistory = () => {
    ...
    httpFetch
      .get(
        `${config.baseUrl}/api/currency/rate/history/list?currencyRateOid=${
          record.currencyRateOid
        }&language=${language}&page=${page}&size=${pageSize}`
      )
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  onEnableRate = e => {
    ...
    if (e) {
      Modal.confirm({
        ...
        content: this.$t(
          'currency.setting.add.enable.influence'
        ) /*启用该币种会影响全公司员工报销*/,
        ...
        onOk: () => {
          httpFetch
            .put(
              `${config.baseUrl}/api/currency/status/enable?currencyRateOid=${
                record.currencyRateOid
              }&enable=${e}`
            )
            .then(res => {
      ...
      Modal.confirm({
        ...
        content: this.$t(
          'currency.setting.add.forbid.influence'
        ) /*禁用该币种会影响全公司员工报销*/,
        okText: this.$t('currency.setting.add.continue.update') /*继续更新*/,
        cancelText: this.$t('currency.setting.add.cancel.update') /*取消更新*/,
        onOk: () => {
  ...
  onOutEditRateSave = () => {
    ...
    if (typeof record.rate !== 'number' || record.rate <= 0) {
      ...
      if (!dateChanged) {
        record.applyDate = moment(new Date())
          .utc()
          .format();
        this.setState(
          {
          ...
          () => {
            if (
              moment(record.applyDate).format('YYYY-MM-DD') ===
                moment(originalRecord.applyDate).format('YYYY-MM-DD') &&
              record.rate === originalRecord.rate
            ) {
  ...
  onEnableAutoUpdate = e => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        httpFetch
          .put(
            `${config.baseUrl}/api/currency/status/enable/auto/update?language=${
              this.props.language.local
            }&tenantId=${this.props.company.tenantId}&setOfBooksId=${
              this.props.match.params.setOfBooksId
            }&enableAutoUpdate=${e.target.checked}&currencyCode=${record.currencyCode}`
          )
          .then(res => {
            if (res.data) {
  ...
  onSearchHistoryRate = () => {
    ...
    httpFetch.get(`${config.baseUrl}/api/currency/rate/history/list`, params).then(res => {
      if (res.status === 200) {
        this.setState({
          page: 0,
          data: res.data.rows,
        ...
        <div className="currency-edit-notBase-content">
          <Row gutter={16} style={{ marginBottom: 20 }}>
            ...
            <Col span={8}>
              ...
              {
                <Checkbox
                  checked={record.enableAutoUpdate}
                  onChange={this.onEnableAutoUpdate}
                  disabled={
                    !this.hasAnyAuthorities(['ROLE_TENANT_ADMIN']) ||
                    !enableAutoUpdate ||
                    outEditRate ||
          ...
          {outEditRate ? (
            <Row gutter={16}>
              ...
              <Col span={8}>
                {this.$t('currency.setting.effective.rate') /*生效汇率：*/}&nbsp;&nbsp;
                <InputNumber
                  value={record.rate}
            ...
            <Row gutter={16}>
              <Col span={8}>
                {this.$t('currency.setting.rate.apply.date') /*汇率生效日期：*/}&nbsp;&nbsp;
                {moment(record.applyDate)
                  .local()
                  .format('YYYY-MM-DD')}
              </Col>
              <Col span={8}>
                {this.$t('currency.setting.effective.rate') /*生效汇率：*/}&nbsp;&nbsp;{record.rate}
              </Col>
            </Row>
          )}
          <Divider />
          ...
          {outEditRate && (
            <span>
              ...
              <Button onClick={this.onOutEditRateSave} style={{ marginLeft: 20 }} type="primary">
          ...
          <div className="currency-edit-block" style={{ marginTop: 20 }}>
            ...
            <div className="currency-edit-history-right" style={{ float: 'right' }}>
              ...
              <Button type="primary" onClick={this.onSearchHistoryRate}>
          ...
          <div>
            <Modal
              ...
              footer={
                <div>
                  <Button style={{ marginRight: 8 }} onClick={this.handleModalCancel}>
                    {this.$t('common.cancel')}
                  </Button>
                  <Button
                    type="primary"
                    onClick={this.handleModalOK}
                    disabled={operatedRecord.rate === originalRate}
                  >
                    {this.$t('common.ok')}
                  </Button>
                </div>
            ...
            >
              <div>
                {this.$t('currency.setting.rate.apply.date') /*汇率生效日期：*/}
                &nbsp;&nbsp;&nbsp;{moment(operatedRecord.applyDate)
                  .local()
                  .format('YYYY-MM-DD HH:mm:ss')}
              </div>
              <div>
                {this.$t('common.currency.rate') /*汇率*/}&nbsp;&nbsp;&nbsp;
                <span>
                  <InputNumber
                    min={0.0000001}
                    precision={7}
