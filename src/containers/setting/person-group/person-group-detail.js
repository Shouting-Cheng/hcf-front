                                ...
                                render: (text, record) => (
                                    <span>
                                        <Popconfirm onConfirm={(e) => this.deleteUserFromGroup(e, record)}
                                            title={this.$t({ id: "common.confirm.delete" }, { controlRule: record.controlRuleName })}>{/* 你确定要删除organizationName吗 */}
                                            <a href="#" onClick={(e) => {
                                                e.preventDefault();
                                                e.stopPropagation();
                                            }}>{this.$t({ id: "common.delete" })}</a>
                                        </Popconfirm>
                                    </span>)
                            },
                        ]
    ...
    _deleteEmptyCondition(Condition) {
        ...
        for (let i = 0; i < condition.conditionDetails.length; i++) {
            if (condition.conditionDetails[i].conditionValues.length > 0 || condition.conditionDetails[i].id) {
                ...
                condition.conditionDetails[i].conditionValues = this._removeDeletedValue(condition.conditionDetails[i].conditionValues);
...
export default connect(mapStateToProps, null, null, { withRef: true })(PersonGroupDetail);
