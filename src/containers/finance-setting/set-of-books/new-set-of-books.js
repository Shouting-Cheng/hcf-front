...
class NewSetOfBooks extends React.Component {
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        httpFetch[method](`${config.baseUrl}/api/setOfBooks`, params)
          .then(res => {
  ...
  getAccountSetOptions = () => {
    this.state.accountSetOptions.length === 0 &&
      httpFetch.get(`${config.baseUrl}/api/account/set/query`).then(res => {
  ...
  getPeriodSetOptions = () => {
    this.state.periodSetOptions.length === 0 &&
      httpFetch.get(`${config.baseUrl}/api/periodset`).then(res => {
        ...
        <Form onSubmit={this.handleSave} style={{ marginTop: 30 }}>
            ...
            {getFieldDecorator('periodSetCode', {
              ...
              <Select onFocus={this.getPeriodSetOptions} disabled={!!params.setOfBooksId}>
                {periodSetOptions.map(item => {
                  return (
                    <Option value={item.periodSetCode} key={item.id}>
                      {item.periodSetCode}
                    </Option>
                  );
                })}
          ...
          <FormItem {...formItemLayout} label={this.$t('set-of-books.name')}>
            {getFieldDecorator('setOfBooksName', {
              ...
              <LanguageInput
                name={params.setOfBooksName || null}
