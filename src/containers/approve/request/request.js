...
class ApproveRequest extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      approvePendingCheckboxList: [{
        id: 'formOIDs',
        items: [{label: messages('request.bill.name'/*单据名称*/), key: 'formOID', options: [], checkAllOption: true}]
      }],
      ...
      searchForm: [
        {type: 'input', id: 'businessCode', label: messages('finance.view.search.documentNo'/*单号*/)},
      ...
      status: [
        {label: messages('my.contract.state.generate'/*编辑中*/), value: '10011000', state: 'processing'},
        {label: messages('my.contract.state.withdrawal'/*已撤回*/), value: '10011001', state: 'warning'},
        {label: messages('finance.view.search.reject'/*已驳回*/), value: '10011002', state: 'error'},
        {label: messages('finance.view.search.auditReject'/*审核驳回*/), value: '10011003', state: 'error'},
        {label: messages('finance.view.search.submitted'/*审批中*/), value: '10021000', state: 'processing'},
        {label: messages('finance.view.search.pass'/*已通过*/), value: '10031000', state: 'success'},
        {label: messages('finance.view.search.auditPass'/*审核通过*/), value: '1004', state: 'success'},
        {label: messages('finance.view.search.payed'/*已付款*/), value: '1005', state: 'success'},
        {label: messages('finance.view.search.refund'/*还款中*/), value: '1006', state: 'processing'},
        {label: messages('finance.view.search.repaid'/*已还款*/), value: '1007', state: 'success'},
        {label: messages('finance.view.search.paying'/*付款中*/), value: '1008', state: 'processing'},
        {label: messages('supplier.management.disuse'/*已停用*/), value: '1009', state: 'default'}
      ],

      columns: [
        {title: messages('common.sequence'/*序号*/), dataIndex: 'index', width: '8%', render: (value, record, index) =>
        ...
        {title: messages('finance.view.search.jobNumber'/*工号*/), dataIndex: 'employeeID', render: value =>
          value ? <Popover placement="topLeft" content={value}>{value}</Popover> : '-'},
        {title: messages('finance.view.search.applicant'/*申请人*/), dataIndex: 'applicant', render: value =>
          (value && value.fullName) ? <Popover placement="topLeft" content={value.fullName}>{value.fullName}</Popover> : '-'},
        {title: messages('finance.view.search.submitDate'/*提交日期*/), dataIndex: 'submittedDate', render: value => moment(value).format('YYYY-MM-DD')},
        {title: messages('request.bill.name'/*单据名称*/), dataIndex: 'formName', render: value =>
          value ? <Popover placement="topLeft" content={value}>{value}</Popover> : '-'},
        {title: messages('finance.view.search.documentNo'/*单号*/), dataIndex: 'businessCode', render: value =>
          value ? <Popover placement="topLeft" content={value}>{value}</Popover> : '-'},
        {title: messages('approve.request.matter'/*事由*/), dataIndex: 'title', render: value =>
          value ? <Popover placement="topLeft" content={value}>{value}</Popover> : '-'},
        {title: messages('finance.view.search.currency'/*币种*/), dataIndex: 'originCurrencyCode', width: '8%'},
        {title: messages('borrowing.limit.control.amount'/*金额*/), dataIndex: 'originCurrencyTotalAmount', render: this.filterMoney},
        {title: messages('request.base.amount'/*本币金额*/), dataIndex: 'totalAmount',sorter: true, render:  value => value?this.filterMoney(value):<span className="money-cell">-</span>},
  ...
  getCache = () => {
    ...
    if (result && JSON.stringify(result) !== "{}") {
      ...
      this.setState({ cacheSearchData: result, approvePendingCheckboxList, approvedCheckboxList },() => {
  ...
  getForms = () => {
    ['approving', 'approved'].map(tab => {
      let checkboxList = this.state[tab === 'approving' ? 'approvePendingCheckboxList' : 'approvedCheckboxList'];
      approveRequestService.getDocumentType(tab === 'approved').then(res => {
        ...
        this.setState({
          [tab === 'approving' ? 'approvePendingCheckboxList' : 'approvedCheckboxList']: checkboxList
        }, () => {
  ...
  getList = () => {
    ...
    approveRequestService.getApproveRequestList(tab === 'approved', page, pageSize, tab === 'approved' ? approvedSearchParams : approvePendingSearchParams).then(res => {
      let data = [];
      res.data.map(item => {
        if (item.application) {
  ...
  handleRowClick = (record) => {
    ...
    url += this.state.tab === 'approving' ? `?approving=true&approverOID=${record.approverOID}` : '';
  ...
  handleApprove = (value, type) => {
    ...
    approveRequestService[type === 'pass' ? 'handleRequestApprovePass' : 'handleRequestApproveReject'](params).then(res => {
      if (res.status === 200) {
        message.success(messages('approve.request.successNum',{total:res.data.successNum}/*成功处理 ${res.data.successNum} 笔单据*/)+
                        messages('approve.request.failNum',{total:res.data.failNum}/*失败 ${res.data.failNum} 笔单据*/));
  ...
  render() {
    const { loading, tab, searchForm, approvePendingCheckboxList, approvedCheckboxList, columns, data, pagination, batchEnabled, selectedRowKeys, selectedRows, passLoading, rejectLoading, page, pageSize } = this.state;
    const rowSelection = {
      selectedRowKeys,
  ...
  {messages('common.total', {total: pagination.total}/*共搜索到 {total} 条数据*/)}
            {tab === 'approving' && batchEnabled && ` / ${messages('common.total.selected', {total: selectedRows.length})/*已选 {total} 条*/}`}
          </div>
        </div>
        <Table rowKey='applicationOID'
               ...
               bordered
               size="middle"/>
        <Affix offsetBottom={0} className={`bottom-bar-approve ${selectedRows.length && tab === 'approving' ? 'show' : 'hide'}`}>
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedApproveRequest)
