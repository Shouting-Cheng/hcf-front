...
export default {
  ...
  getDocumentType(finished) {
    return httpFetch.get(
      `${config.baseUrl}/api/approvals/custom/form?entityType=1001&finished=${finished}`
    );
  },
  ...
  getApproveRequestList(finished, page, size, searchParams) {
    let url = `${
      config.baseUrl
    }/api/approvals/batchfilters?entityType=1001&finished=${finished}&page=${page}&size=${size}`;
