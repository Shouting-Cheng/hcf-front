...
class ApproveRequestBtn extends React.Component{
  ...
  componentWillReceiveProps(nextProps) {
    if (!this.state.info.applicationOID && nextProps.approving) {
      ...
      this.setState({
        info: nextProps.info,
        showPriceViewBtn: formInfo.customFormPropertyMap && formInfo.customFormPropertyMap['travel.booker.price.audit.enable'] === 'true',
      }, () => {
        this.showAdditional();
  ...
  hasRepeatApproveTip = (value, additionalItems, priceAuditor) => {
    ...
    if (additionalHaveApprovedNames.length) {
      Modal.confirm({
        title: `${additionalHaveApprovedNames.join('、')} ${this.$t('approve.request.has.approved')/*已经审批通过，是否继续*/}？`,
  ...
  handleApprovePass = (value, additionalOIDs, priceAuditor) => {
    ...
    approveRequestService.handleRequestApprovePass(params).then(res => {
      if (res.data.failNum === 0) {
        ...
        this.setState({ passLoading: false });
        message.error(`${this.$t('common.operate.filed')}，${res.data.failReason[this.state.info.applicationOID]}`)
      }
    }).catch(e => {
      this.setState({ passLoading: false });
  ...
  handleApproveReject = (value, additionalItems) => {
    ...
    approveRequestService.handleRequestApproveReject(params).then(res => {
      if (res.data.failNum === 0) {
        message.success(this.$t('common.operate.success'));
  ...
  //返回
  goBack = () => {
    this.context.router.push(this.state.approveRequestList.url + `?tab=${this.props.approving ? 'approving' : 'approved'}`)
  };

  render() {
    const { passLoading, rejectLoading, approveRequestList, showAdditionalBtn, showPriceViewBtn, signCompanyOIDs } = this.state;
    let moreButtons = [];
    showAdditionalBtn && moreButtons.push('additional');
    return (
      <div className="approve-request-btn">
        {this.props.approving ? (
          <ApproveBar backUrl={approveRequestList.url + '?tab=approving'}
          ...
          <Button type="primary" className="back-btn" onClick={this.goBack}>{this.$t('common.back')}</Button>
        )}
      </div>
    )
  }
...
ApproveRequestBtn.contextTypes = {
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedApproveRequestBtn)
