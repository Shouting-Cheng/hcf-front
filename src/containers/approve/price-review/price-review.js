...
class PriceReview extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        ...
        {
          title: messages('price.review.submit.date' /*提交时间*/),
          dataIndex: 'lastModifiedDate',
          render: (value, record) => moment(value || record.createdDate).format('YYYY-MM-DD'),
        },
        {
          title: messages('price.review.type' /*类型*/),
          dataIndex: 'type',
          render: type =>
            type === 1001
              ? messages('price.review.type.book' /*订票*/)
              : type === 1002
  ...
  getList = () => {
    ...
    priceReviewService
      .getPriceReviewList(page, pageSize, tab === 'approving' ? 1005 : 1006, searchParams)
      .then(res => {
        this.setState({
          loading: false,
          data: res.data,
