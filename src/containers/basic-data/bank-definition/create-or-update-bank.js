...
class CreateOrUpdateBank extends React.Component {
  ...
  getRequestValue = (values) => {
    if(this.state.isChina){
      ...
      values.countryName = BSService.getCountryNameByCode(values.countryCode, this.state.countryData);
      if(values.openAccount.length > 0){
        values.provinceCode = values.openAccount[0];
        values.province = BSService.getStateNameByCode(values.countryCode, values.provinceCode, this.state.countryData);
      }
      if(values.openAccount.length > 1){
        values.cityCode = values.openAccount[1];
        values.city = BSService.getCityNameByCode(values.countryCode, values.provinceCode, values.cityCode, this.state.countryData);
      }
  ...
  renderCountryOption = (data) => {
    return data.map((item) => {
      return <Option value={item.value} key={item.code}>{item.label}</Option>
    })
  }
  ...
  renderOpenAccountByChina=()=>{
    ...
    if(this.state.isChina){
      ...
      <FormItem {...formItemLayout}
                label={messages('bank.openAccount')}
                help={bankTypeHelp}>
        {getFieldDecorator('openAccount', {
          initialValue: this.state.countryDefaultValue,
        })(
          <Cascader options={BSService.getCountryDataByCode('CHN000000000',this.state.countryData)}
    ...
    return (
      <div className="new-bank-definition">
        ...
        <Form onSubmit={this.handleSubmit} onChange={this.handleFormChange}>
            ...
            {getFieldDecorator('enable', {
              ...
              <div>
                ...
                <span className="enabled-type" style={{
                  marginLeft: 20,
                  width: 100
                }}>{enable ? messages('common.status.enable') : messages('common.disabled')}</span>
              </div>)}
          </FormItem>
          {/*银行代码*/}
          <FormItem {...formItemLayout}
                    label={messages('bank.bankCode')}>
            {getFieldDecorator('bankCode', {
              ...
              <Input disabled={typeof bank.id === 'undefined' ? false : true}
          ...
          {/*详情地址*/}
          <FormItem {...formItemLayout}
                    label={messages('bank.detailAddress')}
                    help={bankTypeHelp}>
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedCreateOrUpdateBank);
