...
class SectionMappingSet extends React.Component{
  ...
  handValueChange = (e,index,flag)=>{
    ...
    if(typeof data[index].glSegmentValueCode !== 'undefined' && typeof data[index].glSegmentValueCode !== 'undefined'){
  ...
  handleAdd = ()=>{
    ...
    if( data.length === 0 ||  data[0].sectionValueCode !== ""){
      let newParams = {segmentValueCode: '', glSegmentValueCode:'', key: paramsKey++, edit: true};
      let array=[];
  ...
  render(){
    const {loading, data, columns, pagination, selectedRowKeys, showImportFrame, isSave, isDelete} = this.state;
    const rowSelection = {
      selectedRowKeys,
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedSectionMappingSet);
