...
class SectionSetting extends React.Component{
  constructor(props){
    ...
    this.state = {
      ...
      searchForm: [
        {                                                    //科目段代码
          type: 'input', id: 'segmentCode', label: this.$t({id: 'section.code'}),
        },
        {                                                                        //科目段名称
          type: 'input', id: 'segmentName', label: this.$t({id: 'section.name'})
        },
        {                                                                        //科目段字段代码
          type: 'input', id: 'segmentField', label: this.$t({id: 'section.field.code'})
        },
      ],
      columns:[
        {          /*科目段代码*/
          title: this.$t({id:"section.code"}), key: "segmentCode", dataIndex: 'segmentCode'
        },
        {          /*科目段名称*/
          title: this.$t({id:"section.name"}), key: "segmentName", dataIndex: 'segmentName',
          render: description => (
            <span>{description ? <Popover content={description}>{description} </Popover> : '-'} </span>)

        },
        {          /*科目段字段代码*/
          title: this.$t({id:"section.field.code"}), key: "segmentField", dataIndex: 'segmentField'
        },
        {          /*科目段字段名称*/
          title: this.$t({id:"section.field.name"}), key: "segmentFieldName", dataIndex: 'segmentFieldName',
          render: description => (
            <span>{description ? <Popover content={description}>{description} </Popover> : '-'} </span>)
        },
        {          /*状态*/
          title: this.$t({id:"common.column.status"}), key: "enabled", dataIndex: 'enabled',
          render: enabled => (
        ...
        {          /*操作*/
          title: this.$t({id:"common.operation"}), key: "operate", dataIndex: 'operate',
          render: (text, record, index) => (
            <span>
            <a onClick={(e) => this.handleUpdate(e, record,index)}>{this.$t({id: "common.edit"})}</a>
            <span className="ant-divider" />
            <a onClick={(e) => this.handleLinkMapping(e, record,index)}>{this.$t({id: "section.mapping.setting"})}</a>
          </span>)
        },
      ]
    };
  ...
  handleBack = () => {
    this.context.router.push(menuRoute.getMenuItemByAttr('section-structure', 'key')
      .url.replace(":setOfBooksId",this.props.match.params.setOfBooksId));
  };
  ...
  render(){
    ...
    return (
      <div className="section-setting">
        ...
        <SlideFrame title= {this.$t({id:"section.mapping"})}
                    show={mapVisible}
