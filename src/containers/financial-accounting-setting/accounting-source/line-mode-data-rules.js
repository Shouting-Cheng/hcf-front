...
import NewUpDataLineModeDataRules from 'containers/financial-accounting-setting/accounting-source/new-updata-line-mode-data-rules'
import accountingService from 'containers/financial-accounting-setting/accounting-source/accounting-source.service'

class LineModeDataRules extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  getLineMode() {
    accountingService.getSourceTransactionModelSobById(this.props.match.params.lineModelId).then((response) => {
      this.setState({
  ...
  getList(searchText) {
    ...
    accountingService.getSourceLineModelDataRules(params).then((response) => {
  ...
  handleAfterClose = (valueRecord) => {
    if (valueRecord && JSON.stringify(valueRecord) != "{}") {
      ...
      if (value.dataRule == "VALUE_OF_RULE" && saveCount < 2) {
        accountingService.getSourceLineModelDataRulesById(value.id).then((res) => {
          let timestamp = (new Date()).valueOf();
          let params = {
            record: res.data,
            isNew: false,
  ...
  handleBack = () => {
    this.context.router.push(menuRoute.getMenuItemByAttr('accounting-source', 'key').children.voucherTemplateSob.url.replace(':id', this.props.match.params.id))
  };
...
export default connect(mapStateToProps, null, null, { withRef: true })(LineModeDataRules);
