...
/**
 * 侧拉组件，该组件内部组件将自带this.props.onClose(params)方法关闭侧拉栏
 */
class SlideFrame extends React.Component{
   ...
   * 组装方法
   * @param content 内部组件
   * @return {*} 给组件添加this.props.onClose(params)方法,params为返回到最外层的值
...
SlideFrame.propTypes = {
  ...
  content: PropTypes.oneOfType([PropTypes.func, PropTypes.string]),  //内容component，包裹后的元素添加this.props.onClose方法进行侧滑关闭
