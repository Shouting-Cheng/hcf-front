...
class NewUpDataLineModeDataRules extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      nextProps.params &&
      nextProps.params.timestamp != this.props.params.timestamp
    ) {
        ...
        <Spin spinning={this.state.spinningLoading}>
            ...
            {buttonRender ? (
              <Row>
                <Col offset={3} span={18}>
                  <Button type="dashed" style={{ high: 40, width: '100%' }} onClick={this.addApply}>
                    <Icon type="plus" />
                    {this.$t({ id: 'accounting.source.addChangeRule' })}{' '}
                  </Button>
                </Col>
              </Row>
            ) : (
          ...
          {changeRulesRender ? (
            <div>
              ...
              <div style={{ marginTop: '24px' }}>{this.renderChangeRules()}</div>
            </div>
          ) : null}
        </Spin>
      </div>
