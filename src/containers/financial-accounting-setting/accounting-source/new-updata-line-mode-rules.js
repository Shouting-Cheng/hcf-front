...
class NewUpdateLineModeRules extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.params.isNew == true) {
      ...
      //编辑
      this.getSegement(this.props.params);
  ...
  getSegement(params) {
    ...
    accountingService
      .getSegmentBySetOfBooksId(setOfBooksId, params.lineModelId)
      .then(res => {
        let segementOption = [];
        let data = res.data;
        data.map(item => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (values.dataRule === 'FIXED_VALUE') {
          ...
          values.dataRule === 'ACCOUNT_ELEMENT' ||
          values.dataRule === 'ACCOUNT_ELEMENT_MAP'
        ) {
        ...
        if (isNew) {
          //新建
          accountingService
            .addSourceLineModelRules(dataValue)
            .then(res => {
          ...
          if (
            editData.dataRule != 'ACCOUNT_ELEMENT' &&
            editData.dataRule != 'ACCOUNT_ELEMENT_MAP' &&
            editData.dataRule != 'FIXED_VALUE'
          ) {
            editData.data = '';
          }
          accountingService
            .upSourceLineModelRules(editData)
            .then(res => {
              message.success(this.$t({ id: 'common.operate.success' }));
              this.setState({ loading: false });
              this.props.onClose(true);
...
*/}
        ...
        <Form onSubmit={this.handleSubmit} className="voucher-template-form">
          {this.props.params.flag && (
            <FormItem
              {...formItemLayout}
              label={this.$t({ id: 'accounting.source.journalFieldCode' })}
            >
              {/*核算分录段*/}
          ...
          <FormItem {...formItemLayout} label={this.$t({ id: 'accounting.source.dataRule' })}>
            {/*取值方式*/}
          ...
          {dataRule === 'FIXED_VALUE' ? (
            <FormItem {...formItemLayout} label={this.$t({ id: 'accounting.source.data' })}>
              ...
              {getFieldDecorator('fixedValue', {
                ...
                <Input
                  className="input-disabled-color"
                  placeholder={this.$t({ id: 'accounting.source.stateData' })}
                />
              )}
            </FormItem>
          ) : (
          ...
          <FormItem {...formItemLayout} label={this.$t({ id: 'accounting.source.segment' })}>
            {/*科目段值*/}
