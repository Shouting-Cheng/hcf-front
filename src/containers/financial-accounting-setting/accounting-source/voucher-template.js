...
class VoucherTemplateSob extends React.Component {
    ...
    this.state = {
      ...
      lov: {
        ...
        columns: [
          ...
          {
            title: this.$t({ id: 'accounting.source.name' }),
      ...
      searchForm: [
        {
          //来源事物代码
          type: 'input',
          id: 'accountingSourceCode',
          label: this.$t({ id: 'accounting.source.code' }),
        },
        {
          //来源事物名称
          type: 'input',
          id: 'accountingSourceName',
      ...
      columns: [
        ...
        {
          /*凭证行模板名称*/
          title: this.$t({ id: 'voucher.template.name' }),
          key: 'description',
          dataIndex: 'description',
          render: recode => <Popover content={recode}>{recode}</Popover>,
        },
        {
          /*核算场景代码*/
          title: this.$t({ id: 'accounting.scene.code' }),
          key: 'glSceneCode',
          dataIndex: 'glSceneCode',
          render: recode => <Popover content={recode}>{recode}</Popover>,
        },
        {
          /*核算场景名称*/
          title: this.$t({ id: 'accounting.scene.name' }),
          key: 'glSceneName',
        ...
        {
          /*基础数据表*/
          title: this.$t({ id: 'basic.data.sheet' }),
          key: 'dataStructureName',
        ...
        },
        {
          ...
          render: (text, record, index) => (
            <span>
              <a onClick={e => this.handleDataRules(e, record, index)}>
                {this.$t({ id: 'accounting.source.dataRules' })}
              </a>{' '}
              {/*取值规则*/}
              <span className="ant-divider" />
              <a onClick={e => this.handleJudgeRules(e, record, index)}>
                {this.$t({ id: 'accounting.source.judgeRuleName' })}
              </a>{' '}
              {/*判断规则*/}
              <span className="ant-divider" />
              <a onClick={e => this.handleRules(e, record, index)}>
                {this.$t({ id: 'accounting.source.rule' })}
              </a>{' '}
              {/*核算规则*/}
            </span>
          ),
  ...
  handleJudgeRules = (e, record, index) => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/financial-accounting-setting/accounting-source/voucher-template-sob/line-mode-judge-rules/:id/:lineModelId'
          .replace(':id', this.props.match.params.id)
          .replace(':lineModelId', record.id),
      })
    );
  };
  ...
  handleRules = (e, record, index) => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/financial-accounting-setting/accounting-source/voucher-template-sob/line-mode-rules/:id/:lineModelId'
          .replace(':id', this.props.match.params.id)
          .replace(':lineModelId', record.id),
      })
    );
  ...
  onChangeEnabled = (e, record) => {
    ...
    data.push(record);
    accountingService
      .upSourceTransactionModelSob(data)
      .then(() => {
  ...
  handleAdd = value => {
    ...
    this.setState({ loading: true });
    accountingService
      .addSourceTransactionModelSob(this.props.match.params.id, valueData)
      .then(() => {
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/financial-accounting-setting/accounting-source/:sourceSetOfBooksId'.replace(
          ':sourceSetOfBooksId',
      ...
      <div className="voucher-template" style={{ paddingBottom: 20 }}>
        <div className="voucher-template-header">
          <h3>
            {`${this.$t({ id: 'voucher.template.header1' })} ` +
              sourceName +
              ` ${this.$t({ id: 'voucher.template.header2' })}`}
          </h3>
        </div>
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            ...
            {/*添加*/}
            <Search
              placeholder={this.$t({ id: 'voucher.template.input' }) /* 请输入名称/代码 */}
