...
class NewUpDataLineModeJudgeRulesSystem extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
    ...
    if (JSON.stringify(nextProps.params) !== "{}" && nextProps.params.time != this.props.params.time) {
      ...
      if (nextProps.params.isNew === true) {
        ...
        this.setState({
          ...
          enabled: nextProps.params.record.enabled
        }, () => {})
      }
    }
  }
...
const WrappedNewUpDataLineModeJudgeRules = Form.create()(NewUpDataLineModeJudgeRulesSystem);
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedNewUpDataLineModeJudgeRules);
