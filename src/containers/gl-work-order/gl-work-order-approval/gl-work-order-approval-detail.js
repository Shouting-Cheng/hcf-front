...
import { Button, Affix, message, Row, Col, Icon, Input, Tabs, Table, Divider, InputNumber, Popover } from 'antd'
...
class GLWorkOrderCheckDetail extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                {
                    title: '序号', dataIndex: 'seq', align: 'center', width: '60',
                ...
                {
                    title: '备注', dataIndex: 'description', align: 'center', width: '130',
                ...
                {
                    title: '公司', dataIndex: 'companyName', align: 'center', width: '140',
                ...
                {
                    title: '科目', dataIndex: 'accountName', align: 'center', width: '130',
                ...
                {
                    title: '借方金额', dataIndex: 'enteredAmountCr', align: 'center', width: '130', fixed: 'right',
                ...
                {
                    title: '贷方金额', dataIndex: 'enteredAmountDr', align: 'center', width: '130', fixed: 'right',
    ...
    onRejectClick = () => {
        ...
        glWorkOrderCheckService.reject(params).then(res => {
            if (res.status === 200) {
                message.success('操作成功');
                this.setState({ operateLoading: false });
                this.onBack();
    ...
    render() {
        ...
        return (
            <div className="gl-work-order-detail background-transparent">
                ...
                {
                    (
                        (docStatus && docStatus === '1002')
                        &&
                        <Affix className='bottom-bar' offsetBottom='0'>
                            <Row gutter={12} type='flex' justify='start' >
                                <Col offset={1}><label>审批意见：</label></Col>
                                <Col span={11} ><Input placeholder='请输入，最多200个字符' value={opinion} onChange={this.onOpinionChange} /></Col>
                                <Col span={1.5} ><Button loading={operateLoading} type='primary' onClick={this.onPassClick}>通过</Button></Col>
                                <Col span={1.5} ><Button loading={operateLoading} onClick={this.onRejectClick} style={{ background: '#F00000', color: 'white' }} type='danger'>驳回</Button></Col>
                                <Col span={3} offset={1}><Button loading={operateLoading} onClick={this.onBack}>返回</Button></Col>
                            </Row>
                        </Affix>
                    )
                    ||
                    (
                        (docStatus && docStatus === '1004')
                        &&
                        <Affix className='bottom-bar' offsetBottom='0'>
                            <Row gutter={12} type='flex' justify='start' >
                                <Col span={3} offset={1}><Button onClick={this.onBack}>返回</Button></Col>
                            </Row>
                        </Affix>
                    )
                }
...
export default connect(mapStateToProps, null, null, { withRef: true })(GLWorkOrderCheckDetail);
