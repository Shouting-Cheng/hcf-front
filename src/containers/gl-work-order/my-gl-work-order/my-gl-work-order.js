...
class MyGLWorkOrder extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            searchForm:
                [
                    {
                        type: 'select', label: '单据类型', id: 'workOrderTypeId', colSpan: '6',
                        getUrl: `${config.accountingUrl}/api/general/ledger/work/order/types/queryByEmployeeId?userId=${this.props.user.id}`,
                        options: [], method: "get", valueKey: "id", labelKey: "workOrderTypeName"
                    },
                    {
                        type: 'items', id: 'requisitionDate', colSpan: '6',
                        items:
                            [
                    ...
                    {
                        type: 'list', label: '申请人', id: 'employeeId', colSpan: '6',
                        listExtraParams: { setOfBooksId: this.props.company.setOfBooksId },
                        listType: 'user_budget',event: 'APPLIER',
                        valueKey: 'id', labelKey: 'fullName', single: true,
                    },
                    {
                        type: 'select', label: '状态', id: 'status', colSpan: '6',
                        options: [
                    ...
                    {
                        type: 'select', label: '币种', id: 'currency', colSpan: '6',
                        options: [], method: "get",
                        getUrl: `${config.baseUrl}/api/currency/rate/list`, listKey: 'rows',
                        getParams: { enable: true, setOfBooksId: this.props.company.setOfBooksId, tenantId: this.props.company.tenantId },
                        valueKey: 'currencyCode', labelKey: 'currencyCode'
                    }
                ],
            //表格
            columns:
                [
                    ...
                    {
                        title: '申请日期', dataIndex: 'requisitionDate', align: 'center',
                        render: (requisitionDate) => {
                            return (
                                <span>{moment(requisitionDate).format('YYYY-MM-DD')}</span>
                            )
                        }
                    },
                    { title: '币种', dataIndex: 'currency', align: 'center' },
                    {
                        ...
                        render: (amount) => {
                            return (
                                <span>{this.filterMoney(amount, 2)}</span>
                            )
                        }
                    },
                    {
  ...
  handleEvent =(key,value)=>{
      switch (key) {
        case 'APPLIER': value.length === 0 && this.setState({searchParams:{...this.state.searchParams,employeeId: ''}})
      }
  };
    ...
    getTypeList = () => {
        ...
        myGlWorkOrderService.getTypeList(userId).then(res => {
            if (res.status === 200) {
                this.setState({
    ...
    getList = () => {
        ...
        let params = {
            ...
            requisitionDateFrom: searchParams.requisitionDateFrom ? moment(searchParams.requisitionDateFrom).format('YYYY-MM-DD') : null,
            requisitionDateTo: searchParams.requisitionDateTo ? moment(searchParams.requisitionDateTo).format('YYYY-MM-DD') : null,
            status: searchParams.status ? searchParams.status : null,
            currency: searchParams.currency ? searchParams.currency : null
        };
        myGlWorkOrderService.workOrderHeadQuery(params).then(res => {
            if (res.status === 200) {
                this.setState({
                    loading: false,
                    data: res.data,
                        ...
                        <Row>
                            <Col span={18}>
                                <Dropdown overlay={
                                    ...
                                    trigger={['click']}>
                                    <Button type='primary'>新建核算工单 <Icon type="down" /></Button>
                                </Dropdown>
                            </Col>
                            <Col span={6}>
                                <Search
...
export default connect(mapStateToProps, null, null, { withRef: true })(MyGLWorkOrder);
