...
class MyGLWorkOrderDetail extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                {
                    title: '序号',
                    dataIndex: 'seq', align: 'center', width: '60',
                ...
                {
                    title: <span><span style={{ color: 'red' }}>*</span>&nbsp;备注</span>,
                    dataIndex: 'description', align: 'center', width: '140',
                ...
                {
                    title: <span><span style={{ color: 'red' }}>*</span>&nbsp;公司</span>,
                    dataIndex: 'companyName', align: 'center', width: '140',
                        ...
                        if (record.lineStatus === 'normal') {
                            ...
                            return (
                                <Chooser
                                    ...
                                    value={record.companyId ? [{ id: record.companyId, name: record.companyName }] : []}
                ...
                {
                    title: '部门', dataIndex: 'unitName', align: 'center', width: '140',
                        ...
                        if (record.lineStatus === 'normal') {
                            ...
                            return (
                                <Chooser
                                    ...
                                    value={record.unitOid ? [{ departmentOid: record.unitOid, departmentId: record.unitId, name: record.unitName }] : []}
                                    type='department'
                                    labelKey='name'
                ...
                {
                    title: <span><span style={{ color: 'red' }}>*</span>&nbsp;科目</span>,
                    dataIndex: 'accountName', align: 'center', width: '140',
                        ...
                        if (record.lineStatus === 'normal') {
                            ...
                            return (
                                <Chooser
                                    ...
                                    value={record.accountId ? [{ id: record.accountId, name: record.accountName, accountName: record.accountName, accountCode: record.accountCode }] : []}
                ...
                {
                    title: <span><span style={{ color: 'red' }}>*</span>&nbsp;借方金额</span>,
                    dataIndex: 'enteredAmountCr', align: 'center', width: '140', fixed: 'right',
                ...
                {
                    title: <span><span style={{ color: 'red' }}>*</span>&nbsp;贷方金额</span>,
                    dataIndex: 'enteredAmountDr', align: 'center', width: '140', fixed: 'right',
                        ...
                        if (record.lineStatus === 'normal') {
                            return (
                                <div>
                                    ...
                                    <Popconfirm title='确认删除？' okText='确认' cancelText='取消' onConfirm={(e) => this.onDelLineClick(e, record, index)}>
                                        <a>删除</a>
                                    </Popconfirm>
                                </div>
                            )
    ...
    onDelLineClick = (e, record, index) => {
        e.preventDefault();
        myGlWorkOrderService.delLineData(record.id).then(res => {
            if (res.status === 200) {
    ...
    addDimensionColumns = (dimensionData) => {
        ...
        if (columns.length <= 8) {
            dimensionData.map(item => {
                ...
                //拼接列
                let dimensionColumn = {
                    title: dimensionTitle, dataIndex: dimensionKey, align: 'center',
                    render: (text, record, index) => {
                        if (record.lineStatus === 'normal') {
                            ...
                            return (
                                <Chooser
                                    onChange={(value) => this.onDimensionChange(value, record, index, dimensionKey, dimensionName)}
                                    value={record[dimensionKey] ? [{ id: record[dimensionKey], name: record[dimensionName] }] : []}
    ...
    onSave = () => {
        ...
        let nullFlag = 0;
        data.map((item, index) => {
            if (!(item.description) || !(item.companyId) || !(item.accountId) || (!(item.enteredAmountCr) && !(item.enteredAmountDr))) {
                nullFlag += 1;
                return;
            }
            if (!item.unitId) {
        ...
        //调用接口实现批量保存
        myGlWorkOrderService.saveLineData(params).then(res => {
            if (res.status === 200) {
    ...
    onSubmit = () => {
        ...
        myGlWorkOrderService.submitDocument(params).then(res => {
            if (res.status === 200) {
                this.setState({ operationLoading: false });
    ...
    back = () => {
        ...
        myGlWorkOrderService.approvalsWithdraw(params).then(res => {
            if (res.status === 200) {
    ...
    onImportOk = (transactionID) => {
        myGlWorkOrderService.importOk(transactionID).then(res => {
            if (res.status === 200) {
    ...
    render() {
        ...
        if (docHeadData.status === 1001 || docHeadData.status === 1003 || docHeadData.status === 1005) {
            ...
            status =
                (
                    <h3 className="header-title">
                        <Button loading={operationLoading} type="primary" onClick={this.back}>撤 回</Button>
                    </h3>
                );
        } else {
                ...
                <Affix offsetBottom={0} className="bottom-bar">
                    {
                        (docHeadData.status === 1001 || docHeadData.status === 1003 || docHeadData.status === 1005)
                            ?
                            <div>
                                <Button style={{ marginLeft: '20px' }} type='primary' loading={operationLoading} onClick={this.onSubmit}>提交</Button>
                                <Button style={{ marginLeft: '20px' }} loading={operationLoading} onClick={this.onSave}>保存</Button>
                                <Button style={{ marginLeft: '20px' }} loading={operationLoading} onClick={this.onDelete}>删除</Button>
                                <Button style={{ marginLeft: '20px' }} loading={operationLoading} onClick={this.onBack}>返回</Button>
                            </div>
                            :
                            <div>
                                <Button style={{ marginLeft: '20px' }} loading={operationLoading} onClick={this.onBack}>返 回</Button>
                            </div>
                    }
                </Affix>
                {/* 导入 */}
