...
class NewGLWorkOrder extends Component {
    ...
    getHeaderData = () => {
        ...
        if (this.props.match.params.id && this.props.match.params.id != ':id') {
            myGLWorkOrderService.getHeaderData(this.props.match.params.id).then(res => {
                if (res.status === 200) {
                    let defaultFileList = [];
                    ...
                    this.props.form.setFieldsValue({
                        ...
                        unitId: [{ departmentOid: orderData.unitOid, departmentId: orderData.unitId, name: orderData.unitName }]
                    });
                }
            }).catch(e => {
     ...
     */
    getDepartmentId = () => {
        myGLWorkOrderService.getDepartmentId(this.props.user.departmentOID).then(res => {
            if (res.status === 200) {
                let departmentId = res.data.id;
                this.props.form.setFieldsValue({ unitId: [{ departmentOid: this.props.user.departmentOID, departmentId: departmentId, name: this.props.user.departmentName }] });
            }
        }).catch(e => {
    ...
    getCurrency = () => {
        ...
        myGLWorkOrderService.getCurrency(setOfBooksId, tenantId).then(res => {
            if (res.status === 200) {
                this.setState({
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                let params = {
                    ...
                    workOrderTypeId: orderData.id ? orderData.workOrderTypeId : this.props.match.params.typeId,
                    ifWorkflow: (orderData.id ? orderData.formOid : this.props.match.params.formOid) === 0 ? false : true,
                ...
                myGLWorkOrderService.orderInsert(params).then(res => {
                    if (res.status === 200) {
            ...
            <div>
                <Form onSubmit={this.handleSave}>
                            ...
                            <FormItem {...formItemLayout} label='公司'>
                                {
                                    getFieldDecorator('companyId', {
                                        ...
                                        initialValue: orderData.id ? [{ id: orderData.companyId, name: orderData.companyName }] : [{ id: this.props.user.companyId, name: this.props.user.companyName }]
                                    })(
                                        <Chooser
                                            type='company'
                                            labelKey='name'
                            ...
                            <FormItem {...formItemLayout} label='部门'>
                                {
                                    getFieldDecorator('unitId', {
                                        ...
                                        initialValue: orderData.id ? [{ departmentOid: orderData.unitOid, departmentId: orderData.unitId, name: orderData.unitName }] : []
                                    })(
                                        <Chooser
                                            type='department'
                                            labelKey='name'
                    ...
                    <Affix offsetBottom={0}
                        ...
                        <Button type="primary" htmlType="submit" loading={loading} style={{ margin: '0 20px' }}>{orderData.id ? "确定" : "下一步"}</Button>
...
export default connect(mapStateToProps, null, null, { withRef: true })(wrappedNewGLWorkOrder);
