...
class GLWorkOrderTypeDC extends Component {
        ...
        [
          ...
          { title: '公司类型', dataIndex: 'companyType' },
          {
            ...
            render: (enabled, record, index) => {
              return (
                <Checkbox checked={enabled} onChange={e => this.onIsEnabledChange(e, record)} />
              )
            }
          }
        ],
   ...
   */
  componentWillMount = () => {
    glWorkOrderTypeService.getTypeById(this.props.match.params.id).then(res => {
      if (res.status === 200) {
        this.setState({
  ...
  onIsEnabledChange = (e, record) => {
    ...
    glWorkOrderTypeService.typeDistributionCompanyUpdate(params).then(res => {
      if (res.status === 200) {
  ...
  getList = () => {
    ...
    glWorkOrderTypeService.getTypeDistributionCompany(this.props.match.params.id, page, pageSize).then(res => {
      if (res.status === 200) {
        this.setState({
          loading: false,
          data: res.data,
  ...
  onCompanyOk = (value) => {
    ...
    glWorkOrderTypeService.typeDistributionCompanyInsert(params).then(res => {
      if (res.status === 200) {
        message.success('操作成功！');
  ...
  render() {
    ...
    return (
      <div>
        <Row gutter={24} type='flex' justify='start' style={{ background: '#f7f7f7', padding: '20px 25px 0', borderRadius: '6px 6px 0 0' }}>
          <Col span={6} style={{ marginBottom: '15px' }}>
            <div style={{ color: '#989898' }}>账套</div>
            <div style={{ wordWrap: 'break-word' }}>{nowTypeList.setOfBooksCode}-{nowTypeList.setOfBooksName}</div>
          </Col>
          <Col span={6} style={{ marginBottom: '15px' }}>
          ...
          <Col span={6} style={{ marginBottom: '15px' }}>
            ...
            <div style={{ wordWrap: 'break-word' }}>
              <Badge status={nowTypeList.enabled ? 'success' : 'error'} text={nowTypeList.enabled ? '启用' : '禁用'} />
            </div>
          </Col>
        </Row>
        <div className="table-header">
          <div className="table-header-buttons">
            <Button type="primary" onClick={this.handleCompany}>分配公司</Button>
          </div>
        </div>
        <Table
          columns={columns}
        ...
        />
        <a onClick={this.onBackClick}><Icon type="rollback" />返回</a>
        {/* 批量分配公司 */}
        <ListSelector
...
export default connect(mapStateToProps, null, null, { withRef: true })(GLWorkOrderTypeDC);
