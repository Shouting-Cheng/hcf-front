...
class NewGLWorkOrderType extends Component {
    ...
    componentDidMount = () => {
      ...
      if (this.props.params.glWorkOrderTypeList.id) {
        //编辑时
        glWorkOrderTypeService.getTypeById(this.props.params.glWorkOrderTypeList.id).then(res => {
          if (res.status === 200) {
            //当前数据
            ...
            res.data.dimensionIdList && res.data.dimensionIdList.map(item => {
              dimensionIdList_value.push({ id: item });
            });
            //科目
            let accountIdList_value = [];
            res.data.accountIdList && res.data.accountIdList.map(item => {
              accountIdList_value.push({ id: item });
            });
            //人员权限
            let departmentOrUserGroupIdList_value = res.data.departmentOrUserGroupIdList ? res.data.departmentOrUserGroupIdList : []
            this.setState({
     ...
     */
    getRelatedFormList = () => {
        glWorkOrderTypeService.getRelatedFormList(this.props.company.setOfBooksId).then(res => {
            if (res.status === 200) {
                this.setState({
            ...
            if (!err) {
                ...
                let params = {
                    generalLedgerWorkOrderType: {
                        ...
                        formName: values.formOid.substring(values.formOid.indexOf('$$') + 2, values.formOid.length),
                ...
                if (nowTypeList.id) {
                    //编辑时
                    glWorkOrderTypeService.typeUpdate(params).then(res => {
                        if (res.status === 200) {
                    ...
                    //新增时
                    glWorkOrderTypeService.typeInsert(params).then(res => {
                        if (res.status === 200) {
                            message.success('保存成功');
                            this.setState({
                                loading: false
        ...
        return (
            <div>
                <Form onSubmit={this.onSubmit}>
                                ...
                                <Select disabled>
                                    {
                                        setOfBooksIdList.map(item => {
                                            return (
                                                <Option value={item.id}>{item.setOfBooksCode}-{item.setOfBooksName}</Option>
                                            )
                                        })
                                    }
                                </Select>
                    ...
                    <FormItem {...formItemLayout} label='维度字段'>
                        ...
                        <Input
                            ...
                            value={dimensionScope === '1001' ? '全部维度' : `已选择了${dimensionIdList.length}个维度`} />
                    </FormItem>
                    ...
                    <FormItem {...formItemLayout} label=' ' colon={false}>
                        {
                            getFieldDecorator('visibleCompany', {
                                ...
                                <RadioGroup>
                                    <Radio style={radioStyle} value='1001'>账套下全部公司</Radio>
                                    <Radio style={radioStyle} value='1002'>本公司及下属公司</Radio>
                                    <Radio style={radioStyle} value='1003'>下属公司</Radio>
                                    <Radio style={radioStyle} value='1004'>仅本公司</Radio>
                                </RadioGroup>
                            )
                        }
                    </FormItem>
                ...
                <ListSelector
                    ...
                    extraParams={{ setOfBooksId: nowTypeList.id ? nowTypeList.setOfBooksId : setOfBooksId, id: nowTypeList.id }}
                    single={false} />
                <ListSelector
...
export default connect(mapStateToProps, null, null, { withRef: true })(WrappedNewGLWorkOrderType);
