...
class GLWorkOrderType extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            searchForm: [
                {
                    type: 'select', id: 'setOfBooksId', colSpan: 6, label: '账套',
                    options: [], labelKey: 'name', valueKey: 'id',
                    isRequired: 'true', event: 'SETOFBOOKSID',
                    defaultValue: this.props.company.setOfBooksId
                },
                {
                    type: 'input', id: 'workOrderTypeCode', colSpan: '6', label: '核算工单类型代码'
                },
                {
                    type: 'input', id: 'workOrderTypeName', colSpan: '6', label: '核算工单类型名称'
                },
                {
                    type: 'select', id: 'enabled', colSpan: '6', label: '状态',
                    options: [
                        { value: true, label: "启用" },
            ...
            columns: [
                {
                    title: '账套', dataIndex: 'setOfBooksName', align: 'center',
    ...
    getList = () => {
        ...
        glWorkOrderTypeService.typeQuery(params).then(res => {
            if (res.status === 200) {
                this.setState({
                    loading: false,
                    data: res.data,
        ...
        return (
            <div>
                ...
                <SlideFrame
                  ...
                  <NewGLWorkOrderType onClose={this.onSlideFrameClose} params={{ glWorkOrderTypeList: glWorkOrderTypeList, setOfBooksId: setOfBooksId, visibleFlag: showSlideFrame }}/>
                </SlideFrame>
            </div>
        )
    }
...
export default connect(mapStateToProps, null, null, { withRef: true })(GLWorkOrderType);
