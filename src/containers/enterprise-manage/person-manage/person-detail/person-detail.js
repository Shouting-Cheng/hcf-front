...
class PersonDetail extends React.Component {
  ...
  renderEditBtn = (val) => {
    if (!val &&
      ...
      return (
        <div className="f-left person-detail-edit-icon"
             onClick={this.BasicInfoToEditing}>
          <Icon type="edit"/>
  ...
  renderBankCards = (cards) => {
    if (cards.length > 0) {
      ...
      cards.map((data, index) => {
        dom.push(<div key={"is-empty-bank-card" + index} className="f-left bank-card-item">
          <BankCard
            isShowEditBtn={this.props.tenantMode &&
            this.state.CREATE_DATA_TYPE}
  ...
  renderContactCards = (cards) => {
    if (cards.length > 0) {
      let dom = [];
      cards.map((data, index) => {
        dom.push(<div key={"is-empty-id-card" + index} className="f-left id-card-item">
          <SomeIdCard
            isShowEditBtn={this.props.tenantMode &&
            this.state.CREATE_DATA_TYPE}
  ...
  renderVendorEditBtn = (val) => {
    if (!val && this.props.tenantMode &&
      this.state.CREATE_DATA_TYPE) {
      return (
        <div className="f-left person-detail-edit-icon"
             onClick={this.VendorToEditing}>
          <Icon type="edit"/>
          <span className="edit-text">
                 {/*编辑*/}
            {messages("common.edit")}
  ...
  render() {
    ...
    return (
      <div className="person-detail-wrap">
        ...
        <div className="person-detail-wrap-for-model"></div>
        <Modal
          getContainer={() => {
...
export default connect(mapStateToProps, null, null, { withRef: true })(PersonDetail);
