...
class AddPayWay extends React.Component {
  ...
  getPayWayList = paymentType => {
    httpFetch
      .get(
        `${config.payUrl}/api/cash/payment/method/selectByPaymentType?paymentType=${paymentType}`
      )
      .then(res => {
      ...
      if (!err) {
        ...
        if (this.state.params.id) {
          temp.push({
            ...
            bankAccountId: this.state.params.companyBankId,
          });
        } else {
        ...
        httpFetch
          .post(`${config.baseUrl}/api/comapnyBankPayment/insertOrUpdate`, temp)
          .then(res => {
