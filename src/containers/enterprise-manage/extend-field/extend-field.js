...
class ExtendField extends React.Component {
  ...
  //获取表单字段类型
  getWidgetList = () => {
    EFService.getWidgetList({type: 1003})
      .then((res) => {
        this.setState({
  ...
  getCustomForm = () => {
    ...
    if (this.props.profile["company.contact.custom.form"]) {
      ...
      EFService.getCustomForm(this.props.profile["company.contact.custom.form"])
        .then((res) => {
...
export default connect(mapStateToProps, null, null, { withRef: true })(ExtendField);
