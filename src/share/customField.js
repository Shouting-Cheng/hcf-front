...
export default {
  ...
  renderFields(fields, info, applicant) {
    ...
    showFields.map(item => {
      if (item.messageKey === 'average_budget') {
        item.renderValue = item.showValue
          ? `${info.currencyCode} ${renderMoney(item.showValue)}`
          : `${info.currencyCode} 0.00`;
      }
      ...
      if (
        item.messageKey === 'start_date' ||
        item.messageKey === 'end_date' ||
        item.messageKey === 'common.date' ||
        item.messageKey === 'date'
      ) {
      ...
      if (item.messageKey === 'time') {
        item.renderValue = item.showValue
          ? moment(item.showValue)['_isValid']
            ? moment(item.showValue).format('HH:mm')
            : item.showValue
          : '-';
      }
          ...
          item.renderValue = (
            <div>
              {JSON.parse(item.fieldContent || '[]').map(item => {
                return (
                  <div key={item.id}>
                    {item.fieldName}：{item.name || item.value || '-'}
                  </div>
                );
              })}
            </div>
          ...
          cols.push(
            <Col span={8} key={item.formValueOID} className="field-container">
              ...
              <div className="field-content">
                {item.value === 'true' ? messages('common.yes') : messages('common.no')}
              </div>
            </Col>
          );
          cols.push(
      ...
      if (item.messageKey === 'switch' || item.messageKey === 'writeoff_flag') {
        item.renderValue =
          item.showValue === 'true'
            ? messages('customField.yes' /*是*/)
            : messages('customField.no' /*否*/);
      }
      ...
      if (
        item.messageKey === 'external_participant_name' ||
        item.messageKey === 'out_participant_name'
      ) {
        //外部乘机人
      ...
      if (item.messageKey === 'employee_expand') {
        if ((item.attachmentImages || []).length) {
          item.renderValue = (
            <ImageUpload
              defaultFileList={item.attachmentImages}
        ...
        let columns = [
          {
            ...
            width: '5%',
            render: (value, record) => (
              <img src={record.expenseType.iconURL} style={{ width: 20 }} />
            ),
          },
          {
            title: messages('customField.expense.type' /*费用类型*/),
          ...
          {
            title: messages('customField.amount' /*金额*/),
            dataIndex: 'amount',
            render: filterMoney,
          },
          {
            title: messages('customField.rate' /*汇率*/),
            dataIndex: 'actualCurrencyRate',
            render: value => filterMoney(value, 4),
          },
          {
            title: messages('customField.base.amount' /*本币金额*/),
            dataIndex: 'baseAmount',
            render: (value, record) =>
              filterMoney((record.amount * record.actualCurrencyRate).toFixed(4)),
          },
          {
            title: messages('customField.pay.way' /*支付方式*/),
            dataIndex: 'paymentType',
            render: value =>
              value === 1001
                ? messages('customField.pay.by.person' /*个人支付*/)
                : messages('customField.pay.by.company' /*公司支付*/),
          },
        ];
        let data = JSON.parse(item.showValue || '{}').budgetDetail;
        item.value &&
          renderTableList.push(
            <Table
      ...
      if (item.messageKey === 'exp_allocate') {
        //费用分摊
        let columns = [];
        let data = [];
        ...
        columns.push({
          title: messages('customField.allocate.scale' /*分摊比例(%)*/),
          dataIndex: 'scale',
        });
        item.value &&
          renderTableList.push(
      ...
      if (
        ...
        !(
          item.messageKey === 'employee_expand' &&
        ...
        cols.push(
          <Col span={8} key={item.formValueOID} className="field-container">
            <div className="field-name">{item.fieldName}</div>
            {applicant && applicant.departmentOID ? (
              <div
                className="field-content"
                style={
                  item.messageKey === 'select_department' &&
                  applicant &&
                  item.value !== applicant.departmentOID
                    ? { color: '#f50' }
                    : {}
                }
              >
    ...
    renderTableList.map((item, index) => {
      rows.push(
        <Row key={index + 1} className="custom-form-row">
          {item}
        </Row>
      );
    });
  ...
  dealSpecialFormShow(cols, item, info, messageKey = item.messageKey) {
    if (messageKey === 'contact_bank_account') {
      ...
      let showValue =
        item.showValue && isExpenseReport
          ? JSON.parse(item.showValue)
          : isExpenseReport
            ? {}
            : { bankAccountNo: item.showValue };
      ...
      if (isExpenseReport) {
        ...
        otherFields.push({
          fieldName: messages('customField.bankCode' /*联行号*/),
          renderValue: showValue.bankCode,
        });
        otherFields.push({
  ...
  renderForm({
    ...
    switch (field.messageKey) {
      ...
      case 'select_department':
        ...
        let leafEnable = React.Component.prototype.checkFunctionProfiles(
          'department.leaf.selection.required',
          [undefined, false]
        )
          ? false
          : true;
        ...
        return (
          <Chooser
            ...
            labelKey={
              React.Component.prototype.checkFunctionProfiles('department.full.path.disabled', [
                true,
              ])
                ? 'name'
                : 'name'
            ...
            disabled={
              type === 1
                ...
                : React.Component.prototype.checkFunctionProfiles(
                    'invoice.header.multi.currency.allowed',
                    [false]
                  )
            }
          />
      ...
      case 'select_cost_center':
        ...
        chooserItem.url = `${config.baseUrl}/api/my/cost/center/items/${field.dataSource &&
          JSON.parse(field.dataSource || '{}').costCenterOID}`;
        return (
          <Chooser
            selectorItem={chooserItem}
            placeholder={field.promptInfo}
      ...
      case 'select_participant':
        ...
        chooserItem.url += `?proposerOID=${
          configureStore.store.getState().login.user.userOID
        }${departmentOIDSet}${costCentreValuesSet}`;
        return (
          <Chooser
            selectorItem={chooserItem}
            valueKey="userOID"
        ...
        if (formDetail.formType != 2005) {
          return (
            <InputNumber
              min={0}
              precision={2}
              style={{ width: '90%', marginRight: 0, float: 'right' }}
            />
          );
        } else {
          return (
            <InputNumber
              min={0}
              precision={2}
              style={{ width: '100%', marginRight: 0, float: 'right' }}
            />
          );
        }
      case 'average_budget':
        ...
        return (
          <DatePicker
            disabledDate={date => {
              return (
                date &&
                date.valueOf() < new Date(new Date().format('yyyy-MM-dd 00:00:00')).valueOf()
              );
            }}
          />
        );
      ...
      case 'cust_list':
        ...
        return <Selector selectorItem={selectorItem} getPopupContainer={getPopupContainer} />;
      case 'number':
        let max = field.fieldConstraint
          ? Math.pow(10, JSON.parse(field.fieldConstraint || '{}').integerMaxLength) -
            Math.pow(0.1, JSON.parse(field.fieldConstraint || '{}').decimalMaxLength)
          : Infinity;
        //修正整数部分长度
        let fieldCon = field.fieldConstraint
          ? JSON.parse(field.fieldConstraint)
          : { decimalMaxLength: 2, integerMaxLength: 'default' };
        ...
        let formatNumber = function(v) {
          ...
          if (temp) {
            ...
            let arr = v.split('.');
            v =
              arr[0].substr(0, fieldCon.integerMaxLength) +
              `${havePoint ? '.' : ''}${arr[1] ? arr[1].substr(0, pre) : ''}`;
          }
          return v;
        };
        return (
      ...
      case 'employee_expand':
        if (fieldDefaultValue && fieldDefaultValue.code === 'image') {
          return (
            <ImageUpload
              defaultFileList={fieldDefaultValue.attachmentImages}
              attachmentType="INVOICE_IMAGES"
              disabled
            />
          );
          ...
          field.fieldContent &&
          (JSON.parse(field.fieldContent) || {}).messageKey === 'cust_list'
        ) {
          let OID = '';
      ...
      case 'select_special_booking_person':
        return (
          <Chooser type="booker" valueKey="userOID" labelKey="fullName" onlyNeed="userOID" single />
        );
      case 'external_participant_name':
        ...
        JSON.parse(field.fieldContent || '{}').isContainCard &&
          external_participant_name_columns.push(external_participant_name_id);
      ...
      case 'start_date':
        ...
        return (
          <SelectDateForm
            copyValue={copyValue}
            field={field}
            callFun={(isSet, date) => copyValue.expectStopDate(isSet, date)}
          />
        );
      case 'end_date':
      ...
      case 'select_company':
        ...
        let selectorItemCompany = {
          url: `${config.baseUrl}/api/refactor/companies/user/setOfBooks?userOID=${
            configureStore.store.getState().login.user.userOID
          }&enabled=true&page=0&size=1000`,
      ...
      case 'applicant':
        ...
        // 关联申请单会修改url信息
        field.applicationOID &&
          (selectorItemApplicant.url = `${selectorItemApplicant.url}?applicationOID=${
            field.applicationOID
          }`);
        ...
        return (
          <Selector
            disabled={true}
            selectorItem={selectorItemApplicant}
            onChange={(e, all) => {
              copyValue && copyValue.checkedChange && copyValue.checkedChange(field, e, all);
            }}
            getPopupContainer={getPopupContainer}
  ...
  getInitialValue(field) {
    ...
    switch (field.messageKey) {
      ...
      case 'select_cost_center':
        return field.value
          ? [{ name: field.showValue, costCenterItemOID: field.value }]
          : undefined;
      case 'select_corporation_entity':
        return field.value
          ? [{ companyName: field.showValue, companyReceiptedOID: field.value }]
          : undefined;
      case 'select_participant':
        let initValue = field.clearDefault
          ? []
      ...
      case 'select_approver':
        field.showValue &&
          field.showValue.split(',').map((name, index) => {
      ...
      case 'contact_bank_account':
        return field.value
          ? [
              {
                bankAccountNo:
                  field.showValue && this.isJson(field.showValue)
                    ? JSON.parse(field.showValue).bankAccountNo
                    : field.showValue,
      ...
      case 'time':
        ...
        return field.showValue
          ? moment(field.showValue)['_isValid']
            ? moment(field.showValue)
            : moment(new Date(`2000/01/01 ${field.showValue}`))
          : undefined;
      case 'switch':
      ...
      case 'select_user':
        ...
        return field.value
          ? JSON.parse(field.fieldConstraint || '{}').selectMode
            ? values
            : [values[0]]
          : undefined;
      case 'employee_expand':
    ...
    switch (field.messageKey) {
      ...
      case 'select_approver':
        if (fieldDefaultValue.value) {
          ...
          fieldDefaultValue.value &&
            fieldDefaultValue.value.split(':').map((value, index) => {
      ...
      case 'select_corporation_entity':
        if (JSON.parse(field.fieldConstraint || '{}').default && fieldDefaultValue.value) {
          return [
            { companyName: fieldDefaultValue.name, companyReceiptedOID: fieldDefaultValue.value },
          ];
        }
        return undefined;
      case 'select_company':
        if (!JSON.parse(field.fieldConstraint || '{}').noDefault && fieldDefaultValue.value) {
          ...
          return {
            ...
            label: showCode
              ? `${fieldDefaultValue.name} - ${fieldDefaultValue.code}`
      ...
      case 'employee_expand':
        if (fieldDefaultValue.code === 'time') {
          return moment(fieldDefaultValue.value)['_isValid']
            ? moment(fieldDefaultValue.value).format('HH:mm:ss')
            : fieldDefaultValue.value;
        }
        if (fieldDefaultValue.code === 'date') {
          return moment(fieldDefaultValue.value)['_isValid']
            ? moment(fieldDefaultValue.value).format('YYYY.MM.DD')
            : fieldDefaultValue.value;
        }
      ...
      case 'payee':
        return {
          label: `${user.employeeID} | ${user.fullName} | ${
            user.departmentName
              ? user.departmentName
              : messages('expense.invoice.type.unknown') /*"未知"*/
          } | ${user.title || messages('expense.invoice.type.unknown') /*"未知"*/}`,
          key: user.userOID,
        };
      case 'select_participant':
        let initValue = field.clearDefault
  ...
  formatFormValue(field, value) {
    ...
    if (field.messageKey === 'employee_expand') {
      if (field.attachmentImages && field.attachmentImages.length) {
        //个人扩展字段为图片时
    ...
    if (field.messageKey === 'start_date' && value instanceof moment) {
      //wjk add 18 04 16
      value = value ? value.utc().format() : '';
    }
    if (field.messageKey === 'end_date' && value instanceof moment) {
      //wjk add 18 04 16
      value = value ? value.utc().format() : '';
    }
  ...
  instructionsTag(customFormPropertyMap) {
    if (
      ...
      customFormPropertyMap['form.description'] &&
      JSON.parse(customFormPropertyMap['form.description'])
    ) {
      ...
      if (instructions.enable) {
        ...
        return (
          ...
          >
            <Icon
              type="info-circle"
              style={{ color: '#1890ff', marginRight: 8, position: 'relative' }}
            />
            {title}
          </a>
        );
